...
enum class status {
    ...
    interator_ends = dnnl_iterator_ends,
...
inline dnnl::engine make_engine_with_allocator(
    ...
    error::wrap_c_api(
            dnnl_graph_make_engine_with_allocator(&c_engine,
                    dnnl::engine::convert_to_c(kind), index, alloc.get()),
...
public:
    ...
    bool has_same_layout(const logical_tensor &lt) const {
        uint8_t is_same {0};
        error::wrap_c_api(dnnl_graph_logical_tensor_has_same_layout(
...
public:
    ...
    op &set_attr(attr name, const Type &value) {
        ...
        error::wrap_c_api(dnnl_graph_op_set_attr_s64(get(), attr, &value, 0),
    ...
    op &set_attr(attr name, const Type &value) {
        dnnl_graph_op_attr_t attr = convert_to_c(name);
        error::wrap_c_api(dnnl_graph_op_set_attr_f32(get(), attr, &value, 0),
                "could not set attribute to the op");
        return *this;
    }
    ...
    op &set_attr(attr name, const Type &value) {
        ...
        error::wrap_c_api(dnnl_graph_op_set_attr_bool(get(), attr, &val, 0),
                "could not set attribute to the op");
        return *this;
    }
...
public:
    ...
    std::vector<logical_tensor> get_in_ports() const {
        size_t num = 0;
        error::wrap_c_api(dnnl_graph_partition_get_in_ports_num(get(), &num),
    ...
    std::vector<logical_tensor> get_out_ports() const {
        size_t num = 0;
        error::wrap_c_api(dnnl_graph_partition_get_out_ports_num(get(), &num),
                "cannot get number of outputs of the partition");
        if (num == 0) return {};

        std::vector<dnnl_graph_logical_tensor_t> c_outputs(num);
        error::wrap_c_api(dnnl_graph_partition_get_out_ports(
    ...
    kind get_kind() const {
        ...
        error::wrap_c_api(dnnl_graph_partition_get_kind(get(), &pkind),
...
public:
    ...
    graph(dnnl::engine::kind engine_kind) {
        ...
        error::wrap_c_api(dnnl_graph_graph_create(
                                  &g, dnnl::engine::convert_to_c(engine_kind)),
    ...
    graph(dnnl::engine::kind engine_kind, dnnl::fpmath_mode mode) {
        dnnl_graph_graph_t g = nullptr;
        error::wrap_c_api(dnnl_graph_graph_create_with_fpmath_mode(&g,
                                  dnnl::engine::convert_to_c(engine_kind),
                                  convert_to_c(mode)),
