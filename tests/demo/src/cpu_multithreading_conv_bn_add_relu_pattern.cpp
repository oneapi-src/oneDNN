...
/// @example cpu_multithreading_conv_bn_add_relu_pattern.cpp
/// @copybrief cpu_multithreading_conv_bn_add_relu_pattern_cpp
/// > Annotated version: @ref cpu_multithreading_conv_bn_add_relu_pattern_cpp

/// @page cpu_multithreading_conv_bn_add_relu_pattern_cpp CPU example for multithread conv+bn+add+relu pattern
///
/// > Example code: @ref cpu_multithreading_conv_bn_add_relu_pattern.cpp
...
int main(int argc, char **argv) {
    ...
    for (size_t t_num=0; t_num < thread_num; t_num++) {
        float *actual_output_ptr = tms[t_num].get(relu1_dst_desc.get_id()).get_data_handle<float>();
