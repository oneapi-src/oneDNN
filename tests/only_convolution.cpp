...
int doit(bool lazy) {
    ...
    auto c3_src_desc = memory::desc({1, 1, 2, {2, 256, 13, 13}}, memory::precision::f32, memory::format::nchw);
    ...
    auto c3_dst_desc = memory::desc({1, 1, 2, {2, 384, 13, 13}}, memory::precision::f32, memory::format::nchw);

    auto c3_src = memory({c3_src_desc, cpu_engine});
...
#if 0
    auto c3_src = memory({{{1, 1, 2, {2, 256, 13, 13}}, memory::format::nchw}, cpu_engine});
    ...
    auto c3_dst = memory({{{1, 1, 2, {2, 384, 13, 13}}, memory::format::nchw}, cpu_engine});
