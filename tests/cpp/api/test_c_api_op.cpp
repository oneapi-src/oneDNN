...
TEST(c_api_test, create_op) {
    ...
    ASSERT_EQ(dnnl_graph_op_create(&op, 1, op_kind, "conv2d"),
...
TEST(c_api_test, op_attr) {
    ...
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      op, "strides", dnnl_graph_attribute_kind_is, &strides, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      op, "auto_pad", dnnl_graph_attribute_kind_s, auto_pad, 1),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      op, "groups", dnnl_graph_attribute_kind_i, &groups, 1),
            dnnl_graph_result_success);
    ...
    int64_t groups_num {};
    ASSERT_EQ(dnnl_graph_op_get_attr(op, "strides",
                      dnnl_graph_attribute_kind_is, &got_strides, &strides_num),
            dnnl_graph_result_success);
    const auto new_strides = reinterpret_cast<const int64_t *>(got_strides);
    ASSERT_EQ(strides_num, 2);
    ASSERT_EQ(*new_strides, 4);
    ASSERT_EQ(*(new_strides + 1), 4);
    ...
    ASSERT_EQ(*new_groups, 2);
    ASSERT_EQ(dnnl_graph_op_destroy(op), dnnl_graph_result_success);

    dnnl_graph_op_t *matmul_op;
    ASSERT_EQ(dnnl_graph_op_create(&matmul_op, 2, kMatMul, "matmul"),
            dnnl_graph_result_success);
    bool transpose_a = true;
    bool transpose_b = false;
    ASSERT_EQ(dnnl_graph_op_add_attr(matmul_op, "transpose_a",
                      dnnl_graph_attribute_kind_b, &transpose_a, 1),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(matmul_op, "transpose_b",
    ...
    ASSERT_EQ(dnnl_graph_op_get_attr_kind(
                      matmul_op, "transpose_a", &got_transpose_a_kind),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_get_attr(matmul_op, "transpose_a",
    ...
    ASSERT_EQ(*new_transpose_a, true);
    ...
    ASSERT_EQ(dnnl_graph_op_get_attr_kind(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_get_attr(matmul_op, "transpose_b",
    ...
    ASSERT_EQ(*new_transpose_b, false);

    ASSERT_EQ(dnnl_graph_op_destroy(matmul_op), dnnl_graph_result_success);
}
