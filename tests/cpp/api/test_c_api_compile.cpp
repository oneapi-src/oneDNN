...
TEST(c_api_test, compile_bn) {
    ...
    ASSERT_EQ(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&beta, 3, dnnl_graph_f32,
                      1, bn_param_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&mean, 4, dnnl_graph_f32,
                      1, bn_param_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&var, 5, dnnl_graph_f32,
                      1, bn_param_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(
    ...
    ASSERT_EQ(dnnl_graph_op_add_attr(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      bn, "data_format", dnnl_graph_attribute_kind_s, "NCX", 1),
            dnnl_graph_result_success);
    ...
    ASSERT_EQ(
            dnnl_graph_partition_create(&partition), dnnl_graph_result_success);
    ...
    ASSERT_EQ(dnnl_graph_partition_compile(
                      partition, compiled_partition, 5, inputs, 1, outputs, e),
            dnnl_graph_result_success);
    ...
    ASSERT_EQ(
            dnnl_graph_partition_destroy(partition), dnnl_graph_result_success);
...
TEST(c_api_test, compile_conv2d) {
    ...
    ASSERT_EQ(dnnl_graph_op_add_attr(conv2d, "filter_format",
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      conv2d, "groups", dnnl_graph_attribute_kind_i, &group, 1),
            dnnl_graph_result_success);
    ...
    api_test_dnnl_graph_engine_create(&e, engine);
    ASSERT_EQ(dnnl_graph_partition_compile(
                      partition, compiled_partition, 2, inputs, 1, outputs, e),
            dnnl_graph_result_success);
    ...
    // NOTE: In real application, you will need this.
    // ASSERT_EQ(dnnl_graph_engine_destroy(e), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
...
TEST(c_api_test, compile_conv2d_bias_sum) {
    ...
    ASSERT_EQ(
            dnnl_graph_logical_tensor_init_with_dims(&output, 3, dnnl_graph_f32,
                    4, output_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bias, 4, dnnl_graph_f32,
                      1, bias_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bias_add_output, 5,
                      dnnl_graph_f32, 4, output_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&sum_src2, 6,
                      dnnl_graph_f32, 4, output_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&sum_dst, 7,
                      dnnl_graph_f32, 4, output_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ...
    ASSERT_EQ(dnnl_graph_add_op(agraph, conv2d), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_add_op(agraph, bias_add), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_add_op(agraph, sum), dnnl_graph_result_success);
    ASSERT_EQ(
            dnnl_graph_graph_filter(agraph, policy), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_get_partition_num(agraph, &part_num),
            dnnl_graph_result_success);
    ...
    api_test_dnnl_graph_engine_create(&e, engine);
    ASSERT_EQ(dnnl_graph_partition_compile(
                      partition, compiled_partition, 4, inputs, 1, outputs, e),
            dnnl_graph_result_success);
    ...
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(conv2d), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(bias_add), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(sum), dnnl_graph_result_success);
    ASSERT_EQ(
            dnnl_graph_partition_destroy(partition), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_compiled_partition_destroy(compiled_partition),
            dnnl_graph_result_success);
...
TEST(c_api_test, compile_conv2d_sum_conv2d) {
    dnnl_graph_graph_t *agraph;
    dnnl_graph_op_t *conv0;
    dnnl_graph_op_t *conv1;
    dnnl_graph_op_t *sum;
    dnnl_graph_engine_kind_t engine = api_test_engine_kind;
    dnnl_graph_partition_policy_t policy = dnnl_graph_partition_policy_max;
    dnnl_graph_partition_t **partition;
    ...
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&conv0_input, 1,
    ...
    ASSERT_EQ(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&conv1_input, 4,
                      dnnl_graph_f32, 4, input_dim, dnnl_graph_layout_type_any),
            dnnl_graph_result_success);
    ASSERT_EQ(
    ...
    ASSERT_EQ(dnnl_graph_op_add_attr(conv0, "strides",
                      dnnl_graph_attribute_kind_is, &stride, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv0, "pads_begin",
                      dnnl_graph_attribute_kind_is, &padding, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv0, "pads_end",
                      dnnl_graph_attribute_kind_is, &padding, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv0, "dilations",
                      dnnl_graph_attribute_kind_is, &dilations, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv0, "data_format",
                      dnnl_graph_attribute_kind_s, "NCX", 1),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv0, "filter_format",
                      dnnl_graph_attribute_kind_s, "OIX", 1),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      conv0, "groups", dnnl_graph_attribute_kind_i, &group, 1),
            dnnl_graph_result_success);

    ASSERT_EQ(dnnl_graph_op_add_attr(conv1, "strides",
                      dnnl_graph_attribute_kind_is, &stride, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv1, "pads_begin",
                      dnnl_graph_attribute_kind_is, &padding, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv1, "pads_end",
                      dnnl_graph_attribute_kind_is, &padding, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv1, "dilations",
                      dnnl_graph_attribute_kind_is, &dilations, 2),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv1, "data_format",
                      dnnl_graph_attribute_kind_s, "NCX", 1),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(conv1, "filter_format",
                      dnnl_graph_attribute_kind_s, "OIX", 1),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_add_attr(
                      conv1, "groups", dnnl_graph_attribute_kind_i, &group, 1),
            dnnl_graph_result_success);

    ASSERT_EQ(dnnl_graph_add_op(agraph, conv0), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_add_op(agraph, conv1), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_add_op(agraph, sum), dnnl_graph_result_success);
    ASSERT_EQ(
            dnnl_graph_graph_filter(agraph, policy), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_get_partition_num(agraph, &part_num),
    ...
    for (size_t i = 0; i < part_num; ++i) {
        ASSERT_EQ(dnnl_graph_partition_create(partition + i),
                dnnl_graph_result_success);
    }
    ASSERT_EQ(dnnl_graph_graph_get_partitions(agraph, part_num, partition),
            dnnl_graph_result_success);
    ...
    ASSERT_EQ(dnnl_graph_partition_compile(*partition, *compiled_partition, 2,
                      p0_inputs, 1, p0_outputs, e),
            dnnl_graph_result_success);

    dnnl_graph_logical_tensor_t opt_conv1_output;
    ASSERT_EQ(dnnl_graph_compiled_partition_query_logical_tensor(
    ...
    // ASSERT_EQ(dnnl_graph_engine_destroy(e), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(conv0), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(conv1), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(sum), dnnl_graph_result_success);
    for (size_t i = 0; i < part_num; ++i) {
        ASSERT_EQ(dnnl_graph_partition_destroy(*(partition + i)),
                dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_compiled_partition_destroy(
                          *(compiled_partition + i)),
                dnnl_graph_result_success);
    }
    free(partition);
...
TEST(c_api_test, compile_conv2d_with_unknown_shape) {
    ...
    ASSERT_EQ(dnnl_graph_partition_infer_shape(
    ...
    ASSERT_EQ(dnnl_graph_partition_compile(partition, compiled_partition, 2,
                      const_inputs, 1, const_outputs, e),
            dnnl_graph_result_success);
...
TEST(c_api_test, compile_add) {
    ...
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&src0, 0, dnnl_graph_f32,
                      4, src0_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&src1, 1, dnnl_graph_f32,
                      4, src1_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&dst, 2, dnnl_graph_f32,
...
TEST(c_api_test, compile_conv_bn) {
    ...
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&conv_input, 1,
                      dnnl_graph_f32, 4, conv_input_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&conv_weight, 2,
                      dnnl_graph_f32, 4, conv_weight_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&conv_output, 4,
                      dnnl_graph_f32, 4, conv_output_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_scale, 5,
                      dnnl_graph_f32, 1, bn_scale_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_shift, 6,
                      dnnl_graph_f32, 1, bn_shift_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_mean, 7,
    ...
    ASSERT_EQ(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_output, 9,
    ...
    api_test_dnnl_graph_engine_create(&e, engine);
    ASSERT_EQ(dnnl_graph_partition_compile(
                      partition, compiled_partition, 6, inputs, 1, outputs, e),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(conv2d), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(bn), dnnl_graph_result_success);
...
TEST(c_api_test, compile_conv_bn_standalone) {
    ...
    dnnl_graph_engine_kind_t engine = api_test_engine_kind;
    dnnl_graph_partition_policy_t policy = dnnl_graph_partition_policy_fusion;
    dnnl_graph_partition_t *partition[2];
    dnnl_graph_compiled_partition_t *compiled_partition[2];
    dnnl_graph_logical_tensor_t conv_input;
    dnnl_graph_logical_tensor_t conv_weight;
    dnnl_graph_logical_tensor_t conv_output;
    ...
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_input, 5,
                      dnnl_graph_f32, 4, bn_input_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_gamma, 6,
                      dnnl_graph_f32, 1, bn_params_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_beta, 7,
                      dnnl_graph_f32, 1, bn_params_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_mean, 8,
                      dnnl_graph_f32, 1, bn_params_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&bn_output, 10,
                      dnnl_graph_f32, 4, bn_output_dim,
                      dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ...
    ASSERT_EQ(part_num, 2);
    ASSERT_EQ(dnnl_graph_partition_create(&partition[0]),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_partition_create(&partition[1]),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_get_partitions(agraph, part_num, partition),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_compiled_partition_create(
    ...
    ASSERT_EQ(dnnl_graph_compiled_partition_create(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_partition_get_ops(partition[0], 1, ops),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_partition_get_ops(partition[1], 1, ops),
            dnnl_graph_result_success);

    dnnl_graph_engine_t *e;
    api_test_dnnl_graph_engine_create(&e, engine);
    ASSERT_EQ(dnnl_graph_partition_compile(partition[1], compiled_partition[1],
                      2, conv_inputs, 1, conv_outputs, e),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_partition_compile(partition[0], compiled_partition[0],
                      5, bn_inputs, 1, bn_outputs, e),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(conv2d), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(bn), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_partition_destroy(partition[0]),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_partition_destroy(partition[1]),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_compiled_partition_destroy(compiled_partition[0]),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_compiled_partition_destroy(compiled_partition[1]),
            dnnl_graph_result_success);
}

TEST(c_api_test, compile_matmul_add_1d) {
    dnnl_graph_graph_t *agraph;
    dnnl_graph_op_t *matmul;
    dnnl_graph_op_t *add;
    dnnl_graph_engine_kind_t engine = api_test_engine_kind;
    dnnl_graph_partition_policy_t policy = dnnl_graph_partition_policy_max;
    dnnl_graph_partition_t *partition;
    ...
    ASSERT_EQ(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&matmul_output, 3,
    ...
    ASSERT_EQ(
            ...
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&add_output, 5,
    ...
    ASSERT_EQ(dnnl_graph_add_op(agraph, matmul), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_add_op(agraph, add), dnnl_graph_result_success);
    ASSERT_EQ(
            dnnl_graph_graph_filter(agraph, policy), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_get_partition_num(agraph, &part_num),
    ...
    api_test_dnnl_graph_engine_create(&e, engine);
    ASSERT_EQ(dnnl_graph_partition_compile(
                      partition, compiled_partition, 3, inputs, 1, outputs, e),
            dnnl_graph_result_success);

    // NOTE: In real application, you will need this.
    // ASSERT_EQ(dnnl_graph_engine_destroy(e), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(matmul), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(add), dnnl_graph_result_success);
    ASSERT_EQ(
            dnnl_graph_partition_destroy(partition), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_compiled_partition_destroy(compiled_partition),
...
TEST(c_api_test, compile_matmul_add_activation) {
    ...
    for (size_t i = 0; i < activation_kinds.size(); i++) {
        ...
        ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&input, 1,
                          dnnl_graph_f32, 2, input_dim,
                          dnnl_graph_layout_type_strided),
                dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&weight, 2,
        ...
        ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&matmul_output, 3,
                          ...
                          dnnl_graph_layout_type_strided),
                dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&other, 4,
        ...
        ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&add_output, 5,
                          ...
                          dnnl_graph_layout_type_strided),
                dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&activation_output,
                          6, dnnl_graph_f32, 2, activation_output_dim,
                          dnnl_graph_layout_type_strided),
                dnnl_graph_result_success);
        ...
        ASSERT_EQ(dnnl_graph_add_op(agraph, matmul), dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_add_op(agraph, add), dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_add_op(agraph, activation),
                dnnl_graph_result_success);
        ...
        dnnl_graph_engine_t *e;
        api_test_dnnl_graph_engine_create(&e, engine);
        ASSERT_EQ(dnnl_graph_partition_compile(partition, compiled_partition, 3,
                          inputs, 1, outputs, e),
                dnnl_graph_result_success);
        ...
        ASSERT_EQ(dnnl_graph_op_destroy(matmul), dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_op_destroy(add), dnnl_graph_result_success);
        ASSERT_EQ(dnnl_graph_op_destroy(activation), dnnl_graph_result_success);
...
TEST(c_api_test, compile_softmax) {
    ...
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&src, 0, dnnl_graph_f32,
                      2, src_dim, dnnl_graph_layout_type_strided),
            dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_logical_tensor_init_with_dims(&dst, 1, dnnl_graph_f32,
    ...
    ASSERT_EQ(dnnl_graph_add_op(agraph, softmax), dnnl_graph_result_success);
    ...
    const dnnl_graph_logical_tensor_t *const_dst[1] = {&dst};
    ASSERT_EQ(dnnl_graph_partition_compile(partition, compiled_partition, 1,
                      const_inputs, 1, const_dst, e),
            dnnl_graph_result_success);
    ...
    //     ASSERT_EQ(dnnl_graph_engine_destroy(e), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_graph_destroy(agraph), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_op_destroy(softmax), dnnl_graph_result_success);
    ASSERT_EQ(
            dnnl_graph_partition_destroy(partition), dnnl_graph_result_success);
    ASSERT_EQ(dnnl_graph_compiled_partition_destroy(compiled_partition),
            dnnl_graph_result_success);
