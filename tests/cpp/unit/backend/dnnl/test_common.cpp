...
TEST(Common, MallocAndFree) {
    ...
    void *mem = nullptr;
    ASSERT_NO_THROW(mem = dnnl_impl::dnnl_allocator_t::malloc(8, p_engine_,
                            g_alloc_, impl::allocator_t::mem_type_t::temp));
    if (mem != nullptr && p_engine_.get_kind() == dnnl::engine::kind::cpu) {
#ifdef DNNL_GRAPH_CPU_SYCL
#ifndef NDEBUG
        EXPECT_DEATH(
                dnnl_impl::dnnl_allocator_t::free(mem, p_engine_, g_alloc_),
                "use event based free");
#endif
#else
        ASSERT_NO_THROW(
                dnnl_impl::dnnl_allocator_t::free(mem, p_engine_, g_alloc_));
            ...
            && p_engine_.get_kind() == dnnl::engine::kind::gpu) {
...
TEST(Common, GetNxcStride) {
    impl::dims shape {1, 2, 3, 4, 5, 6};
    impl::dims shape_def {720, 1, 240, 60, 12, 2};
    const auto &result = impl::dnnl_impl::get_nxc_strides(shape);
...
TEST(Common, MakeDnnlMemory) {
    ...
    impl::tensor_t t1 {lt, &eng, nullptr};
    if (eng.kind() == impl::engine_kind::cpu) {
        ASSERT_NO_THROW(impl::dnnl_impl::make_dnnl_memory(
                t1, dnnl::engine(dnnl::engine::kind::cpu, 0)));
    } else if (eng.kind() == impl::engine_kind::gpu) {
        ASSERT_NO_THROW(impl::dnnl_impl::make_dnnl_memory(
...
TEST(Common, FillLayoutInfo) {
    {
        ...
        lt.ndims = -1;
        dnnl::memory::desc md;
        ASSERT_EQ(dnnl_impl::fill_layout_info(&lt, md), impl::status::success);
        ASSERT_EQ(lt.layout_type, impl::layout_type::undef);
    }
    {
        impl::logical_tensor_t lt = utils::logical_tensor_init(
                0, {1, 2}, impl::data_type::f32, impl::layout_type::any);
    ...
    }
    {
        ...
        dnnl::memory::desc md {{1, 2}, dnnl::memory::data_type::f32, {2, 1}};
        ASSERT_EQ(dnnl_impl::fill_layout_info(&lt, md), impl::status::success);
        ASSERT_TRUE(lt.data_type == impl::data_type::f32 && lt.ndims == 2);
        std::vector<impl::dim_t> dims {1, 2};
        ASSERT_EQ(ltw(lt).vdims(), dims);
        std::vector<impl::dim_t> strides {2, 1};
