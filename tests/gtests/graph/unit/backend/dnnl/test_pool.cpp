...
TEST(Execute, AvgPoolBackwardExcludePad) {
    ...
    graph::op_t avg_pool_bwd_op(graph::op_kind::AvgPoolBackprop);
    ...
    avg_pool_bwd_op.set_attr<std::vector<int64_t>>(
            graph::op_attr::input_shape, std::vector<int64_t> {1, 1, 4, 4});
...
TEST(Execute, MaxPoolBackwardWithIncides) {
    ...
    graph::op_t max_pool_bwd_op(graph::op_kind::MaxPoolBackprop);
    ...
    graph::logical_tensor_t indices_lt;
    if (get_test_engine_kind() == graph::engine_kind::cpu) {
        indices_lt = utils::logical_tensor_init(
                4, {1, 1, 2, 2}, graph::data_type::u8);
    } else {
        indices_lt = utils::logical_tensor_init(
                4, {1, 1, 2, 2}, graph::data_type::s32);
    }
    ...
    graph::tensor_t diff_dst_ts(diff_dst_lt, eng, diff_dst.data());
    graph::tensor_t diff_src_ts(lt, eng, diff_src.data());
    graph::tensor_t indices_ts(indices_lt, eng, indices_data);
