...
TEST(test_sdp_decomp_execute, F32SdpDecomp) {
    ...
    utils::construct_dnnl_float_MHA(&g, dnnl::impl::data_type::f32, batch_size,
            seq_len, num_head, head_dim);
    g.finalize();
    dims QUERY_VALUE_STRIDES = {seq_len * head_dim, size_per_head, head_dim, 1};
    dims KEY_STRIDES = {seq_len * head_dim, size_per_head, 1, head_dim};
    ...
    //mm1 src format tag: acbd
    std::copy(QUERY_VALUE_STRIDES.begin(), QUERY_VALUE_STRIDES.begin() + ndims,
            partition_inputs[0].layout.strides);
    //mm1 wei format tag: adbc
    std::copy(KEY_STRIDES.begin(), KEY_STRIDES.begin() + ndims,
            partition_inputs[1].layout.strides);
    //mm2 wei format tag: acbd
    std::copy(QUERY_VALUE_STRIDES.begin(), QUERY_VALUE_STRIDES.begin() + ndims,
...
TEST(test_sdp_decomp_execute, Bf16SdpDecomp) {
    ...
    int batch_size = 56, seq_len = 384, num_head = 16, head_dim = 256,
        size_per_head = head_dim / num_head;
    utils::construct_dnnl_float_MHA(&g, dnnl::impl::data_type::bf16, batch_size,
            seq_len, num_head, head_dim);
    g.finalize();
    dims QUERY_VALUE_STRIDES = {seq_len * head_dim, size_per_head, head_dim, 1};
    dims KEY_STRIDES = {seq_len * head_dim, size_per_head, 1, head_dim};
...
TEST(test_sdp_decomp_execute, Int8SdpDecomp) {
    ...
    int batch_size = 56, seq_len = 384, num_head = 16, head_dim = 1024,
        size_per_head = head_dim / num_head;
    utils::construct_int8_MHA(&g, batch_size, seq_len, num_head, head_dim);
    g.finalize();
    dims QUERY_VALUE_STRIDES = {seq_len * head_dim, size_per_head, head_dim, 1};
    dims KEY_STRIDES = {seq_len * head_dim, size_per_head, 1, head_dim};
...
TEST(test_sdp_decomp_execute, Int8Bf16SdpDecomp) {
    ...
    int batch_size = 56, seq_len = 384, num_head = 16, head_dim = 1024,
        size_per_head = head_dim / num_head;
    utils::construct_int8_bf16_MHA(&g, batch_size, seq_len, num_head, head_dim);
    g.finalize();
    dims QUERY_VALUE_STRIDES = {seq_len * head_dim, size_per_head, head_dim, 1};
    dims KEY_STRIDES = {seq_len * head_dim, size_per_head, 1, head_dim};
...
TEST(test_sdp_decomp_execute, F32SdpCorr) {
    ...
    ASSERT_TRUE(allclose<float>(outputs1_ts[0], outputs2_ts[0], /*rtol*/ 0.01f,
            /*atol*/ 1.f));
}
...
TEST(test_sdp_decomp_execute, Bf16SdpCorr) {
    ...
    ASSERT_TRUE(
            allclose<bfloat16_t>(outputs1_ts[0], outputs2_ts[0], /*rtol*/ 0.01f,
                    /*atol*/ 1.f));
}
...
TEST(test_sdp_decomp_execute, Int8SdpCorr) {
    ...
    ASSERT_TRUE(allclose<int8_t>(outputs1_ts[0], outputs2_ts[0], /*rtol*/ 0.01f,
            /*atol*/ 1.f));
}
    ...
    auto func = [&]() {
        ...
        for (int i = 0; i < 10; i++)
            ASSERT_EQ(cp.execute(strm, test_tensor::to_graph_tensor(inputs_ts),
                    ...
                    graph::status::success);
        ASSERT_TRUE(
                allclose<int8_t>(outputs1_ts[0], outputs_ts[0], /*rtol*/ 0.01f,
                        /*atol*/ 1.f));
    };
