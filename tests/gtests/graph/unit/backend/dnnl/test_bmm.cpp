...
TEST(ExecuteSubgraphInt8, U8u8bf16DivBmm) {
    ...
    // u8 2 s8 shift by using reorder with -128 zps is not supported on
    // GPU
    SKIP_IF(engine->kind() == graph::engine_kind::gpu, "Skip on GPU device.");
    ...
    graph::logical_tensor_t div_src1
            = utils::logical_tensor_init(7, {1}, graph::data_type::bf16);
    graph::logical_tensor_t div_bf16
            = utils::logical_tensor_init(8, src_shape, graph::data_type::bf16);
...
TEST(ExecuteSubgraphInt8, U8u8bf16DivBmmUnfusible) {
    ...
    graph::logical_tensor_t dst_bf16
            = utils::logical_tensor_init(6, dst_shape, graph::data_type::bf16);
    graph::logical_tensor_t div_src1 = utils::logical_tensor_init(
            7, div_src1_shape, graph::data_type::bf16);
    graph::logical_tensor_t div_bf16
            = utils::logical_tensor_init(8, src_shape, graph::data_type::bf16);
