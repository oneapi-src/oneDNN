...
TEST_F(runtime_attr_test_t, TestPool) {
    ...
    pooling_v2_forward::desc op_d(prop_kind::forward_inference,
            algorithm::pooling_max, src_md, dst_md, {2, 2}, {2, 2}, {0, 0},
            {0, 0}, {0, 0});
    CHECK_OK(pooling_v2_forward::primitive_desc(op_d, eng));
    CHECK_UNIMPL(pooling_v2_forward::primitive_desc(
            op_d, gen_attr_with_oscale(false), eng));
    CHECK_UNIMPL(pooling_v2_forward::primitive_desc(
            op_d, gen_attr_with_oscale(true), eng));
