            ...
            for (int ic = 0; ic < c.ic / c.ng; ++ic) {
                for (int kh = 0; kh < c.kh; kh++) {
                    for (int kw = 0; kw < c.kw; kw++) {
                        ...
                        weights_data[map_index(weights_d, widx)] = 0.0;
...
protected:
    virtual void SetUp()
    {
        ...
        auto aprop_kind = prop_kind::backward_weights;
        ...
        auto ref_memory = memory(memory::primitive_desc(c_weights_desc, eng));
        compute_ref_conv_bwd_weights<data_t>(cd, c_src, ref_memory, c_dst);
        compare_data<data_t>(ref_memory, c_weights);
    }
};
