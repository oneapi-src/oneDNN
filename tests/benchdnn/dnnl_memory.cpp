    ...
    switch (dt(buffer_index)) {
        ...
        case dnnl_s4:
            elem = static_cast<float>(dnnl::impl::int4_t::extract(
                    static_cast<uint8_t *>(data)[idx / 2], idx % 2));
            break;
        case dnnl_u4:
...
void dnn_mem_t::set_elem(int64_t idx, float value, int buffer_index) const {
    ...
    switch (dt(buffer_index)) {
        ...
        case dnnl_s4: {
            ...
            bool high_half = idx % 2;
            uint8_t dst_val = high_half ? ((uint8_t *)data)[idx / 2] : 0;
            ((type *)data)[idx / 2] = type(value).insert(dst_val, high_half);
            break;
        }
        case dnnl_u4: {
