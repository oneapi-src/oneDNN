...
void gru_fwd(const prb_t &p, float *dst_iter_h_, float *gates_,
    ...
    AOC<const float> weights_iter_h(weights_iter_h_, p.sic, p.n_gates(), p.dhc);
    AOC<float> gates(gates_, p.mb, p.n_gates(), p.dhc);

    gemm("C", "N", "N", p.mb, p.n_gates() * p.dhc, p.slc, 1.0, src_layer_, p.wc,
            weights_layer_, p.n_gates() * p.dhc, 0.0, gates_,
            p.n_gates() * p.dhc);
    gemm("C", "N", "N", p.mb, (p.n_gates() - 1) * p.dhc, p.sic, 1.0,
            src_iter_h_, p.wc, weights_iter_h_, p.n_gates() * p.dhc, 1.0,
            gates_, p.n_gates() * p.dhc);

    gru_fwd_postgemm_part1(p, gates_, src_iter_h_, bias_, dst_iter_h_);

    gemm("C", "N", "N", p.mb, p.dhc, p.sic, 1.0, dst_iter_h_, p.wc,
            &(weights_iter_h(0, GRU_O, 0)), p.n_gates() * p.dhc, 1.0,
...
void gru_bwd(const prb_t &p, float *diff_src_layer_, float *diff_src_iter_,
    ...
    AOC<float> diff_weights_iter_h(
            diff_weights_iter_h_, p.sic, p.n_gates(), p.dhc);
    AOC<float> b_gates(b_gates_, p.mb, p.n_gates(), p.dhc);
    ...
    gemm("C", "N", "T", p.mb, p.sic, p.dhc, 1.0, &(b_gates(0, GRU_O, 0)),
            p.n_gates() * p.dhc, &(weights_iter_h(0, GRU_O, 0)),
    ...
    gemm("C", "T", "N", p.sic, p.dhc, p.mb, 1.0, hr_, p.wc,
            ...
            &(diff_weights_iter_h(0, GRU_O, 0)), p.n_gates() * p.dhc);
    ...
    gemm("C", "N", "T", p.mb, p.sic, (p.n_gates() - 1) * p.dhc, 1.0, b_gates_,
            p.n_gates() * p.dhc, weights_iter_h_, p.n_gates() * p.dhc, 1.0,
            diff_src_iter_, p.wc);
