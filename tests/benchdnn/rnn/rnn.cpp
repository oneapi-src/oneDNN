...
void create_dnnl_rnn_attr(const prb_t &p, dnnl_primitive_attr_t *dnnl_attr) {
    ...
    if (p.is_lstm_projection() && p.wei_proj_scales_policy != policy_t::NONE)
        DNN_SAFE_V(dnnl_primitive_attr_set_rnn_weights_projection_qparams(
                *dnnl_attr, p.wei_proj_nscales, p.wei_proj_scales_mask,
                p.wei_proj_scales));
    ...
    switch (kind) {
        ...
        case WEIGHTS_ITER:
            dnnl::impl::parallel_nd(n_chunks, [&](int idx) {
                quantize(p.wei_scales, p.wei_nscales, idx);
            });
            break;
        case WEIGHTS_PROJECTION:
            dnnl::impl::parallel_nd(n_chunks, [&](int idx) {
                quantize(p.wei_proj_scales, p.wei_proj_nscales, idx);
            });
            break;
...
int fill_memory(const prb_t &p, data_kind_t kind, dnn_mem_t &mem_dt,
    ...
    switch (kind) {
        case WEIGHTS_PROJECTION:
            dnnl::impl::parallel_nd(n_chunks, [&](int idx) {
                fill_chunk(p.wei_proj_scales, p.wei_proj_nscales, 0.0f, idx);
            });
            break;
        case WEIGHTS_LAYER:
        case WEIGHTS_ITER:
    ...
    if (p.is_int8()) {
        ...
        switch (kind) {
            ...
            case WEIGHTS_PROJECTION:
                dnnl::impl::parallel_nd(n_chunks, [&](int idx) {
                    quantize_chunk(p.wei_proj_scales, p.wei_proj_nscales, idx);
                });
                break;
...
int doit(const prb_t &p, res_t *r) {
    ...
    dnn_mem_t weights_projection_dt(weights_projection_md, engine_tgt);
    ...
    dnn_mem_t weights_projection_fp(
            weights_projection_md, fp, dnnl_ldio, engine_tgt);
    ...
    if (p.prop == dnnl_backward) {
        ...
        bwd_weights_projection_dt
                = dnn_mem_t(bwd_weights_projection_md, engine_tgt);
        ...
        diff_weights_projection_dt
                = dnn_mem_t(diff_weights_projection_md, engine_tgt);
        ...
        dnn_mem_t diff_weights_projection_fp(
                diff_weights_projection_md, fp, dnnl_ldio, engine_tgt);
