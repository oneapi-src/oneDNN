...
void check_correctness(const settings_t &s) {
    ...
    for (const auto &i_scratchpad_mode : s.scratchpad_mode) {
        ...
        const prb_t prb(s.dims, i_dir, i_dt, i_tag, attr);
        std::stringstream ss;
        ss << prb;
...
int bench(int argc, char **argv) {
    ...
    for (; argc > 0; --argc, ++argv) {
        ...
        if (!parsed_options) {
            ...
            std::vector<dims_t> multi_dims;
            parse_multi_dims(multi_dims, argv[0]);
            s.dims.push_back(multi_dims.at(0));
            s.dims.push_back(multi_dims.at(1));
            check_correctness(s);
            s.dims.clear();
        }
    }
