...
void check_correctness(const settings_t &s) {
    ...
    for_(const auto &i_dts : s.dts)
    for (const auto &i_mb : s.mb) {
        deserialized_graph dg;
        dg.load(locate_file(s.json_file));
        flex_rewrite fw(
                i_in_shapes, i_op_attrs, i_fpmath_mode, i_mb, i_dt, i_dts);
        fw.rewrite(dg);
        ...
        const auto &cpp_pstr = case_to_str(s.json_file, i_in_shapes, i_op_attrs,
                i_fpmath_mode, i_expected_n_partition, i_mb, i_dt, i_dts);
