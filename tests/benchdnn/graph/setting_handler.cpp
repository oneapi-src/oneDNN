...
bool get_deconv_cfg(const deserialized_op &base_op_ref,
        const ::deconv::dt_conf_t **cfg,
        const std::unordered_set<size_t> &rewrite_lt_ids) {
    bool ret = false;
    std::string cfg_str {"f32"};
    const auto &op_kind = base_op_ref.kind_;
    ...
    if (src_dt == wei_dt) {
        if (wei_dt == dst_dt) {
            if (src_dt == "f32" || src_dt == "f16") {
                cfg_str = src_dt;
                *cfg = ::deconv::str2cfg(cfg_str.c_str());
                ret = true;
            } else if (src_dt == "bf16") {
                cfg_str = src_dt + wei_dt + dst_dt;
                *cfg = ::deconv::str2cfg(cfg_str.c_str());
...
::deconv::settings_t get_setting(const deserialized_op &base_op_ref,
    ...
    DNN_GRAPH_CHECK_SETTINGS(deconv::get_deconv_cfg(base_op_ref,
                                     &op_setting.cfg.front(), rewrite_lt_ids),
            res);
    DNN_GRAPH_CHECK_SETTINGS(
            get_driver_stag_and_dtag(base_op_ref, op_setting.stag.front(),
