...
int doit(const ::binary::prb_t *prb, res_t *res) {
    ...
    auto src0_dt = make_dnn_mem(ins[0], tag::abx);
    auto src1_dt = make_dnn_mem(ins[1], tag::abx);
    ...
    if (bench_mode & CORR) {
        ...
        const auto binary_add_check = [prb](int64_t i, float got, float diff) {
