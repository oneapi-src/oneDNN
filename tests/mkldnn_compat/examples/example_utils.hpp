...
inline void read_from_mkldnn_memory(void *handle, mkldnn::memory &mem) {
...
#if MKLDNN_WITH_SYCL
    bool is_cpu_sycl = (MKLDNN_CPU_RUNTIME == MKLDNN_RUNTIME_SYCL
            && eng.get_kind() == mkldnn::engine::kind::cpu);
    bool is_gpu_sycl = (MKLDNN_GPU_RUNTIME == MKLDNN_RUNTIME_SYCL
            && eng.get_kind() == mkldnn::engine::kind::gpu);
    ...
    if (eng.get_kind() == mkldnn::engine::kind::gpu) {
        mkldnn::stream s(eng);
