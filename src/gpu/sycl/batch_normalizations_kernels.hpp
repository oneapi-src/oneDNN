...
struct batch_normalization_fwd_kernel_vec_t {
    batch_normalization_fwd_kernel_vec_t(
            ...
            hrt::sycl::out_memory_arg_t &ws, hrt::sycl::in_memory_arg_t &src1)
        : conf_(conf)
        , data_(data)
        , scale_(scale)
        , shift_(shift)
...
private:
    ...
    const hrt::sycl::md_t &stat_d() const { return conf_.stat_md; }
    const hrt::sycl::md_t &dst_md() const { return conf_.dst_md; }
    const float epsilon() const { return conf_.batch_norm_epsilon; }

    inline static dim_t DATA_OFF(const hrt::sycl::md_t &mdw, dim_t n, dim_t c,
            ...
            for (dim_t w = 0; w < conf_.W; ++w) {
                ...
                if (data_md().data_type() == data_type::s8) {
                    bn_res = ::dnnl::impl::sycl::qz_a1b0<float,
                            hrt::sycl::prec_traits<data_type::s8>::type>()(
...
struct batch_normalization_bwd_kernel_vec_t {
    batch_normalization_bwd_kernel_vec_t(
            ...
            hrt::sycl::in_memory_arg_t &dst, hrt::sycl::in_memory_arg_t &ws,
            hrt::sycl::in_memory_arg_t &diff_src1)
        : conf_(conf)
        , data_(data)
...
private:
    ...
    const hrt::sycl::md_t &diff_src1_md() const { return conf_.diff_src1_md; }
    const hrt::sycl::md_t &stat_d() const { return conf_.stat_md; }
    const hrt::sycl::md_t &ws_md() const { return conf_.ws_md; }
    const hrt::sycl::md_t &data_scaleshift_md() const {
        return conf_.data_scaleshift_md;
    ...
    static dim_t DATA_OFF(const hrt::sycl::md_t &mdw, dim_t n, dim_t c, dim_t d,
            dim_t h, dim_t w) {
        switch (mdw.ndims()) {
            case 1: return mdw.off(n);
            case 2: return mdw.off(n, c);
    ...
    hrt::sycl::in_memory_arg_t dst_;
    hrt::sycl::in_memory_arg_t ws_;
    hrt::sycl::in_memory_arg_t diff_src1_;
};
