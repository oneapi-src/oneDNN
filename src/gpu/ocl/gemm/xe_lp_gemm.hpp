...
struct xe_lp_gemm_t : public gpu_gemm_t {
    ...
    struct pd_t : public gpu_gemm_pd_t {
        ...
        bool set_default_formats() {
            ...
            ok = ok && is_md_gemm_compatible_plain_format(&desc_.b_desc)
                    && is_md_gemm_compatible_plain_format(&desc_.a_desc)
                    && is_md_gemm_compatible_plain_format(&desc_.c_desc, true);
    ...
    status_t init_nocopy(engine_t *engine) {
        ...
        for (bool aligned : {false, true}) {
            ...
            CHECK(create_kernel(compute_engine,
                    &compute_x8x8s32_kernel_[aligned], kernel_name,
                    kernel_ctx));
        ...
        CHECK(create_kernel(compute_engine, &scale_x8x8s32_kernel_, kernel_name,
                kernel_ctx));
