...
#ifndef GPU_OCL_XE_HPC_BF16_CONVOLUTION_HPP
#define GPU_OCL_XE_HPC_BF16_CONVOLUTION_HPP
...
struct xe_hpc_bf16_convolution_bwd_weights_t : public gpu_primitive_t {
    struct pd_t : public gpu_convolution_bwd_weights_pd_t {
        ...
        DECLARE_COMMON_PD_T(
                "ocl:xe_hpc", xe_hpc_bf16_convolution_bwd_weights_t);
    ...
    status_t init(engine_t *engine) override {
        ...
        if (pd()->conf.use_split_barrier)
            kernel_names.push_back("xe_hpc_conv_bwd_wei_bf16_split_bar");
        else
            kernel_names.push_back("xe_hpc_conv_bwd_wei_bf16");
        kernel_names.push_back("xe_hpc_wei_f32_zero_init");
        if (pd()->conf.weights_data_type == data_type::bf16)
            kernel_names.push_back("xe_hpc_wei_convert_f32_to_bf16");
    ...
    xe_hpc_bf16_convolution_bwd_weights_t(const pd_t *apd)
        : gpu_primitive_t(apd) {}
