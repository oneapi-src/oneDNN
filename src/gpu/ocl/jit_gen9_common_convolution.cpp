...
status_t jit_gen9_convolution_fwd_init_conf(
    ...
    set_default_conf(jcp, cd, *pd->src_md(), *pd->weights_md(), *pd->dst_md(),
    ...
    const bool is_32oc
            = true && IMPLICATION(src_mdw.data_type() == f16, jcp.oc % 32 == 0);
    ...
    switch (jcp.ver) {
        case ver_16mb16c:
            ...
            if (src_mdw.data_type() == f16 && jcp.mb % 32 != 0) {
                ...
                jcp.gws_d[2] = (src_mdw.data_type() == f16 && !jcp.is_depthwise)
    ...
    switch (jcp.ver) {
            ...
            if (utils::one_of(src_mdw.data_type(), f16)) {
                if (jcp.mb % 32 == 0) {
                    ...
                    wei_tag = jcp.with_groups
                            ...
                            : utils::pick(jcp.ndims - 3, IOw16i16o, IOhw16i16o,
                                    IOdhw16i16o);
                }
            } else {
                src_tag = utils::pick(
        ...
        case ver_8ow16c:
            ...
            wei_tag = jcp.is_depthwise
                    ...
                    : (jcp.with_groups ? utils::pick(jcp.ndims - 3, gOIw16i16o,
...
status_t jit_gen9_convolution_fwd_init_const_def(
    ...
    const int use_fast_path = 1 && jcp.scale_idx_mult == 0 && jcp.ngroups == 1
            && !jcp.with_bias;
...
status_t jit_gen9_convolution_bwd_data_init_conf(
    ...
    switch (jcp.ver) {
        case ver_16mb16c:
            ...
            if (jcp.is_depthwise) {
                ...
                jcp.gws_d[2] = jcp.mb / 16 * (jcp.ic / jcp.icb) * jcp.ngroups;
            }
            break;
        case ver_8ow16c:
            jcp.mb_block = 1;
    ...
    switch (jcp.ver) {
        case ver_16mb16c:
            ...
            wei_tag = jcp.is_depthwise
                    ? utils::pick(jcp.ndims - 3, Goiw16g, Goihw16g, Goidhw16g)
                    : (jcp.with_groups ? utils::pick(jcp.ndims - 3, gOIw16o16i,
...
static void bwd_w_compute_block_sizes(
    ...
    auto get_src_dst_size = [&]() {
        ...
        size_t src_size = sizeof(float) * jcp.mb_block
                ...
                * ispb;
        size_t dst_size
                = sizeof(float) * jcp.mb_block * (oc_nb * jcp.oc_block) * ospb;
    ...
    if (jcp.ver == ver_8ow16c || jcp.ver == ver_1stconv) jcp.owb = jcp.ow_block;
    ...
    for (int i = 0; i < 128; i++) {
        ...
        if (jcp.ver == ver_8ow16c || jcp.ver == ver_1stconv) {
            ...
            owb_next = jcp.ow_block
                    * next_candidate(
                            ow_padded / jcp.ow_block, jcp.owb / jcp.ow_block);
        } else {
    ...
    jcp.osp_chunk = utils::div_up(jcp.od, jcp.odb)
            * utils::div_up(jcp.oh, jcp.ohb) * utils::div_up(jcp.ow, jcp.owb);
...
status_t jit_gen9_convolution_bwd_weights_init_conf(
    ...
    switch (jcp.ver) {
        ...
        case ver_8ow16c:
            ...
            jcp.ow_block = 8;
            bwd_w_compute_block_sizes(jcp, pd);
            break;
        case ver_16mb16c:
            jcp.mb_block = 16;
            jcp.oc_block = 16;
    ...
    switch (jcp.ver) {
        ...
        case ver_16mb16c:
            ...
            wei_tag = jcp.is_depthwise
                    ? utils::pick(jcp.ndims - 3, Goiw16g, Goihw16g, Goidhw16g)
                    : (jcp.with_groups ? utils::pick(jcp.ndims - 3, gIOw16i16o,
                               gIOhw16i16o, gIOdhw16i16o)
                                       : utils::pick(jcp.ndims - 3, IOw16i16o,
                                               IOhw16i16o, IOdhw16i16o));
            break;
...
status_t jit_gen9_convolution_bwd_weights_init_const_def(
    ...
    kernel_ctx.define_int("MB_CHUNK_SIZE", utils::div_up(jcp.mb, jcp.mb_chunk));
    kernel_ctx.define_int("OW_BLOCK", jcp.ow_block);
