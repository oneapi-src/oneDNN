...
__kernel void gen9_fused_reduce_init(
    ...
    variance[c] = 0.0f;
#else
    diff_scaleshift[c] = 0.0f;
#if DIFF_SCALESHIFT == 1
    diff_scaleshift[IC + c] = 0.0f;
#elif DIFF_SCALE == 1 || DIFF_SHIFT == 1
    diff_shift[c] = 0.0f;
#else
    diff_scaleshift[IC + IC * REDUCE_STAT_NBLOCKS + c] = 0.0f;
#endif
#endif
    return;
}
...
void gen9_calc_fused_reduction(volatile __global atomic_float *diff_scaleshift,
...
#endif
            {
                atomic_add_global(&diff_scaleshift[offset], d_gamma);
#if DIFF_SCALESHIFT == 1
                atomic_add_global(&diff_scaleshift[IC + offset], d_beta);
#elif DIFF_SCALE == 1 || DIFF_SHIFT == 1
                atomic_add_global(&diff_shift[offset], d_beta);
#else
                atomic_add_global(&diff_scaleshift[IC + IC * REDUCE_STAT_NBLOCKS
                                          + offset],
                        d_beta);
#endif
            }
...
__kernel void gen9_calculate_stats(__global DATA_T *src, __global float *mean,
        ...
        volatile __global atomic_float *diff_scaleshift,
    ...
    gen9_calc_fused_reduction(diff_scaleshift, diff_shift, ic_block_offset,
...
__kernel void gen9_calculate_stats(__global DATA_T *src, __global float *mean,
    ...
    __local float local_gamma[2 * CALC_SLM_SIZE];
    __local float *local_beta = local_gamma + CALC_SLM_SIZE;
    gen9_calc_fused_reduction(diff_scaleshift, diff_shift, c, &diff_gamma,
...
__kernel void gen9_fused_reduce_final(
    ...
    const int c = GWS_GET_IC_AUX();
    diff_scaleshift[c] *= 1.0f / sqrt(variance[c] + eps);
    return;
}
