...
cell_execution_sig((_ref_rnn_common_t<aprop>::cell_execution)) {
    ...
    dim_t cell_scratch_offset, cell_wei_iter_offset;
    ...
    if (aprop == prop_kind::forward) {
        ...
        CHECK((this->*elemwise_common)(ctx, dir, lay, iter, rnn.dhc, rnn.mb,
                ...
                scratch_diff_gates, scratch_diff_states, scales, bias,
                tm_scales, diff_bias));

        CHECK(gemm_primitive(engine, ctx, wei_iter, cell_wei_iter_offset,
                *scratch_diff_gates, cell_scratch_diff_off, scratch_diff_states,
        ...
        if (!rnn.merge_gemm_iter) {
            CHECK(gemm_primitive(engine, ctx, *scratch_diff_gates,
                    cell_scratch_diff_off, *cell_iter, 0, diff_weights_iter,
