...
struct init_kernel_t {
    ...
    init_kernel_t(xpu::sycl::out_memory_arg_t &out_arg, dnnl_alg_kind_t alg)
...
struct atomic_finalize_kernel_t {
    ...
    atomic_finalize_kernel_t(::sycl::handler &cgh, const exec_ctx_t &ctx,
            ...
            dnnl_alg_kind_t alg, float p, float eps, sycl_post_ops_t &post_ops,
    ...
    void operator()(::sycl::item<1> item) const {
        ...
        if (alg_ == dnnl_reduction_norm_lp_max) {
            ...
            val = val + eps_;
        } else if (alg_ == dnnl_reduction_mean) {
...
struct Reducer {
    ...
    Reducer(dnnl_alg_kind_t alg, float p, float eps, bool needs_prepare,
    ...
    inline bool needs_finalize() const {
        return (alg_ == dnnl_reduction_mean
                || alg_ == dnnl_reduction_norm_lp_max
    ...
    inline void finalize(float &val, int size) {
        if (alg_ == dnnl_reduction_mean) {
            val /= size;
        } else if (alg_ == dnnl_reduction_norm_lp_max && iter_needs_finalize_) {
            val = ::sycl::rootn(::sycl::max(val, eps_), p_);
        } else if (alg_ == dnnl_reduction_norm_lp_sum && iter_needs_finalize_) {
            val = ::sycl::rootn(val + eps_, p_);
        } else if (alg_ == dnnl_reduction_norm_lp_power_p_max
...
struct LocalMemTile {
    ...
    static constexpr int RowDim = Dim == 3 ? 1 : 0;
    static constexpr int ColDim = Dim == 3 ? 2 : 1;
    int row_tile_;
    int col_tile_;
    int row_lim_;
    int col_lim_;
...
public:
    void transpose(dnnl_data_type_t in_dt, void *input, dnnl_data_type_t out_dt,
            void *output) {
        transpose_impl_<false>(in_dt, input, out_dt, output);
    }

    void transpose(dnnl_data_type_t dt, void *input) {
        transpose_impl_<true>(dt, input, dt, input);
    }

    void sg_reduce_impl(dnnl_data_type_t in_dt, void *input,
            dnnl_data_type_t out_dt, void *output, bool is_transposed,
            Reducer &reducer) {
    ...
    void sg_reduce(dnnl_data_type_t in_dt, void *input, dnnl_data_type_t out_dt,
    ...
    void sg_reduce(dnnl_data_type_t dt, void *input, int num_sg_reductions,
    ...
    void wi_reduce_impl(dnnl_data_type_t in_dt, void *input,
            dnnl_data_type_t out_dt, void *output, bool is_transposed,
            Reducer &reducer) {
    ...
    void wi_reduce(dnnl_data_type_t dt, void *input, bool is_transposed,
            Reducer &reducer) {
    ...
    template <::sycl::memory_order Order, ::sycl::memory_scope Scope,
            ::sycl::access::address_space
                    Space /* , typename InMem, typename OutMem */>
    void atomic_reduce(dnnl_data_type_t in_dt, void *input,
            dnnl_data_type_t out_dt, void *output,
            const xpu::sycl::md_t &out_md, Index finalize_param,
...
struct reduction_kernel_fwd_t {
    ...
    void operator()(::sycl::nd_item<3> nd_item) const {
        ...
        if (conf_.alg == dnnl_reduction_mean && conf_.num_sg_reductions == 0) {
