...
// function to get which format tag to reorder to from number of dimensions of src
format_tag_t get_cfirst_format_from_ndims(int ndims) {
    // If the input is just 2 dimensional, handle it in gemm(NT / TN) rather than a reorder
    assert(ndims > 2);
    switch (ndims) {
...
status_t get_primitive_descriptor(op_desc_t *op_desc,
    ...
    return assigned ? status::success : status::unimplemented;
}
...
status_t ref_inner_product_fwd_t::pd_t::init(impl::engine_t *engine) {
    ...
    if (src_needs_reorder) {
        ...
        if (!src_needs_reorder
                && detail::get_dim_order(
                           src_wrapper.ndims(), src_wrapper.strides())
                        != detail::get_dim_order(
                                wei_wrapper.ndims(), wei_wrapper.strides())
                && wei_wrapper.ndims() > 2) {
            ...
            if (src_strides[ndims - 1] == 1) {
                wei_reordered_tag = detail::get_cfirst_format_from_ndims(
                        wei_wrapper.ndims());
            } else if (src_strides[1] == 1) {
                switch (wei_wrapper.ndims()) {
                    case 3: wei_reordered_tag = format_tag::acb; break;
                    case 4: wei_reordered_tag = format_tag::acdb; break;
...
status_t ref_inner_product_bwd_data_t::pd_t::init(impl::engine_t *engine) {
    ...
    if (not detail::strides_in_desc_order(
        ...
        CHECK(memory_desc_init_by_tag(dst_reordered_desc, dst_wrapper.ndims(),
                ...
                detail::get_cfirst_format_from_ndims(dst_wrapper.ndims())));
