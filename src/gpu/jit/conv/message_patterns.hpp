    ...
    conv_stride_layout_t(const conv_problem_t &prb, input_tensor_t type)
        ...
        switch (type) {
            ...
            case input_tensor_t::dst: {
                ...
                int i = 0;
                s = write_strides(s, conv_dims::mb, i++, prb.mb);
                if (is_src)
                    s = write_strides(s, conv_dims::ic, i++, prb.ic);
                else
                    s = write_strides(s, conv_dims::oc, i++, prb.oc);
                ...
                if (mdw.ndims() >= 5) {
                    bool is_padded = is_src
                            && (prb.pd
                                    || prb.id < prb.od * prb.sd
                                                    + (prb.kd - 1)
                                                            * (prb.dd + 1));
                    auto x_dim = !prb.is_bwd_d ? conv_dims::od : conv_dims::id;
                    ...
                    s = write_strides(
                            s, conv_dims::kd, i++, kx, kxas, is_padded);
                }
                if (mdw.ndims() >= 4) {
                    bool is_padded = is_src
                            && (prb.ph
                                    || prb.ih < prb.oh * prb.sh
                                                    + (prb.kh - 1)
                                                            * (prb.dh + 1));
                    auto x_dim = !prb.is_bwd_d ? conv_dims::oh : conv_dims::ih;
                    ...
                    s = write_strides(s, x_dim, i, x, xas, is_padded);
                    s = write_strides(
                            s, conv_dims::kh, i++, kx, kxas, is_padded);
                }
                bool is_padded = is_src
                        && (prb.pw
                                || prb.iw < prb.ow * prb.sw
                                                + (prb.kw - 1) * (prb.dw + 1));
                auto x_dim = !prb.is_bwd_d ? conv_dims::ow : conv_dims::iw;
                auto x = !prb.is_bwd_d ? prb.ow : prb.iw;
                auto xas = !prb.is_bwd_d ? prb.sw : prb.sw == 1;
                auto kx = prb.kw;
                auto kxas = !prb.is_bwd_w ? prb.dw + 1 : prb.dw == 0;
                s = write_strides(s, x_dim, i, x, xas, is_padded);
                s = write_strides(s, conv_dims::kw, i++, kx, kxas, is_padded);
                break;
            }
            case input_tensor_t::wei: {
                int i = 0;
                if (prb.with_groups)
                    s = write_strides(s, conv_dims::g, i++, prb.g);
                s = write_strides(s, conv_dims::oc, i++, prb.oc);
                s = write_strides(s, conv_dims::ic, i++, prb.ic);
                if (mdw.ndims() >= 5 + prb.with_groups) {
                    s = write_strides(s, conv_dims::kd, i++, prb.kd);
                }
                if (mdw.ndims() >= 4 + prb.with_groups) {
                    s = write_strides(s, conv_dims::kh, i++, prb.kh);
                }
                s = write_strides(s, conv_dims::kw, i++, prb.kw);
                break;
            }
