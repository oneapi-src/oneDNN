...
#if NGEN_HAS_INCLUDE(<filesystem>)
#include <filesystem>
#if defined(__cpp_lib_filesystem) && __cpp_lib_filesystem >= 201703L
#define NGEN_HAS_FILESYSTEM
#endif
#endif

#if NGEN_ENABLE_SOURCE_LOCATION && NGEN_HAS_INCLUDE(<source_location>) && NGEN_HAS_INCLUDE(<filesystem>)
#include <source_location>
#if defined(__cpp_lib_source_location) && __cpp_lib_source_location >= 201907L
#define NGEN_HAS_SOURCE_LOCATION
#endif
#endif
...
#ifdef NGEN_HAS_SOURCE_LOCATION
...
#ifndef NGEN_HAS_SOURCE_LOCATION
        // Unsupported
...
#pragma pack(pop)
    ...
    std::pair<std::vector<char>, uint64_t> encodeLineStatements(const DebugLineHeaderBase & base) const {
        struct {
            ...
            void encode(std::vector<char> &out, const Line &l) {
                ...
                if(l.line != line) {
                    printf("Advancing Line by %ld\n", static_cast<int64_t>(l.line) - line);
    ...
    std::pair<std::vector<char>, uint64_t> createDebugLine() const {
        ...
        uint32_t dirEntriesBytes = dirEntries.size() * sizeof(dirEntries[0]);
        DebugLineHeaderBase::FileEntriesHeader fileHeader;
        std::vector<char> filenamesCount = toULEB128(fileEntries.size());
        uint32_t fileEntriesBytes = fileEntries.size() * sizeof(fileEntries[0]);
        auto encode = encodeLineStatements(base);
        std::vector<char> lineStatements = encode.first;
        uint64_t relocation_offset = encode.second;

        uint32_t headerEntriesSize = sizeof(dirHeader) + directoriesCount.size() + dirEntriesBytes +
            sizeof(fileHeader) + filenamesCount.size() + fileEntriesBytes;
        uint32_t lineTableSize = lineStatements.size();
