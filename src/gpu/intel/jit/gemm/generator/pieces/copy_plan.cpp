...
void CopyPlan::planEmulatedHFToE3M0(CopyInstruction &i)
{
    ...
    // and (nz)f0     null:uw x:uw    0xFFFF   /* preserve zeros */
    // mov (nz)f0     t0:uw   x:uw
    // (f0) add       t0:uw   t0:uw   -0x200   /* subtract 1/2 ULP */
    // and (ne)f1     null:uw t0:uw   0x7FF    /* RTNE logic */
    // shr            t0:uw   t0:uw   10       /* extract exponent */
    // and            t0:uw   t0:uw   0x1F     /* remove sign bit */
    // (f1) add       t0:uw   t0:uw   1        /* round up */
    // and (ne)f1     null:uw x:uw    0x8000   /* check sign bit */
    // (f0) add (sat) t0:uw   t0:uw   -12      /* adjust bias, clamp exponent low */
    // sel (le)       t0:uw   t0:uw   7        /* clamp exponent high */
    // (f1) or        t0:uw   t0:uw   8        /* insert sign bit */
    ...
    ie[idx]->op = Opcode::and_;
    ie[idx]->flag = selFlag;
    ie[idx]->cmod = ConditionModifier::ne;
    ie[idx]->dst = CopyOperand();
    ie[idx]->dst.type = DataType::uw;
    ie[idx]->src0 = bits;
    ie[idx]->src0.type = DataType::uw;
    ie[idx]->src1 = Immediate::uw(0x7FF); 
    ++idx;
