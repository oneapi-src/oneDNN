...
std::vector<tensor_t> tiles(const hw_t &hw, layout_t a, layout_t b) {
    ...
    auto cmp = [&](const tensor_t &l, const tensor_t &r) {
        ...
        auto r_a_msg = estimate_message_info(hw, a, l);
        auto r_b_msg = estimate_message_info(hw, a, l);
    ...
    for (auto tile : tiles) {
        int elem_size = unit_size;
        auto a_info = estimate_message_info(hw, a, tile);
        auto b_info = estimate_message_info(hw, b, tile);
        if (utils::one_of(message_kind_t::scattered, a_info.kind, b_info.kind))
            elem_size = std::max(4, elem_size);
        dim_t max_elems = max_size / elem_size;
        if (tile.elems() > max_units) break;
        if (tile.elems() > max_elems) continue;
