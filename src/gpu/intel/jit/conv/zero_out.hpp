...
public:
    ...
    zero_out_kernel_desc_t(
            int regs, int simd, bool dpas, const std::string &arg_name)
        : regs_(regs), simd_(simd), dpas_(dpas), arg_name_(arg_name) {}
...
public:
    ...
    zero_out_kernel_t(const exec_config_t &exec_cfg,
        ...
        : zero_out_kernel_t<hw>(zero_out_kernel_desc_t(exec_cfg.regs(),
                                        exec_cfg.simd(), require_dpas, "ptr"),
                engine) {}
