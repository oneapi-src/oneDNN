...
#define CASE(hw) \
    case ngen::HW::hw: { \
        ...
        auto s = kernel.kernel_iface().serialize(); \
        metadata = s.get_data(); \
        return kernel.getBinary(engine->context(), engine->device()); \
    }
            // TODO: introduce getter for hw.
            switch (kernel_desc_.exec_cfg(engine).hw().to_ngen()) {
...
public:
    ...
    kernel_iface_t &kernel_iface() { return kernel_iface_; }
