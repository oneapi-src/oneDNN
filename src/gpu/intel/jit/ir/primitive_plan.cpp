...
kernel_info_t primitive_init_plan_t::create_kernel_info(
        const xpu::binary_t &metadata_binary) const {
    auto iface = kernel_iface_t::deserialize(
            serialized_t::from_data(metadata_binary));
    kernel_info_t info;
...
status_t primitive_init_plan_t::add_kernel(primitive_exec_plan_t &exec_plan,
    ...
    auto kernel_info = create_kernel_info(kernel.get_metadata());
...
status_t primitive_init_plan_t::add_zero_out_kernel(
        ...
        gpu_primitive_t *primitive, impl::engine_t *engine) const {
    auto desc = std::make_shared<zero_out_kernel_desc_t>(
            regs_, simd_, dpas_, buf.name);
