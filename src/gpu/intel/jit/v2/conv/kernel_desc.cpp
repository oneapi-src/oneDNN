...
status_t kernel_desc_t::set_attr(const convolution_pd_t *pd,
    ...
    CHECK(gpu_scales_t::make(scales, attr->scales_));
    if (!pd->with_groups()) {
        // Extend weights scales mask to include groups as the kernel can be
        // reused for group convolution as well.
        for (size_t i = 0; i < scales.len(); i++) {
            if (scales[i].arg == DNNL_ARG_WEIGHTS) {
                auto &e = scales[i].quant_entry;
                if (e.get_mask() & ((1 << 0) | (1 << 1))) {
                    e.set((e.get_mask() << 1) | 1, e.get_data_type());
                }
                break;
            }
        }
...
#define PACK(member) decltype(kernel_desc_t::member), &kernel_desc_t::member
    ...
    scales_entry._default = [](const kernel_desc_t &) {
        return serialize_to_hex(gpu_scales_t());
    };
...
std::string arg_helper_t::scales_name(size_t idx) const {
    gpu_assert(idx < desc_.scales.len());
    auto &e = desc_.scales[idx];
    switch (e.arg) {
...
int arg_helper_t::scales_key(size_t idx) const {
    gpu_assert(idx < desc_.scales.len());
    auto &e = desc_.scales[idx];
    return DNNL_ARG_ATTR_SCALES | e.arg;
}
...
tensor_config_t get_tensor_config(
    ...
    for (size_t i = 0; i < desc.scales.len(); i++) {
        auto name = h.scales_name(i);
        if (name.empty()) continue;
        int key = h.scales_key(i);
