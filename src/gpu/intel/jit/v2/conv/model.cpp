...
#if 0
    virtual float pad_eff() const { return 1.0f; }
#endif
...
public:
    data_parallel_sample_t() = default;
    data_parallel_sample_t(const problem_t &prb, const kernel_desc_t &desc,
        ...
        //pad_eff_ = h.pad_eff;
        //ops_ = 2.0f * h.b * h.m * h.n * h.k;
        //eff_ = ops_ / nsec_ / hw_cfg_.max_gops_per_sec();
    }
    ...
    //float pad_eff() const override { return pad_eff_; }
...
public:
    ...
    //float pad_eff() const { return impl_->pad_eff(); }
...
size_t model_t::coef_size(model_kind_t kind) {
    switch (kind) {
...
void model_set_t::parse(std::istream &in) {
    ...
    while (!d.empty()) {
        ...
        size_t coef_size = model_t::coef_size(kind);
        vec1d coef(coef_size);
