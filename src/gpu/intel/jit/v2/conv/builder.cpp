...
public:
    buffer_info_t(buffer_manager_t &buf_mgr, const kernel_desc_t &desc,
        ...
        for (size_t i = 0; i < desc.scales.len(); i++) {
            auto name = arg_helper.scales_name(i);
            auto &e = entries_[name];
            e.mem_buf = var_mgr.get_arg(name);
        }
...
private:
    ...
    expr_t build_post_ops(const layout_t &layout,
            ...
            layout_t &out_layout) {
        if (desc_.post_ops.len() == 0 && desc_.scales.len() == 0
                && !desc_.with_bias_fwd()) {
            out_layout = layout;
            return _buf;
        }
        ...
        auto build_scale = [&](int i) {
            auto rhs_buf_name = arg_helper.scales_name(i);
            auto arg = desc_.scales[i].arg;
            auto mask = desc_.scales[i].quant_entry.get_mask();
            auto data_type = desc_.scales[i].quant_entry.get_data_type();
        ...
        for (size_t i = 0; i < desc_.scales.len(); i++) {
            if (desc_.scales[i].arg != DNNL_ARG_DST) build_scale(i);
        }
        // Apply bias.
