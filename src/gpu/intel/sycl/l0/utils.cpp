...
status_t get_device_ip(ze_device_handle_t device, uint32_t &ip_version) {
    ...
    func_zeDeviceGetProperties(device, &deviceProps);
    ip_version = devicePropsIP.ipVersion;
    return status::success;
}

status_t get_lz_device_enabled_systolic_intel(
        ze_device_handle_t device, bool &mayiusesystolic) {
    ...
    func_zeDeviceGetModuleProperties(device, &deviceModProps);
    mayiusesystolic
            = deviceModPropsExt.flags & ZE_INTEL_DEVICE_MODULE_EXP_FLAG_DPAS;
    return status::success;
}

status_t get_lz_device_enabled_native_float_atomics(
...
status_t get_lz_device_eu_count(ze_device_handle_t device, int &eucount) {
    ze_device_properties_t deviceProps = {ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES};
...
void init_gpu_hw_info(impl::engine_t *engine, ze_device_handle_t device,
    ...
    status_t ret
            = get_lz_device_enabled_systolic_intel(device, mayiuse_systolic);
    // TODO: xelpg has no f64 support. check that the query properly handle that
    ret = get_lz_device_enabled_native_float_atomics(device, native_extensions);
    MAYBE_UNUSED(ret);
