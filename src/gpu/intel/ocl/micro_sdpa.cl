...
#pragma unroll
    ...
    const int page_size = 4, start_row = 0;
    for (int block_index_no = block_start_index; block_index_no < block_stop_index; block_index_no++) {
        const int block_no = block_indices[block_index_no];
        const int k0 = PAGE_SIZE * (block_index_no - block_start_index);
        const int key_offset = KEY_OFF(1, 0, 0, 0) * block_no;
        const int value_offset = VAL_OFF(1, 0, 0, 0) * block_no;
        bool first = (block_index_no == block_start_index);
        bool last = (block_index_no == block_stop_index - 1);
