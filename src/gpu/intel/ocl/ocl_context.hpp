...
struct ocl_event_t final : compute::event_t {
    ocl_event_t() = default;
    ocl_event_t(const std::vector<hrt::ocl::wrapper_t<cl_event>> &events)
        : events(events) {}
    ocl_event_t(std::vector<hrt::ocl::wrapper_t<cl_event>> &&events)
        : events(std::move(events)) {}
    ocl_event_t(hrt::ocl::wrapper_t<cl_event> &&event) {
        events.emplace_back(std::move(event));
    }

    const hrt::ocl::wrapper_t<cl_event> &operator[](size_t i) const {
        return events[i];
    }
    hrt::ocl::wrapper_t<cl_event> &operator[](size_t i) { return events[i]; }
    ...
    std::vector<hrt::ocl::wrapper_t<cl_event>> events;
};

struct ocl_context_t final : public gpu::intel::compute::context_t {
    ocl_context_t() = default;
    ocl_context_t(const std::vector<hrt::ocl::wrapper_t<cl_event>> &&events)
    ...
    void set_deps(std::vector<hrt::ocl::wrapper_t<cl_event>> &&event) {
