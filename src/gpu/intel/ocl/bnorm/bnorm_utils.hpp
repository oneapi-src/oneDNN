...
#define DPRINT(fmt, ...) \
    do { \
        if (get_debug_verbose_dev_mode(verbose_t::debug_debug)) { \
...
#define DPRINT_PARAMS(fmt, ...) \
    do { \
        if (get_debug_verbose_dev_mode(verbose_t::debug_info)) { \
            printf(fmt, __VA_ARGS__); \
            fflush(0); \
        } \
    } while (0)
...
#define DPRINT_MODEL(fmt, ...) \
    do { \
        if (get_debug_verbose_dev_mode(verbose_t::debug_trace)) { \
            printf(fmt, __VA_ARGS__); \
            fflush(0); \
        } \
    } while (0)
