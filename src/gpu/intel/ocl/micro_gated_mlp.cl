...
__kernel void micro_gated_mlp(const __global SRC_DATA_T *src,
        const __global WTS_GATE_DATA_T * W_gate, const __global WTS_UP_DATA_T * W_up,
...
#define ugemm_slm_size ugemm_wgu_slm_size
    ...
    local uint *ugemm_u_slm = (local uint *)&slm[WGU_slm_size + ugemm_wgu_slm_size];
    ...
    for (int k0 = 0; k0 < IC; k0 += ugemm_wgu_wg_tile_m) {
...
#else
        tile_load_packed_vec2(&src_tile, src, IC, MB, lds, k0, wg_i0 + wgu0_copy);
#endif
        ...
        tile_store_t_sys_src1(
                src_tile, (local uint *)&wg_slm[0], ugemm_wgu_wg_tile_m/2, wgu0_copy, 0);
...
#else
    tile_store(S_tile_half, dst, MB, OC, lda, wg_i0 + sg_j0_wgu, wg_j0 + sg_i0_wgu);
#endif
