...
using executable_creator_func = std::function<std::shared_ptr<op_executable_t>(
        std::shared_ptr<impl::op_t> &, const dnnl::engine &,
        fusion_info_mgr_t &, pd_cache_t &)>;
...
\
    ...
    static desc_t create_desc(std::shared_ptr<impl::op_t> &op, \
...
struct bn_folding_t : public op_executable_t {
    ...
    bn_folding_t(std::shared_ptr<impl::op_t> &op, const dnnl::engine &p_engine,
            fusion_info_mgr_t &mgr, pd_cache_t &pd_cache) {
