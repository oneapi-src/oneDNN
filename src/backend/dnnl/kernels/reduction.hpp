...
#ifndef BACKEND_DNNL_KERNELS_REDUCTION_HPP
#define BACKEND_DNNL_KERNELS_REDUCTION_HPP
...
public:
    ...
    impl::status_t prepare_inplace_pairs_impl(const impl::engine_t *g_engine,
        ...
        op_t *reduction_op = nullptr;
        for (auto &op : subgraph_->get_ops()) {
            if (op->get_kind() == op_kind::dnnl_reduction) {
                reduction_op = op.get();
                break;
            }
        }

        bool with_sum = reduction_op->has_attr("with_sum")
                ? reduction_op->get_attr<bool>("with_sum")
                : false;
        ...
        if (with_sum) {
            ...
            if (reduction_op->has_attr("primitive_attr_key")) {
                int64_t key
                        = reduction_op->get_attr<int64_t>("primitive_attr_key");
            ...
            auto val = reduction_op->get_input_value(
