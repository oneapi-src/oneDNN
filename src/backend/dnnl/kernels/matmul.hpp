...
public:
    ...
    impl::status_t prepare_inplace_pairs_impl(const impl::engine_t *g_engine,
        ...
        if (with_sum) {
            ...
            if (matmul_op->has_attr("primitive_attr_key")) {
                int64_t key
                        = matmul_op->get_attr<int64_t>("primitive_attr_key");
            ...
            auto val = matmul_op->get_input_value(
                    matmul_op->num_inputs() - post_sum_index);
            if (val->has_producer()
