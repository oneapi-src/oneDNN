...
#ifndef BACKEND_DNNL_LAYOUT_PROPAGATOR_HPP
#define BACKEND_DNNL_LAYOUT_PROPAGATOR_HPP
...
using layout_propagator_func = std::function<impl::status_t(
        std::shared_ptr<impl::op_t> &, const dnnl::engine &,
        fusion_info_mgr_t &, pd_cache_t &, subgraph_rewriter_t &)>;

impl::status_t insert_reorder_before(std::shared_ptr<impl::op_t> &, size_t,
        const dnnl::memory::desc &, const dnnl::engine &, fusion_info_mgr_t &,
        pd_cache_t &, subgraph_rewriter_t &);

impl::status_t insert_reorder_after(std::shared_ptr<impl::op_t> &, size_t,
        const dnnl::memory::desc &, const dnnl::engine &, fusion_info_mgr_t &,
        pd_cache_t &, subgraph_rewriter_t &);

#define DECLARE_LAYOUT_PROPAGATOR(op_name) \
    impl::status_t layout_propagator_for_##op_name( \
            std::shared_ptr<impl::op_t> &, const dnnl::engine &, \
            fusion_info_mgr_t &, pd_cache_t &, subgraph_rewriter_t &);
