 ...
 *******************************************************************************/
#ifndef BACKEND_DNNL_PASSES_LOWER_HPP
#define BACKEND_DNNL_PASSES_LOWER_HPP
...
using handler_func = std::function<impl::status_t(
        const std::shared_ptr<impl::op_t> &, subgraph_rewriter_t &)>;
...
impl::status_t common_handler(
        const std::shared_ptr<impl::op_t> &op, subgraph_rewriter_t &rewriter) {
    auto new_op = std::make_shared<impl::op_t>(
            static_cast<impl::op_kind_t>(op_kind));
