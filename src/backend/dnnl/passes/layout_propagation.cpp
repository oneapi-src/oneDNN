...
static void layout_propagation_for_eltwise_bwd(op_ptr &op,
    ...
    auto opt_desc = (op->has_attr("use_dst") && op->get_attr<bool>("use_dst"))
            ? pd.dst_desc()
            : pd.src_desc();
    insert_reorder_before(op, 0, opt_desc, reorder_ops);
...
static void layout_propagation_for_prelu_bwd(op_ptr &op,
    ...
    insert_reorder_after(op, 1, pd.diff_weights_desc(), reorder_ops);
...
static void layout_propagation_for_from_group(
        op_ptr &op, std::vector<op_ptr> &reorder_ops) {
    ...
    if (ltw(dst_lt).is_any()) {
        ...
        insert_reorder_after(op, 0, infered_dst_md, reorder_ops);
    }
}
    ...
    for (const auto &out_op : sg->get_output_ops()) {
        ...
        for (size_t i = 0; i < out_vals.size(); ++i) {
            ...
            if (lt.id != std::numeric_limits<size_t>::max()
                ...
                insert_reorder_after(out_op_ptr, i, ori_mem_desc, reorder_ops);
            }
        }
    }
