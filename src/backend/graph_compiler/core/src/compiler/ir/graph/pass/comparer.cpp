...
bool compare_graph(sc_op_ptr &first_diff_lhs, sc_op_ptr &first_diff_rhs,
    ...
    op_visitor_t visitor {/*selector with early stopping*/
            ...
            op_visitor_t::create_DAG_updater(lhs.ops_.size())};
...
#define DO_RETURN \
    ...
    visitor.visit_graph(lhs, [&](const sc_op_ptr &lnode) {
        sc_op_ptr rnode;
        ...
        // check the mapping of input tensors. because the input nodes are
        // already visited (DAG_updater), we just need to check if the LHS
        // tensor is correctly mapped to RHS tensor
        ...
        for (size_t i = 0; i < lhs_inputs.size(); i++) {
            ...
            if (itr == tsr_map.end() || itr->second != rhs_tsr) { DO_RETURN; }
        }

        // the output tensors are owned by the current node, so they should have
        // not been visited. For each output tensor, we need to:
        // 1. check if the logical tensor contents are exactly the same (e.g.
        // shape, dtype, etc.)
        // 2. set the lhs_tsr => rhs_tsr mapping (the mapping will be checked
        // when the tsr is an input of other ops)
        // 3. check if the sizes of uses_ matches for LHS and RHS
        // 4. for each consumer Op of the tensor (which we call "use"), set the
        // op mapping from LHS to RHS
        ...
        for (size_t i = 0; i < lhs_outputs.size(); i++) {
            ...
            // 1. contents
            if (!(lhs_tsr->details_ == rhs_tsr->details_)) { DO_RETURN; }
            // 2. tsr mapping
            auto itr = tsr_map.find(lhs_tsr);
            ...
            for (size_t u = 0; u < lhs_uses.size(); u++) {
                ...
                int &mapped_id = op_id_map.at(lhs_use.second->logical_op_id_);
                // if there is already a mapping in the op map, and the mapped
                // RHS does not match rhs_use's Op id, return false
                if (mapped_id != -1
                        && mapped_id != rhs_use.second->logical_op_id_) {
                    DO_RETURN;
                }
                // set the mapping. When visiting the node, we need this mapping
                // to find the RHS
                mapped_id = rhs_use.second->logical_op_id_;
            }
        }
