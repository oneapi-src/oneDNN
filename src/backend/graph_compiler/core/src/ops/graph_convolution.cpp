...
managed_conv_bwd_data_op_t::managed_conv_bwd_data_op_t(
        const std::vector<graph_tensor_ptr> &ins,
        const std::vector<graph_tensor_ptr> &outs, const any_map_t &attrs) {
    COMPILE_ASSERT(ins.size() == 2 || ins.size() == 3,
            "managed_conv_bwd_data's inputs size should be 2(output_delta, "
            "filter) or "
            "3(output_delta, filter, output_shape).");
    info_.inputs_ = ins;
    info_.outputs_ = outs;
    attrs_ = attrs;
    op_name_ = "managed_conv_bwd_data";
    COMPILE_ASSERT(attrs_.has_key("output_shape"),
            "managed_conv_bwd_data currently does not support reading dynamic "
            "shape "
            "passed as one of the input.");
    auto out_shape = attrs_.get<sc_dims>("output_shape");
    auto out_dtype = info_.inputs_[0]->details_.dtype_;
    if (outs.empty()) {
        ...
        COMPILE_ASSERT(info_.outputs_.size() == 1,
                "managed_conv_bwd_data expects 1 output.");
        COMPILE_ASSERT(
                info_.outputs_[0]->details_.get_plain_dims() == out_shape,
                "Bad output shape for managed_conv_bwd_data");
        COMPILE_ASSERT(info_.outputs_[0]->details_.dtype_ == out_dtype,
                "Bad output dtype for managed_conv_bwd_data");
    }
}

void managed_conv_bwd_data_op_t::get_graph_impl(
    ...
    if (!is_3D && is_use_amx(ctx) && is_3x3 && stride_all_1 && valid_padding) {
        ...
        conv = graph->make("managed_conv_bwd_data_core", {output_delta, filter},
                {}, attrs);
    }
...
void managed_conv_bwd_data_op_t::query_format(context_ptr ctx,
        std::vector<std::vector<format_stride_pair>> &supported_ins,
        std::vector<std::vector<format_stride_pair>> &supported_outs) {}
...
OP_REGISTER(::sc::ops::managed_conv_bwd_data_op_t, managed_conv_bwd_data)
