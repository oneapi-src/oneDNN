...
struct _jit_avx512_common_1x1_convolution_fwd_t : public cpu_primitive_t {
    ...
    struct pd_t: public _cpu_convolution_fwd_pd_t<with_relu> {
        ...
        DECLARE_COMMON_PD_T(_jit_avx512_common_1x1_convolution_fwd_t<with_relu,
            itype, otype>);
        ...
        virtual status_t init() override {
            ...
            bool ok = true
                ...
                && utils::everyone_is(itype,
                        this->cdesc_().src_desc.data_type,
                        this->cdesc_().weights_desc.data_type)
                && utils::everyone_is(otype,
                    this->cdesc_().dst_desc.data_type)
                && utils::implication(this->with_bias(),
                    otype == this->cdesc_().bias_desc.data_type);
      ...
      protected:
        virtual status_t set_default_params() override {
            ...
            if (this->weights_pd_.desc()->format == any) {
                if (itype == data_type::f32)
                    CHECK(this->weights_pd_.set_format(this->with_groups()
                                                ? gOIhw16i16o : OIhw16i16o));
                else if (itype == data_type::s16)
                    CHECK(this->weights_pd_.set_format(this->with_groups()
    ...
    typedef typename prec_traits<itype>::type src_data_t;
    typedef typename prec_traits<otype>::type dst_data_t;

    typedef typename prec_traits<itype>::type data_t;
...
struct jit_avx512_common_1x1_convolution_bwd_data_t : public cpu_primitive_t {
    struct pd_t : public cpu_convolution_bwd_data_pd_t {
        ...
        DECLARE_COMMON_PD_T(jit_avx512_common_1x1_convolution_bwd_data_t);
