...
void _jit_avx512_mic_convolution_fwd_t<with_relu>::execute_forward()
{
    ...
    auto ker = [&](
            int g, int n, int oc, int ic, int oh, jit_conv_call_s *par_conv) {

        const int ij = oh * jcp.stride_h;
        const int i_t_overflow = nstl::max(0, jcp.t_pad - ij);
        const int i_b_overflow
                = nstl::max(jcp.ih, ij + jcp.kh - jcp.t_pad) - jcp.ih;
        ...
        par_conv->bias_prf = const_cast<data_t *>(
                &bias[bias_d.blk_off((g * jcp.nb_oc + oc) * jcp.oc_block)]);
        par_conv->filt_prf = const_cast<data_t *>(&weights[conf_.with_groups() ?
                        weights_d.blk_off(
                                g, oc, jcp.ic == 3 ? 0 : ic, i_t_overflow, 0) :
                        weights_d.blk_off(
                                oc, jcp.ic == 3 ? 0 : ic, i_t_overflow, 0)]);

        par_conv->kh_padding = par_conv->kh_padding_prf;
        par_conv->kh_padding_prf = jcp.kh - i_t_overflow - i_b_overflow;
