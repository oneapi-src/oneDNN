...
void _jit_avx512_common_conv_bwd_data_kernel_f32<Vmm>::compute_loop_fma(
    ...
    if (jcp.ndims == 5) {
        ...
        add(aux_reg_ker_d,
                typesize * jcp.stride_d * jcp.kw * jcp.kh * oc_block
                        * ic_block);
    ...
    L(kh_label);
    {
        ...
        L(ic_block_label);
        {
            if (l_pad != 0) {
                ...
                add(reg_input,
                        jcp.typesize_in * (ur_w * jcp.stride_w - l_pad)
                                * inp_mult);
            ...
            if (ur_w_trips > 0) {
                ...
                L(ow_block_label);
                {
                    ...
                    add(reg_input,
                            jcp.typesize_in * ur_w * jcp.stride_w * inp_mult);
                    add(reg_output, jcp.typesize_in * ur_w * oc_block);
...
void jit_avx512_common_conv_bwd_weights_kernel_f32 ::compute_oh_loop_common() {
    ...
    if (t_pad > 0) {
        ...
        if (kh_range
            ...
            L(oh_tpad_tail_label);
            {
                ...
                cmp(reg_ih_count, nstl::min(t_pad, oh * stride_h));
    ...
    cmp(reg_ih_count, ihp - b_pad - (kh - 1) * dilate_h);
    jge(oh_label_end, T_NEAR);
    cmp(reg_oj, oh);
    jge(oh_label_end, T_NEAR);
