...
void jit_int8_matmul_utils_kernel_t::gen_reo_b() {
    ...
    int lp = (dyn.ktail > 0) ? dyn.ktail : dyn.k_blk;

    set_preg(prd_ld.b, dyn.n_blk, X_TMP_4, X_TMP_1);
    ...
    b(EQ, last_k);
    L(k_loop);
    reo_B_8x24(dyn.k_blk, 0);
    add_imm(reg_aux_a, reg_aux_a, dyn.k_blk * dyn.N, X_TMP_4);
    sub(reg_k_loop, reg_k_loop, 1);
    cmp(reg_k_loop, 1);
    b(GT, k_loop);
    b(LT, k_end);
    ...
    reo_B_8x24(lp, 0);
    L(k_end);
    add_imm(reg_src, reg_src, dyn.n_blk, X_TMP_4);
    ...
    b(EQ, last_k_1);
    L(k_loop_1);
    reo_B_8x24(dyn.k_blk, dyn.ntail);
    add_imm(reg_aux_a, reg_aux_a, dyn.k_blk * dyn.N, X_TMP_4);
    sub(reg_k_loop, reg_k_loop, 1);
    cmp(reg_k_loop, 1);
    b(GT, k_loop_1);
