...
#include "cpu/aarch64/jit_transpose_utils.hpp"
    ...
    const auto ker = [&](int ithr_oc_mb, int nthr_oc_mb, int ithr_ic, int n,
        ...
        if (gemm_batch > 0 && brg_kernel != nullptr) {
            ...
            if (jbgp.nthr_ic_b == 1 && are_post_ops_applicable
                ...
                brgemm_kernel_execute(
                        brg_kernel, gemm_batch, addr_batch, (void *)ptr_C);
            }
        }
        ...
        if (is_ic_tail) {
            ...
            if (jbgp.nthr_ic_b == 1 && are_post_ops_applicable) {
                ...
                brgemm_kernel_execute(
                        brg_kernel_ic_tail, 1, addr_batch, (void *)ptr_C);
            }
        }
    };
...
template struct brgemm_inner_product_fwd_t<sve_512>;
...
template struct brgemm_inner_product_bwd_data_t<sve_512>;
...
void brgemm_inner_product_bwd_weights_t<isa>::transpose_matrix_c_chunk(
    ...
    ctx.tr_src = (void *)(ti->diff_weights
            + types::data_type_size(jbgp.wei_dt) * get_wei_offset(ocb, icb));
...
char *brgemm_inner_product_bwd_weights_t<isa>::get_wei_acc_ptr(
    ...
    if ((jbgp.nthr_mb > 1 && buf_idx < 0)
        ...
        const int icb_scale = (jbgp.wei_dt == jbgp.acc_dt)
...
template struct brgemm_inner_product_bwd_weights_t<sve_512>;
