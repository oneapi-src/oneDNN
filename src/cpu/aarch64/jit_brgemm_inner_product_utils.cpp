...
#include "cpu/aarch64/jit_brgemm_inner_product_utils.hpp"
...
int get_os_block(const jit_brgemm_primitive_conf_t &jbgp, bool try_to_adjust,
        bool is_adjustment) {
    const bool is_sve512_bf16 = false;
...
bool ip_fwd_adjust_thread_balance(const jit_brgemm_primitive_conf_t &jbgp) {
    if (IMPLICATION(jbgp.is_wei_layout_any, true)) return false;
...
int ip_fwd_get_adjusted_oc_block(const jit_brgemm_primitive_conf_t &jbgp) {
    ...
    if (IMPLICATION(false, not_adjustable_oc_block_size))
...
status_t init_ip_conf_bwd_d(jit_brgemm_primitive_conf_t &jbgp) {
    ...
    jbgp.ic_block = !avoid_max_ic_block && jbgp.ic >= (is_f32 ? 512 : 64) ? 64
            : jbgp.ic >= 32                                               ? 32
                                                                          : 16;
    ...
    if ((num_work_to_parallel < 2 * jbgp.nthr
        ...
        const int min_chunck_sz = (is_sve512_bf16) ? 32 : 16;
        ...
        const int max_nthr_oc_b = (jbgp.oc > 32000) ? jbgp.nthr / 2 : 4;
...
status_t init_ip_conf_bwd_w(jit_brgemm_primitive_conf_t &jbgp) {
    ...
    const bool do_rnd_os = false;

    jbgp.K = jbgp.os_block;
    jbgp.K_tail = (jbgp.os % jbgp.os_block) + (do_rnd_os ? 1 : 0);

    jbgp.nb_os_blocking = 1;
    int os_blocking_max = nstl::min(64, jbgp.nb_os);
...
status_t init_ip_conf(cpu_isa_t isa, jit_brgemm_primitive_conf_t &jbgp,
    ...
    if (is_int8) return status::unimplemented;

    if (is_bf16) return status::unimplemented;

    if (!IMPLICATION(is_f32, isa == sve_512)) return status::unimplemented;
