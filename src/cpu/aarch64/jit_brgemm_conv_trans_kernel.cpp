...
namespace jit_sve512_core_brgemm_conv_trans_kernel {

#define GET_OFF(field) offsetof(jit_brgemm_conv_trans_kernel_call_s, field)

jit_sve512_core_brgemm_conv_trans_kernel_t::
        jit_sve512_core_brgemm_conv_trans_kernel_t(
...
int jit_sve512_core_brgemm_conv_trans_kernel_t::inp_w(int out_w, int kw) const {
    return get_inp_size(out_w, kw, jcp.stride_w, jcp.dilate_w);
}

int jit_sve512_core_brgemm_conv_trans_kernel_t::inp_w(int out_w) const {
    return inp_w(out_w, jcp.ext_kw);
}

int jit_sve512_core_brgemm_conv_trans_kernel_t::dst_w(
...
int jit_sve512_core_brgemm_conv_trans_kernel_t::inp_w_start(int owb) const {
    return get_inp_start(owb, jcp.ow_block, jcp.stride_w, jcp.l_pad);
}

// use different vmovdqu32/16/8 due to case when tail mask used
void jit_sve512_core_brgemm_conv_trans_kernel_t::load(
...
void jit_sve512_core_brgemm_conv_trans_kernel_t::store(
...
void jit_sve512_core_brgemm_conv_trans_kernel_t::zero_ic_block(
...
void jit_sve512_core_brgemm_conv_trans_kernel_t::copy_ic_block(bool is_ic_tail,
...
void jit_sve512_core_brgemm_conv_trans_kernel_t::generate() {
    preamble();
...
void jit_sve512_core_brgemm_conv_trans_kernel_t::copy_ow_block(
        bool is_ic_tail) {
    if (jcp.nb_ow == 1) {
...
void jit_sve512_core_brgemm_conv_trans_kernel_t::copy_ow_block_body(
...
jit_sve512_core_brgemm_conv_rtus_kernel_t::
        jit_sve512_core_brgemm_conv_rtus_kernel_t(
                const jit_brgemm_conv_conf_t &ajcp)
    : jit_sve512_core_brgemm_conv_trans_kernel_t(ajcp) {
    ic_block_sz = inp_dsz * jcp.LDA; // output may or may not be zero padded
    dst_h_offset = jcp.iwp * ic_block_sz;
}

void jit_sve512_core_brgemm_conv_rtus_kernel_t::generate() {
    preamble();
