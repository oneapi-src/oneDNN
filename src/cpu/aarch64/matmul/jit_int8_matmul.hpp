...
#include "cpu/aarch64/jit_dyn_quant_src.hpp"
...
struct jit_int8_matmul_kernel_t : public jit_generator {
    ...
    jit_int8_matmul_kernel_t(
            const dnnl::impl::cpu::aarch64::matmul::brg_int8_t &k)
        : brg(k) {}
...
private:
    ...
    dnnl::impl::cpu::aarch64::matmul::brg_int8_t brg;
...
struct jit_int8_matmul_t : public primitive_t {
    struct pd_t : public dnnl::impl::cpu::matmul::cpu_matmul_pd_t {
        ...
        bool formats_ok() const {
            ...
            const bool is_src
                    = memory_desc_wrapper(src_md()).matches_one_of_tag(
                            format_tag::ab, format_tag::abc, format_tag::abcd);
            return is_dst && is_wei && is_src;
        }
        const brg_int8_t &get_b() const { return brg; }

        const dyn_vals_t &get_d() const { return dyn; }

        int get_idx(int z, int m, int k, int n, brg_int8_t b) const {

            if (b.zp_type_a == jit_int8_broadcast_t::none
                    && b.zp_type_b == jit_int8_broadcast_t::none
                    && b.is_src_f32 == false && z == 1)
                return -1;
            int mt = b.M % b.m_blk;
...
private:
    ...
    std::unique_ptr<jit_dyn_quant_src_kernel_t> reo_ker_a_;
    std::unique_ptr<jit_dyn_quant_src_kernel_t> reo_ker_b_;
    std::unique_ptr<jit_dyn_quant_src_kernel_t> min_max_ker;
    std::unique_ptr<jit_dyn_quant_src_kernel_t> dyn_src_ker;
};
