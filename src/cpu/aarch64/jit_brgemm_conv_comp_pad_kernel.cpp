...
void jit_uni_brgemm_conv_comp_pad_kernel_t::store_accumulators(
    ...
    if (jcp_.src_zero_point) { assert(!"unsupported\n"); }

    if (jcp_.s8s8_compensation_required) { assert(!"unsupported\n"); }
}
...
void jit_uni_brgemm_conv_comp_pad_kernel_t::generate() {
    ...
    const bool is_int8_sve_512
            = (jcp_.src_dt == data_type::s8 || jcp_.src_dt == u8)
            && jcp_.wei_dt == data_type::s8 && !jcp_.has_vnni;
    if (is_int8_sve_512) {
    ...
    const int max_regs = isa_max_regs
            - (is_int8_sve_512 ? 6 : (jcp_.s8s8_compensation_required ? 4 : 3));
