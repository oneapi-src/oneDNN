...
void jit_brdgmm_kernel_base_t::read_params() {
    ...
    if (brg.type == brgemm_offs) {
        ...
        if (brg.brgattr.max_bs > 1) {
            add_imm(X_DEFAULT_ADDR, x4, reg_A_offs_, X_TMP_0); //rsp=x4
            str(reg_aux1_A, ptr(X_DEFAULT_ADDR));
            add_imm(X_DEFAULT_ADDR, x4, reg_B_offs_, X_TMP_0);
    ...
    if (one_of(brg.type, brgemm_addr, brgemm_offs) || has_vpad()) {
        ...
        if (brg.brgattr.max_bs > 1) {
            add_imm(X_DEFAULT_ADDR, x4, reg_batch0_addr_offs_,
                    X_TMP_0); //rsp=x4
    ...
    if (brg.with_bias) {
        ...
        add_imm(X_DEFAULT_ADDR, x4, reg_bias_offs_, X_TMP_0); //rsp=x4
    ...
    if (brg.with_scales) {
        ...
        ldr(reg_tmp, ptr(X_DEFAULT_ADDR));
        add_imm(X_DEFAULT_ADDR, x4, reg_scales_offs_, X_TMP_0);
        str(reg_tmp, ptr(X_DEFAULT_ADDR));
    }
    ...
    if (brg.with_dst_scales) {
        ...
        ldr(reg_tmp, ptr(X_DEFAULT_ADDR));
        add_imm(X_DEFAULT_ADDR, x4, reg_dst_scales_offs_, X_TMP_0);
        str(reg_tmp, ptr(X_DEFAULT_ADDR));
    }

    if (brg.with_binary) {
        add_imm(X_DEFAULT_ADDR, x4, abi_param1_offs_, X_TMP_0);
...
void jit_brdgmm_kernel_base_t::restore_A_B_matrices() {
    if (brg.brgattr.max_bs > 1
        ...
        add_imm(X_DEFAULT_ADDR, x4, reg_batch0_addr_offs_, X_TMP_0);
        ldr(reg_aux_batch_addr, ptr(X_DEFAULT_ADDR));
    }

    if (brg.type == brgemm_strd && brg.brgattr.max_bs > 1) {
        add_imm(X_DEFAULT_ADDR, x4, reg_A_offs_, X_TMP_0);
        ldr(reg_aux1_A, ptr(X_DEFAULT_ADDR));
        add_imm(X_DEFAULT_ADDR, x4, reg_B_offs_, X_TMP_0);
        ldr(reg_aux1_B, ptr(X_DEFAULT_ADDR));
    }
...
void jit_brdgmm_kernel_base_t::set_A_B_matrices() {
    ...
    if (brg.type == brgemm_addr) {
        ...
        ldr(X_TMP_1, ptr(X_DEFAULT_ADDR));
        add(reg_aux_B, reg_aux_A, X_TMP_1);
    } else if (brg.type == brgemm_strd) {
    ...
    add(X_DEFAULT_ADDR, reg_aux_B, X_TMP_1);
    ldr(reg_aux_B, ptr(X_DEFAULT_ADDR));
}
...
void jit_brdgmm_kernel_base_t::store_accumulators_apply_post_ops(
    ...
    const int v_substep = vnni_substep();
    if (brg.with_scales) {
        add_imm(reg_aux_scales, x4, reg_scales_offs_, X_TMP_0); //rsp=x4
        ldr(reg_aux_scales, ptr(X_DEFAULT_ADDR));
        if (brg.is_oc_scale) {
            mov_imm(X_TMP_1, sizeof(float));
            mul(X_TMP_1, reg_aux_N, X_TMP_1);
            add(X_DEFAULT_ADDR, X_TMP_1, reg_aux_scales);
            ldr(reg_aux_scales, ptr(X_DEFAULT_ADDR));
        }
        for_(int m = 0; m < m_blocks; m++)
        ...
        for (int v_i = 0; v_i < v_substep; ++v_i) {
            ...
            if (brg.is_oc_scale) {
                add_imm(X_DEFAULT_ADDR, reg_aux_scales, scales_offset(n, v_i),
                        X_TMP_0);
                ld1w(vmm_tmp(0).s, P_ALL_ONE / T_z, ptr(X_DEFAULT_ADDR));
                if (mask_flag) { //zeroing
                    not_(P_TMP_0.b, P_ALL_ONE.b, k_mask.b);
                    fmul(vmm.s, vmm.s, vmm_tmp(0).s);
                    mov(vmm.s, P_TMP_0 / T_m, 0);
                } else //no mask
                    fmul(vmm.s, vmm.s, vmm_tmp(0).s);
            } else {
                ld1w(vmm_tmp(0).s, P_ALL_ONE / T_z, ptr(reg_aux_scales));
                if (mask_flag) { //zeroing
                    not_(P_TMP_0.b, P_ALL_ONE.b, k_mask.b);
                    fmul(vmm.s, vmm.s, vmm_tmp(0).s);
                    mov(vmm.s, P_TMP_0 / T_m, 0);
    ...
    if (brg.with_bias) {
        add_imm(X_DEFAULT_ADDR, x4, reg_bias_offs_, X_TMP_0);
        ldr(reg_aux_bias, ptr(X_DEFAULT_ADDR));
        mov_imm(X_TMP_1, brg.typesize_bias);
        mul(X_TMP_1, reg_aux_N, X_TMP_1);
        add(X_DEFAULT_ADDR, X_TMP_1, reg_aux_bias);
        ldr(reg_aux_bias, ptr(X_DEFAULT_ADDR));
    }
    ...
    if (brg.with_dst_scales) {
        add_imm(X_DEFAULT_ADDR, x4, reg_dst_scales_offs_, X_TMP_0); //rsp=x4
        ld1w(vmm_tmp(0).s, P_ALL_ONE / T_z, ptr(X_DEFAULT_ADDR));
        auto vmm_dst_scales = vmm_tmp(0);
        ld1rw(vmm_dst_scales.s, P_ALL_ONE, ptr(reg_aux_dst_scales));
        ...
        for (int v_i = 0; v_i < v_substep; ++v_i) {
            ...
            const bool mask_flag = substep_simd < simd_w_;
            const ZReg vmm = accm(m_blocks, n_blocks, m, n, v_i);
            ld1w(vmm_tmp(0).s, P_ALL_ONE / T_z, ptr(reg_aux_dst_scales));
            if (mask_flag) { //zeroing
                not_(P_TMP_0.b, P_ALL_ONE.b, k_mask.b);
                fmul(vmm.s, vmm.s, vmm_tmp(0).s);
                mov(accm(m_blocks, n_blocks, m, n, v_i).s, P_TMP_0 / T_m, 0);
            } else //no mask
                fmul(vmm.s, vmm.s, vmm_tmp(0).s);
        }
    }
...
void jit_brdgmm_kernel_base_t::brdgmm_microkernel(int m_blocks, int n_blocks,
    ...
    auto dot_product = [&](ZReg vmma, ZReg vmmb, int m_i, int n_i, int v_i) {
        ...
        if (brg.is_f32) {
            if (is_fma_embd()) {
                ...
                const auto addr = ptr(X_DEFAULT_ADDR);
                ld1w(vmm_tmp(0).s, P_ALL_ONE / T_m, addr);
                if (mask_flag) {
                    fmla(vmm_acc.s, k_mask / T_m, vmmb.s, vmm_tmp(0).s);
                    not_(P_TMP_0.b, P_ALL_ONE.b, k_mask.b);
                    mov(vmm_acc.s, P_TMP_0 / T_m, 0);
                } else
                    fmla(vmm_acc.s, P_ALL_ONE / T_m, vmmb.s, vmm_tmp(0).s);
            } else {
    ...
    if (!has_padding) {
        ...
        if (has_top_padding) {
            ...
            mov_imm(X_TMP_1, sizeof(void *));
            mul(X_TMP_1, reg_aux_A_vpad_top, X_TMP_1);
            add(X_DEFAULT_ADDR, X_TMP_1, reg_table_base);
            ldr(reg_table_base, ptr(X_DEFAULT_ADDR));
            const auto addr = ptr(reg_table_base);
            br((const Xbyak_aarch64::XReg &)addr);
            align(8);
            L(jmp_table_base);
...
void jit_brdgmm_kernel_base_t::batch_loop(
        ...
        if (has_vpad()) {
            ...
            if (brg.brgattr.max_bottom_vpad > 0) {
                if (do_check_effective_padding) {
                    ...
                    ldr(X_TMP_1, ptr(X_DEFAULT_ADDR));
                    add(reg_aux_A_vpad_bottom, reg_aux_A_vpad_bottom, X_TMP_1);
            ...
            if (brg.brgattr.max_top_vpad > 0) {
                ...
                if (do_check_effective_padding) {
                    ...
                    sub(reg_aux_A_vpad_top, reg_aux_A_vpad_top, reg_aux_M);
    ...
    auto call_brdgmm_microkernel = [&]() {
        ...
        if (vpad_exists) {
            cmp_imm(reg_total_padding, 0, X_TMP_0);
...
void jit_brdgmm_kernel_base_t::compute_loop() {
    ...
    const bool need_separate_n_block1_tail_block
            = n_block1_tail() != 0 && !has_n_block2_tail && nb_n_block2() > 1;
        ...
        L(n_loop_label);
        {
            if (do_loop_n) {
                if (vlen_tail_in_loop) {
                    ...
                    b(LT, done_k_mask);
                    not_(P_TMP_0.b, P_ALL_ONE / T_z, P_NOT_256.b);
                    mov(k_mask.b, P_TMP_0 / T_z, k_tail_mask.b);
                    L(done_k_mask);
                }
            }
...
void jit_brdgmm_kernel_base_t::generate() {
    ...
    preamble();
    mov(x4, X_SP);
    sub_imm(x4, x4, stack_space_needed_, X_TMP_0); //rsp=x4
    ...
    add_imm(x4, x4, stack_space_needed_, X_TMP_0);
    postamble();
