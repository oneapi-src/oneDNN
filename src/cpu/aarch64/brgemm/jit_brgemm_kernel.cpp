    ...
    jit_brgemm_kernel_t(const brgemm_t &abrg)
        ...
        if (brg.with_eltwise || brg.with_binary || brg.with_sum) {
            ...
            const binary_injector::rhs_arg_static_params_t rhs_sp {
                    static_cast<size_t>(Xbyak::Zmm(1).getIdx()), XReg(14),
...
private:
    ...
    const XReg reg_binary_postops_oc_l = reg_rdb_loop;
    const XReg reg_aux_binary_postops_oc_l = reg_rdb_loop;
    const XReg reg_aux_binary_postops_sp = reg_rdb_loop;
    const XReg reg_binary_po_stack_frame = reg_rdb_loop;
...
void jit_brgemm_kernel_t::advance_ldb_post_op_regs() {
    if (brg.with_bias) {
        LDR_IMM(reg_aux_bias, X_TRANSLATOR_STACK, reg_aux_bias_offs_);
        add_imm(reg_aux_bias, reg_aux_bias, bias_offset(1), X_TMP_0);
        STR_IMM(reg_aux_bias, X_TRANSLATOR_STACK, reg_aux_bias_offs_);
    }
    if (brg.with_scales) {
        LDR_IMM(reg_aux_scales, X_TRANSLATOR_STACK, reg_aux_scales_offs_);
        add_imm(reg_aux_scales, reg_aux_scales, scales_offset(1), X_TMP_0);
        STR_IMM(reg_aux_scales, X_TRANSLATOR_STACK, reg_aux_scales_offs_);
    }
    if (with_binary_per_oc_bcast_) {
        LDR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
                reg_aux_binary_postops_oc_l_offs_);
        add_imm(reg_aux_binary_postops_oc_l, reg_aux_binary_postops_oc_l,
                oc_logical_offset(1), X_TMP_0);
        STR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
                reg_aux_binary_postops_oc_l_offs_);
    }
    if (brg.zp_type_a != brgemm_broadcast_t::none) {
        LDR_IMM(reg_aux_zp_comp_a, X_TRANSLATOR_STACK, reg_aux_zp_comp_a_offs_);
        add_imm(reg_aux_zp_comp_a, reg_aux_zp_comp_a, zp_comp_a_offset(1),
                X_TMP_0);
        STR_IMM(reg_aux_zp_comp_a, X_TRANSLATOR_STACK, reg_aux_zp_comp_a_offs_);
    }
    if (brg.zp_type_c == brgemm_broadcast_t::per_n) {
        LDR_IMM(reg_aux_zp_c_values, X_TRANSLATOR_STACK,
                reg_aux_zp_c_values_offs_);
        add_imm(reg_aux_zp_c_values, reg_aux_zp_c_values, zp_c_values_offset(1),
                X_TMP_0);
        STR_IMM(reg_aux_zp_c_values, X_TRANSLATOR_STACK,
                reg_aux_zp_c_values_offs_);
    }
}
...
void jit_brgemm_kernel_t::restore_ldb_post_op_regs(int ld_block2) {
    ...
    if (with_binary_per_oc_bcast_) {
        LDR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
                reg_aux_binary_postops_oc_l_offs_);
        sub_imm(reg_aux_binary_postops_oc_l, reg_aux_binary_postops_oc_l,
                oc_logical_offset(ld_block2 - 1), X_TMP_0);
        STR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
                reg_aux_binary_postops_oc_l_offs_);
    }
    if (brg.zp_type_a != brgemm_broadcast_t::none) {
...
void jit_brgemm_kernel_t::advance_bdb_post_op_regs(int adj_bd_block) {
    ...
    if (brg.zp_type_b != brgemm_broadcast_t::none) {
        LDR_IMM(reg_aux_zp_comp_b, X_TRANSLATOR_STACK, reg_aux_zp_comp_b_offs_);
        add_imm(reg_aux_zp_comp_b, reg_aux_zp_comp_b, bdb_zp_b_offset(1),
                X_TMP_0);
        STR_IMM(reg_aux_zp_comp_b, X_TRANSLATOR_STACK, reg_aux_zp_comp_b_offs_);
    }
...
void jit_brgemm_kernel_t::ldb_regs_shift(int ld_block2, bool is_tail) {
    ...
    if (brg.req_s8s8_compensation) {
        LDR_IMM(reg_aux_compensation, X_TRANSLATOR_STACK, reg_aux_comp_offs_);
        add_imm(reg_aux_compensation, reg_aux_compensation,
                (is_tail) ? compensations_offset(1, true)
                          : compensations_offset(ld_block2),
                X_TMP_0);
        STR_IMM(reg_aux_compensation, X_TRANSLATOR_STACK, reg_aux_comp_offs_);
    }
    if (brg.with_scales) {
        LDR_IMM(reg_aux_scales, X_TRANSLATOR_STACK, reg_aux_scales_offs_);
    ...
    if (with_binary_channel_bcast_) {
        ...
        LDR_IMM(reg_aux_binary_postops_sp, X_TRANSLATOR_STACK,
                reg_aux_binary_postops_sp_offs_);
        add_imm(reg_aux_binary_postops_sp, reg_aux_binary_postops_sp, po_offset,
                X_TMP_0);
        STR_IMM(reg_aux_binary_postops_sp, X_TRANSLATOR_STACK,
                reg_aux_binary_postops_sp_offs_);
    }
    if (with_binary_per_oc_bcast_) {
        LDR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
...
void jit_brgemm_kernel_t::advance_bd_block2_post_op_regs(int bd_block2) {
    if (with_binary_per_oc_sp_bcast_) {
        ...
        add_imm(reg_aux_binary_postops_oc_l, reg_aux_binary_postops_oc_l,
                bd_block2 * brg.bd_block, X_TMP_0);
        STR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
                reg_binary_postops_oc_l_offs_);
    }
    if (with_binary_channel_bcast_) {
        LDR_IMM(reg_aux_binary_postops_sp, X_TRANSLATOR_STACK,
                reg_binary_postops_sp_offs_);
        add_imm(reg_aux_binary_postops_sp, reg_aux_binary_postops_sp,
                bdb_po_offset(bd_block2), X_TMP_0);
        STR_IMM(reg_aux_binary_postops_sp, X_TRANSLATOR_STACK,
                reg_binary_postops_sp_offs_);
    }
    if (brg.zp_type_b != brgemm_broadcast_t::none) {
        LDR_IMM(reg_zp_comp_b, X_TRANSLATOR_STACK, reg_binary_postops_sp_offs_);
        add_imm(reg_zp_comp_b, reg_zp_comp_b, bdb_zp_comp_b_offset(bd_block2),
                X_TMP_0);
        STR_IMM(reg_zp_comp_b, X_TRANSLATOR_STACK, reg_binary_postops_sp_offs_);
    }
}
...
void jit_brgemm_kernel_t::copy_post_ops_stack_values_to_aux(bool is_reg_tail) {
    if (!is_reg_tail) {
        ...
        if (brg.with_bias) {
            LDR_IMM(reg_bias, X_TRANSLATOR_STACK, reg_bias_offs_);
            STR_IMM(reg_bias, X_TRANSLATOR_STACK, reg_aux_bias_offs_);
        }
        if (brg.req_s8s8_compensation) {
            LDR_IMM(reg_compensation, X_TRANSLATOR_STACK, reg_comp_offs_);
            STR_IMM(reg_compensation, X_TRANSLATOR_STACK, reg_aux_comp_offs_);
        }
        if (brg.with_scales) {
            LDR_IMM(reg_scales, X_TRANSLATOR_STACK, reg_scales_offs_);
            STR_IMM(reg_scales, X_TRANSLATOR_STACK, reg_aux_scales_offs_);
        }
        if (with_binary_channel_bcast_) {
            LDR_IMM(reg_aux_binary_postops_sp, X_TRANSLATOR_STACK,
                    reg_binary_postops_sp_offs_);
        ...
        if (with_binary_per_oc_bcast_) {
            LDR_IMM(reg_binary_postops_oc_l, X_TRANSLATOR_STACK,
                    reg_binary_postops_oc_l_offs_);
            STR_IMM(reg_binary_postops_oc_l, X_TRANSLATOR_STACK,
                    reg_aux_binary_postops_oc_l_offs_);
        }

        if (brg.zp_type_a != brgemm_broadcast_t::none) {
            LDR_IMM(reg_zp_comp_a, X_TRANSLATOR_STACK, reg_zp_comp_a_offs_);
            STR_IMM(reg_zp_comp_a, X_TRANSLATOR_STACK, reg_aux_zp_comp_a_offs_);
        }

        if (brg.zp_type_c != brgemm_broadcast_t::none) {
            LDR_IMM(reg_zp_c_values, X_TRANSLATOR_STACK, reg_zp_c_values_offs_);
            STR_IMM(reg_zp_c_values, X_TRANSLATOR_STACK,
                    reg_aux_zp_c_values_offs_);
        }
    }
    if (brg.zp_type_b != brgemm_broadcast_t::none) {
        LDR_IMM(reg_zp_comp_b, X_TRANSLATOR_STACK, reg_zp_comp_b_offs_);
        STR_IMM(reg_zp_comp_b, X_TRANSLATOR_STACK, reg_aux_zp_comp_b_offs_);
    }
    if (with_binary_per_oc_sp_bcast_) {
        LDR_IMM(reg_aux_binary_postops_oc_l, X_TRANSLATOR_STACK,
                reg_binary_postops_oc_l_offs_);
...
void jit_brgemm_kernel_t::read_params() {
    ...
    if (brg.with_binary) {
        STR_IMM(reg_bias, X_TRANSLATOR_STACK, abi_param1_offs_);
    }
    ...
    if (brg.type == brgemm_addr) {
        ...
        if (brg.type == brgemm_offs) {
            ...
            STR_IMM(reg_offs_batch, X_TRANSLATOR_STACK,
                    origin_offs_batch_offs_);
        } else {
            LDR_IMM(reg_strd_batch, param1, GET_OFF(batch));
            STR_IMM(reg_strd_batch, X_TRANSLATOR_STACK,
    ...
    if (brg.req_s8s8_compensation) {
        ...
        str(reg_buf, ptr(X_TRANSLATOR_STACK, reg_buf_offs_));
    }

    if (brg.with_bias) {
        ldr(reg_bias, ptr(param1, GET_OFF(ptr_bias)));
        str(reg_bias, ptr(X_TRANSLATOR_STACK, reg_bias_offs_));
    }
    if (brg.with_scales) {
        ldr(reg_scales, ptr(param1, GET_OFF(ptr_scales)));
        str(reg_scales, ptr(X_TRANSLATOR_STACK, reg_scales_offs_));
    }
    if (with_binary_no_bcast_) {
        ldr(reg_aux_binary_postops_sp, ptr(param1, GET_OFF(data_C_ptr_)));
        str(reg_aux_binary_postops_sp,
                ptr(X_TRANSLATOR_STACK, reg_aux_binary_postops_sp));
    }
    if (with_binary_channel_bcast_) {
        ldr(reg_aux_binary_postops_sp,
                ptr(param1, GET_OFF(first_mb_matrix_addr_off)));
        str(reg_aux_binary_postops_sp,
                ptr(X_TRANSLATOR_STACK, reg_binary_postops_sp_offs_));
    }
    if (with_binary_per_oc_bcast_) {
        ldr(reg_binary_postops_oc_l, ptr(param1, GET_OFF(oc_logical_off)));
        str(reg_binary_postops_oc_l,
                ptr(X_TRANSLATOR_STACK, reg_binary_postops_oc_l_offs_));
    } else if (with_binary_per_oc_sp_bcast_) {
        ldr(reg_binary_postops_oc_l, ptr(param1, GET_OFF(dst_row_logical_off)));
        str(reg_binary_postops_oc_l,
                ptr(X_TRANSLATOR_STACK, reg_binary_postops_oc_l_offs_));
    }
    ...
    if (brg.zp_type_a != brgemm_broadcast_t::none) {
        ...
        str(reg_zp_comp_a, ptr(X_TRANSLATOR_STACK, reg_zp_comp_a_offs_));
    }

    if (brg.zp_type_b != brgemm_broadcast_t::none) {
        ldr(reg_zp_comp_b, ptr(param1, GET_OFF(b_zp_compensations)));
        str(reg_zp_comp_b, ptr(X_TRANSLATOR_STACK, reg_zp_comp_b_offs_));
    }

    if (brg.zp_type_c != brgemm_broadcast_t::none) {
        ldr(reg_zp_c_values, ptr(param1, GET_OFF(c_zp_values)));
        str(reg_zp_c_values, ptr(X_TRANSLATOR_STACK, reg_zp_c_values_offs_));
    }

    if (brg.with_dst_scales) {
        ldr(reg_dst_scales, ptr(param1, GET_OFF(ptr_dst_scales)));
        str(reg_dst_scales, ptr(X_TRANSLATOR_STACK, reg_dst_scales_offs_));
    }

    ldr(reg_do_post_ops, ptr(param1, GET_OFF(do_post_ops)));
    str(reg_do_post_ops, ptr(X_TRANSLATOR_STACK, reg_do_post_ops_offs_));

    ldr(reg_skip_accm, ptr(param1, GET_OFF(skip_accm)));
    str(reg_skip_accm, ptr(X_TRANSLATOR_STACK, reg_skip_accm_offs_));

    ldr(reg_zp_a_val, ptr(param1, GET_OFF(zp_a_val)));
    str(reg_zp_a_val, ptr(X_TRANSLATOR_STACK, reg_zp_a_val_offs_));

    ldr(reg_do_comp, ptr(param1, GET_OFF(do_apply_comp)));
    str(reg_do_comp, ptr(X_TRANSLATOR_STACK, reg_do_comp_offs_));
}
...
void jit_brgemm_kernel_t::store_accumulators_apply_post_ops(
    ...
    if (brg.with_scales) {
        add_imm(X_DEFAULT_ADDR, X_TRANSLATOR_STACK, reg_aux_scales_offs_,
                X_TMP_0);
    ...
    if (brg.with_dst_scales) {
        add_imm(X_DEFAULT_ADDR, X_TRANSLATOR_STACK, reg_dst_scales_offs_,
                X_TMP_0);
    ...
    if (brg.zp_type_c != brgemm_broadcast_t::none) {
        add_imm(X_DEFAULT_ADDR, X_TRANSLATOR_STACK, reg_aux_zp_c_values_offs_,
                X_TMP_0);
...
void jit_brgemm_kernel_t::apply_compensation(
    ...
    if (!brg.req_cal_comp_pads && brg.zp_type_a != brgemm_broadcast_t::none) {
        ...
        add_imm(X_DEFAULT_ADDR, X_TRANSLATOR_STACK, reg_zp_a_val_offs_,
                X_TMP_0);
        ldr(reg_zp_a_val, ptr(X_DEFAULT_ADDR));
        ldr(W_TMP_0, ptr(reg_zp_a_val));
        dup(vmm_zp_a_val.s, W_TMP_0);

        add_imm(X_DEFAULT_ADDR, X_TRANSLATOR_STACK, reg_aux_zp_comp_a_offs_,
                X_TMP_1);
    ...
    if (brg.zp_type_b != brgemm_broadcast_t::none) {
        add_imm(X_DEFAULT_ADDR, X_TRANSLATOR_STACK, reg_aux_zp_comp_b_offs_,
                X_TMP_0);
    ...
    if (!brg.req_cal_comp_pads && brg.req_s8s8_compensation) {
        ldr(reg_aux_compensation, ptr(X_TRANSLATOR_STACK, reg_aux_comp_offs_));
        for (int ld = 0; ld < ld_block2; ld++) {
...
void jit_brgemm_kernel_t::store_accumulators(int bd_block2, bool is_bdb_tail,
    ...
    if (are_post_ops_applicable) {
        ...
        LDR_IMM(reg_do_post_ops, X_TRANSLATOR_STACK, reg_do_post_ops_offs_);
...
void jit_brgemm_kernel_t::restore_A_B_matrices() {
    ...
    if (brg.type == brgemm_addr) {
        ...
        if (restore_reg_batch) {
            if (brg.type == brgemm_offs) {
                ldr(reg_offs_batch,
                        ptr(X_TRANSLATOR_STACK, origin_offs_batch_offs_));
            } else {
                ldr(reg_offs_batch,
                        ptr(X_TRANSLATOR_STACK, origin_strd_batch_offs_));
            }
        }
    }
}
...
void jit_brgemm_kernel_t::ldb_loop(int bd_block2, bool is_bdb_tail,
    ...
    L_aligned(ldb_loop_label, 64);
    {
        ...
        if (is_ldb_loop_) {
            STR_IMM(reg_D, X_TRANSLATOR_STACK, reg_D_offs_);
        } else {
            mov(reg_ldb_loop, reg_D);
        }
        if (brg.brgattr.max_bs > 1) {
            STR_IMM(reg_aux_D, X_TRANSLATOR_STACK, reg_aux_D_offs_);
        }
                ...
                if (check_top_vpad || check_bottom_vpad) {
                    ...
                    if (vpad_exist) {
                        ...
                        if (brg.type == brgemm_strd) {
                            LDR_IMM(reg_strd_batch, X_TRANSLATOR_STACK,
                                    origin_strd_batch_offs_);
                        }
                        ldr(reg_aux_A_vpad,
                                ptr(reg_batch,
        ...
        if (is_ldb_loop_) {
            LDR_IMM(reg_D, X_TRANSLATOR_STACK, reg_D_offs_);
        } else {
            mov(reg_D, reg_ldb_loop);
        }
        if (brg.brgattr.max_bs > 1) {
            LDR_IMM(reg_aux_D, X_TRANSLATOR_STACK, reg_aux_D_offs_);
        }
...
void jit_brgemm_kernel_t::bdb_loop() {
    ...
    if (brg.brgattr.generate_skip_accumulation) {
        ...
        LDR_IMM(reg_skip_accm, X_TRANSLATOR_STACK, reg_skip_accm_offs_);
...
void jit_brgemm_kernel_t::generate() {
    ...
    sub_imm(X_TRANSLATOR_STACK, X_TRANSLATOR_STACK, stack_space_needed_,
            X_TMP_0); //rsp=X_TRANSLATOR_STACK
    ...
    if (brg.is_int8 && !brg.has_vnni) { assert(!"unsupported\n"); }

    read_params();

    bdb_loop();

    add_imm(X_TRANSLATOR_STACK, X_TRANSLATOR_STACK, stack_space_needed_,
            X_TMP_0);
