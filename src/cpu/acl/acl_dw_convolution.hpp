...
#ifndef CPU_ACL_DW_CONVOLUTION_HPP
#define CPU_ACL_DW_CONVOLUTION_HPP
...
struct acl_dw_resource_t : public resource_t {
    acl_dw_resource_t()
    ...
    status_t configure(const acl_conv_conf_t &acp) {
        ...
        acl_obj_->conv.configure(&acl_obj_->src_tensor, &acl_obj_->wei_tensor,
                ...
                &acl_obj_->dst_tensor, acp.padstride_info, 1, acp.act_info,
                acp.dilation_info);
    ...
    DNNL_DISALLOW_COPY_AND_ASSIGN(acl_dw_resource_t);
...
struct acl_dw_convolution_fwd_t : public primitive_t {
    struct pd_t : public cpu_convolution_fwd_pd_t {
        ...
        DECLARE_COMMON_PD_T(
                "dw:acl", acl_dw_convolution_fwd_t, USE_GLOBAL_SCRATCHPAD);
        ...
        status_t init(engine_t *engine) {
            ...
            CHECK(acl_convolution_utils::init_conf_dw(acp_, src_md_,
    ...
    acl_dw_convolution_fwd_t(const pd_t *apd) : primitive_t(apd) {}

    status_t create_resource(
            engine_t *engine, resource_mapper_t &mapper) const override {
        if (mapper.has_resource(this)) return status::success;

        auto r = utils::make_unique<acl_dw_resource_t>();
