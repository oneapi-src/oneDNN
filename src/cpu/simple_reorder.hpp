...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
        typename utils::enable_if<(tag_i == oiw && tag_o == OIw4i16o4i)
                        || (tag_i == goiw && tag_o == gOIw4i16o4i)
                        || (utils::one_of(tag_i, hwio, oihw)
                                && tag_o == OIhw4i16o4i)
                        || (utils::one_of(tag_i, goihw, hwigo)
                                && one_of(tag_o, gOIhw4o4i, gOIhw2i8o4i,
                                        gOIhw4i16o4i)),
                spec::conv_s8s8>::type> {
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
            ...
            for (int I = 0; I < NB_IC; I++)
                for (int h = 0; h < H; h++)
                    for (int w = 0; w < W; w++) {
                        ...
                        const int oc_block
                                = nstl::min(blksize, OC - O * blksize);
                        const int ic_block
                                = nstl::min(blksize, IC - I * blksize);
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
        typename utils::enable_if<false || (tag_i == goiw && tag_o == Goiw16g)
                        || (utils::one_of(tag_i, goihw, hwigo)
                                && tag_o == Goihw16g),
                spec::conv_s8s8>::type> {
    static bool is_applicable(const memory_desc_wrapper &input_d,
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
        typename utils::enable_if<((tag_i == goihw || tag_i == oihw)
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
        typename utils::enable_if<(tag_i == nchw && tag_o == nChw16c)
                && type_i == data_type::f32
                && type_o == data_type::bf16>::type> {
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
...
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
        typename utils::enable_if<false
                ...
                || (utils::one_of(tag_i, nCw4c, nCw8c)
                        && tag_o == nCw16c)>::type> {
    static bool is_applicable(const memory_desc_wrapper &input_d,
            const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
            ...
            if (alpha == 1.0 && beta == 0.0) {
                ...
                for (int l = 0; l < L; ++l)
                    for (int blk = 0; blk < block; ++blk) {
                        ...
                        if (order_keep) {
                            o[l * l_blk_stride + blk]
                                    = _qz<type_i, type_o>()(i[flat_off],
                                            o[l * blksize + blk], alpha, beta);
                        } else {
                ...
                for (int h0 = 0; h0 < block_h0; ++h0)
                    for (int h1 = 0; h1 < block_h1; ++h1) {
                        ...
                        if (order_keep) {
                            ...
                            o[flat_off]
                                    = _qz<type_i, type_o>()(i[blk_off(h0, h1)],
...
template <SIMPLE_REORDER_TEMPL_DECL>
struct simple_reorder_impl<SIMPLE_REORDER_TEMPL_CALL,
        typename utils::enable_if<tag_i == any && tag_o == any
