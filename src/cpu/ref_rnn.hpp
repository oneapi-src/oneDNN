...
#define elemwise_sig(f)                                                        \
    void f(int s_size, int batch, int n_states, int n_gates, float *ws_gates_, \
            float *states_t_l_, float *states_t_lm1_, float *states_tm1_l_,    \
            float *diff_states_t_l_, float *diff_states_t_lp1_,                \
            float *diff_states_tp1_l_, const float *bias_)

#define cell_execution_sig(f)                                                 \
    void f(int s_size, int x_size, int h_size, int batch, int n_gates,        \
...
#define grid_execution_sig(f)                                              \
    void f(int s_size, int x_size, int h_size, int batch, int n_layer,     \
...
#define gemm_sig(f)                                                    \
    void f(int m, int n, int k, const float *a_, float *b_, float *c_, \
            bool is_B_trans, float beta)
...
struct _ref_rnn_common_t : public cpu_primitive_t {
    ...
    _ref_rnn_common_t(const pd_t *pd, const input_vector &inputs,
        ...
        /// iterations and layer to one if x_size != s_size and h_size != s_size
        /// respectively
        ...
        // - for states:
        //   - TODO: allocate only n_iter * s_size + s_size for linear execution
        //   (inference)
        //   - TODO: allocate only n_layer_wav * (2*s_size) for wavefront
        //   execution (inference)
        // - for gates:
        //   - TODO: allocate only batch * n_gates * s_size for linear execution
        //   (inference)
        //   = TODO: allocate only n_layer_wav * batch * n_gates * s_size for
...
private:
    ...
    void copy_init_layer(bool lr, bool rl, int n_direction, int n_layer,
            int n_iter, int batch, int x_size, int n_states, float *ws_states_,
    ...
    void copy_init_iter(int n_layer, int n_direction, int n_states, int batch,
            int h_size, int n_iter, float *ws_states_, float *ws_diff_states_,
            const float *firstit_states_, const float *diff_dst_iter);
    void copy_res_layer(bool lr, bool rl, int n_layer, int n_direction,
            int n_iter, int batch, int n_output_features, int s_size,
    ...
    void copy_res_iter(int n_layer, int n_direction, int n_states, int batch,
            int s_size, int n_iter, float *dst_iter_, float *diff_src_iter,
