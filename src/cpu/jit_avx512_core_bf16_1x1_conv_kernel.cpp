        ...
        int nthreads, bool reduce_src)
{
    ...
    jcp.bia_dt = jcp.with_bias ? pick_by_prop_kind(jcp.prop_kind, cd.bias_desc.data_type,
    ...
    auto wei_tag = utils::pick(2 * ndims - 6 + with_groups + 4 * is_bwd_d,
    ...
    if (jcp.expl_bcast) {
        ...
        if (spatial <= SMALL_SPATIAL && jcp.reduce_dim >= BIG_REDUCE_DIM)
            reduce_blocking = 16;
        else if (spatial > SMALL_SPATIAL && jcp.reduce_dim >= BIG_REDUCE_DIM)
            reduce_blocking = 8;
    ...
    if (jcp.bcast_dim * reduce_blocking > way_size * max_hits) {
        ...
        for (int start_off = 0; start_off < jcp.ur; start_off++) {
            for (int off = start_off, hits = 0; off < sp * nrb; off += wl) {
                ...
                reduce_blocking = nstl::min(reduce_blocking, max_r_blocking);
                break;
            }
        }
    }
    ...
    if (nthreads <= 28 && jcp.mb < nthreads
        ...
        auto calc_job_cost = [&](int lb, int tg, float mem_k) {
            ...
            float calc_size = (float)(bb_size * jcp.ur) * (lb * jcp.load_block)
                    * jcp.reduce_dim;
    ...
    if (jcp.expl_bcast && jcp.bcast_dim <= 64
            && load_size >= L2_size) {
        jcp.load_grp_count = nstl::max(jcp.load_grp_count, 4);
    } else if (jcp.bcast_dim <= 49 && jcp.mb <= nthreads && jcp.load_dim > 512
