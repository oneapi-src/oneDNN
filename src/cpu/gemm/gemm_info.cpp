...
void gemm_info_t<a_type, b_type, c_type>::jit_init(void) {
    ...
    static void (*kern[2][2][2][2])(const dim_t *m, const dim_t *n,
            const dim_t *k, const float *alpha, const a_type *a,
            const b_type *b, c_type *c, const dim_t ldc,
            const c_type *col_offset, const c_type *row_offset)
            = {{{{NULL}}}};

    // gemv_kern[trans]
    static void (*gemv_kern[2])(const dim_t *m, const dim_t *n,
            const float *alpha, const a_type *a, const dim_t *lda,
            const b_type *x, const dim_t *incy, c_type *y)
            = {NULL};
                    ...
                    for (int isBeta0 : {no_beta0, do_beta0})
                        for (int isColOffset : {no_col_offset, do_col_offset})
                            for (int isRowOffset :
                                ...
                                kernel[isBeta0][no_alpha1][isColOffset]
                                      [isRowOffset]
                ...
                if (mayiuse(avx512_core)) {
                    for (int isBeta0 : {no_beta0, do_beta0})
                        for (int isAlpha1 : {no_alpha1, do_alpha1}) {
                            kernel[isBeta0][isAlpha1][no_col_offset]
                                  [no_row_offset]
            ...
            for (int isAlpha1 : {no_alpha1, do_alpha1})
                for (int isColOffset : {no_col_offset, do_col_offset})
                    for (int isRowOffset : {no_row_offset, do_row_offset}) {
                        auto *p_kernel = kernel[isBeta0][isAlpha1][isColOffset]
                                               [isRowOffset];
                        if (p_kernel != NULL)
        ...
        case data_type::s8:
            if (mayiuse(avx512_core)) {
                for (int isBeta0 : {no_beta0, do_beta0})
                    for (int isColOffset : {no_col_offset, do_col_offset})
                        for (int isRowOffset : {no_row_offset, do_row_offset})
                            if (!this->kernel[isBeta0][isColOffset]
                                             [isRowOffset])
                                return false;
