...
struct jit_brgemm_kernel_diff_bias : public jit_generator {
    jit_brgemm_kernel_diff_bias(
            const jit_brgemm_primitive_conf_t &ajbgp, const brgemm_t &abrg)
        : brg(abrg)
        , jbgp(ajbgp)
        , ddst_dt_(jbgp.dst_dt)
        , bia_dt_(jbgp.bia_dt)
        , acc_dt_(jbgp.acc_dt)
    ...
    DECLARE_CPU_JIT_AUX_FUNCTIONS(jit_brgemm_kernel_diff_bias)

    brgemm_t brg;
    jit_brgemm_primitive_conf_t jbgp;
...
private:
    ...
    void loop_by_N(int n_loop, int nb_tail) {
        ...
        for (; n_ < n_iters; n_++) {
            ...
            auto addr = ptr[reg_bias_acc + acc_typesize_ * n_ * brg.n_block];
            vmovups(vbias, addr);
        }
        if (nb_tail > 0) {
        ...
        mov(reg_k_iter, utils::div_up(brg.K, mult));
        L(k_loop);
        {
            int n_ = 0;
            for (; n_ < n_iters; n_++) {
                ...
                auto addr = ptr[aux_reg_ddst
                        + ddst_typesize_ * mult * n_ * brg.n_block];
                vmovups(vddst, addr);
            ...
            add(aux_reg_ddst, ddst_typesize_ * mult * brg.LDB);
        ...
        for (; n_ < n_iters; n_++) {
            auto vbias = get_bias_reg(n_);
            auto addr = ptr[reg_bias + bia_typesize_ * n_ * brg.n_block];
    ...
    void generate() override {
        ...
        int nb = utils::div_up(brg.N, brg.n_block);
        int nb_tail = brg.N % brg.n_block;
        ...
        for (int nb_ = 0; nb_ < n_loop; nb_++) {
            ...
            add(reg_ddst, ddst_typesize_ * mult * n_max_regs_ * brg.n_block);
            add(reg_bias, bia_typesize_ * n_max_regs_ * brg.n_block);
            add(reg_bias_acc, acc_typesize_ * n_max_regs_ * brg.n_block);
        }
