...
/// Sets LDB2 in brgattr if needed
///
/// @param brgattr Output kernel attributes to set LDB2
/// @param weights_d Specifies the layout of wights.
/// @param vN Specifies the N of the brgemm.
/// @param vnni_granularity Specifies the required vnni granularity.
///
template <cpu_isa_t isa>
status_t brgemm_matmul_t<isa>::pd_t::maybe_set_LDB2() {

    if (bgmmc_.LDB >= bgmmc_.N_blk) {
        if (bgmmc_.LDB % bgmmc_.N_blk == 0 || bgmmc_.N_blk == bgmmc_.N) {
            return status::success;
        } else {
            return status::unimplemented;
        }
    } else {
        if (bgmmc_.N_blk % bgmmc_.LDB == 0 || bgmmc_.N_blk == bgmmc_.N) {
