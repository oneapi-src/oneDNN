...
struct jit_brgemm_matmul_copy_a_impl_t : public jit_brgemm_matmul_copy_a_t,
    ...
    jit_brgemm_matmul_copy_a_impl_t(const brgemm_matmul_conf_t *conf)
        ...
        , src_stride_(conf_->src_tag == format_tag::acbd
...
private:
    ...
    static constexpr int k_loop_unroll_ = 16;
    static constexpr int num_comp_acc_ = 8;
...
void jit_brgemm_matmul_copy_a_impl_t<Vmm>::copy_K_loop(
    ...
    if (do_compute_compensation_) {
        for (int i = 0; i < num_acc; i++) {
            const auto zmm_acc = get_vmm_comp_acc(i);
            vpxord(zmm_acc, zmm_acc, zmm_acc);
        }
    }

    auto maybe_compute_compensation = [=](int k_idx, Vmm zmm_copy) {
        if (do_compute_compensation_) {
            const auto zmm_comp_acc = get_vmm_comp_acc(k_idx % num_acc);
            if (conf_->src_dt == data_type::s8)
                vpdpbusd(zmm_comp_acc, vmm_comp_mul, zmm_copy);
            else
                vpdpbusd(zmm_comp_acc, zmm_copy, vmm_comp_mul);
        }
    };
    ...
    for (int kb = 0; kb < div_up(num_k_iters, k_loop_unroll_); kb++) {
        int k_start = 0;
        int k_end = nstl::min(
                (int)k_loop_unroll_, num_k_iters - kb * k_loop_unroll_);
        for (int k = k_start; k < k_end; k++) {
            const int k_idx = kb * k_loop_unroll_ + k;
            const size_t offset = (size_t)k_idx * k_step_ * typesize_;
            const auto addr = EVEX_compress_addr(reg_src, offset);
            if (conf_->isa == avx512_core_fp16) {
                vcvtph2psx(get_vmm_copy(k), addr);
            } else {
                vmovdqu8(get_vmm_copy(k), addr);
            }
        ...
        if (conf_->is_bf32) {
            ...
            for (int k = k_start; k < k_end; k++) {
                const size_t offset = ((size_t)kb * k_loop_unroll_ + k)
                        * k_step_ * tr_typesize_;
                auto tr_src_addr = EVEX_compress_addr(reg_tr_src, offset);
                vmovdqu8(tr_src_addr, get_vmm_copy(k));
            }
        }
    }
    ...
    if (k_tail > 0) {
        ...
        const size_t dt_step = conf_->is_bf32 || conf_->isa == avx512_core_fp16
                ? 1
                : typesize_;
        ...
        auto load_addr = EVEX_compress_addr(
                reg_src, num_k_iters * k_step_ * typesize_);
        if (conf_->is_bf32)
        ...
        auto tr_src_addr = EVEX_compress_addr(
                reg_tr_src, num_k_iters * k_step_ * tr_typesize_);
        if (conf_->is_bf32) {
    ...
    if (do_compute_compensation_) {
        ...
        if (!is_first_K_iter)
            vpaddd(get_vmm_comp_acc(0), get_vmm_comp_acc(0), addr_buf);
        if (!is_last_K_iter) {
            vmovups(addr_buf, get_vmm_comp_acc(0));
            return;
        }
        ...
        const auto zmm_zp_b_neg_val = get_vmm_comp_acc(1);
        vbroadcastss(zmm_zp_b_neg_val, ptr[reg_zp_b_neg_val_ptr]);
        vpmulld(get_vmm_comp_acc(0), get_vmm_comp_acc(0), zmm_zp_b_neg_val);
...
void jit_brgemm_matmul_copy_a_transposed_impl_t::transpose_bf16(
    ...
    auto kmovx = [=](Opmask k, unsigned w, bool use_word_sz = false) {
        mov(regw_tmp, w);
        if (use_word_sz)
    ...
    for (int i = 0; i < nrows / 2; i++) {
        ...
        auto src_addr_0 = EVEX_compress_addr(src, idx0 * src_stride);
        auto src_addr_1 = EVEX_compress_addr(src, idx1 * src_stride);
        if (is_bf32) {
    ...
    if (nrows % 2) {
        ...
        auto src_addr = EVEX_compress_addr(src, 2 * i * src_stride);
        if (is_bf32) {
...
void jit_brgemm_matmul_copy_a_transposed_impl_t::transpose_f32(
    ...
    auto load = [=](int i) {
        const auto addr = EVEX_compress_addr(src, i * src_stride);
        if (i < nrows)
            if (conf_->isa == avx512_core_fp16)
...
struct jit_brgemm_matmul_copy_b_transposed_t
    ...
    jit_brgemm_matmul_copy_b_transposed_t(const brgemm_matmul_conf_t *conf)
        ...
        , tr_typesize(conf_->tr_b_dt_sz)
        , vnni_granularity(data_type_vnni_granularity(conf_->wei_dt))
        , k_blk_step(bytes_in_zmm / tr_typesize)
        , do_compute_compensation(
                  conf_->has_zero_point_a || conf_->s8s8_compensation_required)
        , is_bf32(conf->is_bf32)
        , req_zp_comp(conf_->has_zero_point_a)
        , req_s8s8_comp(conf_->s8s8_compensation_required)
        , src_stride(conf_->wei_tag == format_tag::adbc
                          ? conf_->copy_B_wei_stride
                          : conf_->K * typesize)
        , tr_src_stride(conf_->LDB * vnni_granularity * tr_typesize) {}
...
void jit_brgemm_matmul_copy_b_transposed_t::copy_16x64_vnni(
    ...
    const int columns_tail
            = ncolumns % (is_bf32 ? bf32_k_blk_step : k_blk_step);
    if (columns_tail > 0) {
        const int dt_step
                = (is_bf32 || conf_->isa == avx512_core_fp16) ? 1 : typesize;
    ...
    auto load_bf32 = [=](int i) {
        ...
        auto zmm_src = columns_tail > 0 && ncolumns < bf32_k_blk_step
                ? src_reg | kTail | T_z
                : src_reg;
        vmovups(zmm_src, EVEX_compress_addr(reg_src, i * src_stride));

        if (ncolumns <= bf32_k_blk_step) {
            ...
            vmovups(zmm_src_next,
                    EVEX_compress_addr(reg_src,
                            i * src_stride + bf32_k_blk_step * typesize));
        }
    ...
    auto load = [=](int i) {
        ...
        const auto addr = EVEX_compress_addr(reg_src, i * src_stride);
        if (conf_->isa == avx512_core_fp16)
...
void jit_brgemm_matmul_copy_b_transposed_t::compute_N_loop(
    ...
    add(reg_tr_src_base, n_blk_step * vnni_granularity * tr_typesize);
...
status_t create_brgemm_matmul_copy_a(
    ...
    if (conf->transposed_A) {
        ...
        if (is_superset(conf->isa, avx512_core))
            ...
            assert(!"Unsupported templated JIT kernel");
    }
