...
struct jit_uni_fork_dw_conv_fwd_kernel {
    ...
    jit_uni_fork_dw_conv_fwd_kernel(const jit_conv_conf_t &ajcp, const primitive_attr_t &attr) : ker_(nullptr) {
...
status_t jit_uni_fork_dw_conv_fwd_kernel<isa, kernel_dt>::init_conf(
    ...
    const auto blocked_tag = (ndims == 5) ? one_of(isa, avx512_common, avx512_core) ? nCdhw16c : nCdhw8c
