...
struct brg_blocking_t : public jit_brgemm_conv_conf_t {
    ...
    brg_blocking_t() : jit_brgemm_conv_conf_t() {
...
status_t brg_blocking_t::estimate_brgemm_ur() {
    ...
    LDA = is_rtus ? (oc_block)
                  : (kh_sets > 1 ? kh_sets : 1) * (kw_sets > 1 ? kw_sets : 1)
                    * (exec_type == exec_trans ? oc_block
                                               : ngroups * oc_without_padding);
    LDB = ic_block;
    ...
    N_tail = ic % ic_block;
    K = kh_sets * kw_sets * (oc >= oc_block ? oc_block : 0);
    K_tail = kh_sets * kw_sets
            * (exec_type == exec_trans
                            ? oc_block
                            : rnd_up(oc % oc_block, last_oc_block_size));
...
float brg_blocking_t::est_eff() {
    ...
    const auto nb_ih_thr = nstl::min(nb_ih, div_up(job, dim_ih));
    const auto ih_thr = nstl::min(ih, nb_ih_thr * ih_block);

    const auto dim_id = nb_ih * dim_ih;
    const auto nb_id_thr = nstl::min(nb_id, div_up(job, dim_id));
    const auto id_thr = nstl::min(id, nb_id_thr * id_block);

    src_is = kd * kh * rnd_inp_simd(sp_block, kw, oc);

    auto wei_ip = kd * kh * kw * icblock * oc;
    if (loop_order == loop_ndhwgc) {
        ...
        wei_ip = kd * kh * kw * nsimd_ic_thr * oc;
        loop[l].wei.set(wei_is, 1);
    }
    ...
    loop[l].wei.set(wei_ip * simd_w, nb_sp_thr);
    ...
    loop[l].wei.set(wei_ip * simd_w, nb_ih_thr);
    ...
    loop[l].wei.set(wei_ip * simd_w, nb_id_thr);
    ...
    const auto src_ip = static_cast<dim_t>(mb_thr) * id_thr
            * (is_is_blocking ? 1 : ih_thr) * sp_thr * kd * kh * kw * oc;
    const auto dst_ip = static_cast<dim_t>(mb_thr) * id_thr
            * (is_is_blocking ? 1 : ih_thr) * sp_thr * nsimd_ic_thr;
    wei_ip = kd * kh * kw * nsimd_ic_thr * oc;

    // for "real" application set bench_iterations to 1
    const auto iterations = bench_iterations;
    l++;
    loop[l].src.set(src_ip, iterations);
    loop[l].dst.set(dst_ip * simd_w, iterations);
    loop[l].wei.set(wei_ip * simd_w, iterations);
    ...
    const auto src_ips = (src_ip * src_rp) / iterations;
    const auto dst_ips = (dst_ip * dst_rp) / iterations;
    const auto wei_ips = (wei_ip * wei_rp) / iterations;

    const auto src_cost = src_mem_k * src_ips;
    const auto dst_cost = dst_mem_k * dst_ips;
    const auto wei_cost = wei_mem_k * wei_ips;
...
void brg_blocking_t::iterate_ker_block(brg_blocking_t &best_brgb, int kd_block_,
    ...
    unsigned est_k_amount = oc * ic_block * wei_dsz;
    ...
    if (exec_type == exec_vpad) {
        ...
        const auto w_block_size
                = 2 * src_dsz * oc * owp + dst_dsz * iw * ic_block;
        ...
        if (is_amx(isa)) {
            ...
            if (cur_id_block == 1) {
                for (; cur_ih_block > 1; cur_ih_block--) {
                    ...
                    if ((static_cast<float>(ih) / rnd_up(ih, cur_ih_block))
                                    > 0.9f
                            && sp_size > 128) {
                        L1_fit_res = true;
                        break;
        ...
        const auto thr_ic_block = div_up(
                nthr, mb * div_up((ic > 32 ? ngroups : 1) * ic, ic_block));
        ...
        while ((id_block % stride_d != 0 || id % id_block != 0)
                && id_block < id)
            id_block++;
        while ((ih_block % stride_h != 0 || ih % ih_block != 0)
                && ih_block < ih)
            ih_block++;
    } else {
        id_block = 1;
...
float brg_blocking_t::est_eff_1x1() {
    ...
    const auto sp_amount = is_is_blocking ? div_up(nb_is, nb_is_blocking)
                                          : nb_id * nb_ih * nb_sp;
    const auto work_amount = mb * ngroups * nb_ic * sp_amount;
    ...
    const auto nb_ih_thr = nstl::min(nb_ih, div_up(job, dim_ih));
    const auto ih_thr
            = is_is_blocking ? 1 : nstl::min(ih, nb_ih_thr * ih_block);

    const auto dim_id = nb_ih * dim_ih;
    const auto nb_id_thr = nstl::min(nb_id, div_up(job, dim_id));
    const auto id_thr
            = is_is_blocking ? 1 : nstl::min(id, nb_id_thr * id_block);
    ...
    if (job < nthr) {
        ...
        for (int ithr = 0; ithr < nthr; ithr++) {
            ...
            int start {0}, end {0};
            balance211(work_amount, nthr, ithr, start, end);
            int n {0}, g {0}, icb {0}, iss {0}, idp {0}, ihp {0}, spb {0};
            if (loop_order == loop_ndhwgc) {
                if (is_is_blocking)
                    nd_iterator_init(start, n, mb, iss, sp_amount, g, ngroups,
                            icb, nb_ic);
                else
                    nd_iterator_init(start, n, mb, idp, id, ihp, ih, spb, nb_sp,
                            g, ngroups, icb, nb_ic);
            } else if (loop_order == loop_ngcdhw) {
                if (is_is_blocking)
                    nd_iterator_init(start, n, mb, g, ngroups, icb, nb_ic, iss,
                            sp_amount);
                else
                    nd_iterator_init(start, n, mb, g, ngroups, icb, nb_oc, idp,
                            id, ihp, ih, spb, nb_sp);
            }
            ...
            for (auto work = start; work < end; work++) {
                ...
                int sp_sz = 0;
                if (is_is_blocking) {
                    const auto isb_start = iss * nb_is_blocking;
                    const auto isb_range
                            = nstl::min(nb_is - isb_start, nb_is_blocking);
                    for (int isb = 0; isb < isb_range; isb++) {
                        const int isp = (isb_start + isb) * sp_block;
                        sp_sz = nstl::min(is - isp, sp_block);
                    }
                } else {
                    const int spp = spb * sp_block;
                    sp_sz = nstl::min(sp - spp, sp_block);
                ...
                if (loop_order == loop_ndhwgc) {
                    if (is_is_blocking)
                        nd_iterator_step(
                                n, mb, iss, sp_amount, g, ngroups, icb, nb_ic);
                    else
                        nd_iterator_step(n, mb, idp, id, ihp, ih, spb, nb_sp, g,
                                ngroups, icb, nb_ic);
                } else if (loop_order == loop_ngcdhw) {
                    if (is_is_blocking)
                        nd_iterator_step(
                                n, mb, g, ngroups, icb, nb_ic, iss, sp_amount);
                    else
                        nd_iterator_step(n, mb, g, ngroups, icb, nb_ic, idp, id,
                                ihp, ih, spb, nb_sp);
                }
    ...
    if (is_is_blocking) {
        ...
        l++;
        loop[l].src.set(
                id_block * ih_thr * sp_thr * rnd_simd(oc_blocking_size), 1);
    ...
    const auto src_op = static_cast<dim_t>(mb_thr) * id_thr
            * (is_is_blocking ? 1 : ih_thr) * sp_thr * oc_blocking_size;
    const auto dst_op = static_cast<dim_t>(mb_thr) * nsimd_ic_thr * id_thr
            * (is_is_blocking ? 1 : ih_thr) * sp_thr;
    ...
    const auto up_sp_size = is_is_blocking ? 1 : id * ih;
...
status_t init_jcp(jit_brgemm_conv_conf_t &jcp, cpu_isa_t isa,
    ...
    // TODO implement these optimizations after initial will pass correctness
    jcp.nthr = nthreads;
    jcp.kh_sets = 1;
    jcp.kw_sets = 1;
...
status_t init_conf(jit_brgemm_conv_conf_t &jcp, cpu_isa_t isa,
    ...
    if (try_exec_trans) {
        ...
        jcp.is_oc_padded = one_of(jcp.wei_dt, bf16, s8)
                && jcp.oc * jcp.kw_sets > oc_padded_block;

        if (is_amx(isa)
                && (/* TODO adapt this heuristic to bwd_d */ jcp.kw_sets == 1
                        && jcp.iw < 256)) {
            jcp.use_M_mask = 0;
            ...
            const auto A_ds
                    = jcp.src_dsz * bd_blocking * jcp.oc * jcp.kd * jcp.kh;
            const auto B_ds = jcp.wei_dsz * ld_blocking * jcp.oc * jcp.kd
                    * jcp.kh * jcp.kw;
    ...
    if (jcp.exec_type == exec_trans) {
        jcp.inp_buffer_size = rnd_up(jcp.odp * jcp.ohp * jcp.owp * jcp.ngroups
                        * jcp.nb_oc * jcp.oc_block * jcp.kh_sets * jcp.kw_sets,
                P4K);
        jcp.inp_buffer_mask_size = rnd_up(static_cast<dim_t>(jcp.nb_id)
                        * jcp.nb_ih * jcp.nb_iw * jcp.ngroups * jcp.nb_oc,
                P4K);
    }
    ...
    jcp.comp_a_buffer_size = jcp.ngroups * jcp.nb_ic * jcp.ker_ranges_size
            * jcp.iw * jcp.ic_block;
