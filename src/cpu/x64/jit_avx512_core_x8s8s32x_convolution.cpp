...
status_t jit_avx512_core_x8s8s32x_convolution_fwd_t::execute_forward_1d(
    ...
    int32_t *compensation = (apply_signed_compensation(jcp))
    ...
    parallel(jcp.nthr, [&](const int ithr, const int nthr) {
        ...
        while (start < end) {
            ...
            p.bias = bias ? bias + (bias_d.blk_off(g_oc) * bia_dt_size)
                          : nullptr;
            p.compensation = (apply_signed_compensation(jcp)) ? compensation + g_oc : nullptr;
            p.zp_compensation
        ...
        while (start < end) {
            for (int occ1 = 0; occ1 < jcp.nb_oc_blocking_thr_chunk;
                ...
                int32_t *compensation_w
                        = (apply_signed_compensation(jcp)) ? compensation + g_oc : nullptr;
                ...
                for (int oj = oh_s, ij = ih_s; oj < oh_e;
                    ...
                    size_t wei_stride = (apply_signed_compensation(jcp) || jcp.src_zero_point)
                            ? 0
    ...
    parallel_nd(jcp.mb, jcp.oh, jcp.nb_ow, nb_groups,
            [&](dim_t n, dim_t oh_s, dim_t owb, dim_t gg) {
                ...
                auto bias_w = bias ? bias + (bias_d.blk_off(g) * bia_dt_size)
                                   : nullptr;
                int32_t *compensation_w
                        = apply_signed_compensation(jcp) ? compensation + g : nullptr;
