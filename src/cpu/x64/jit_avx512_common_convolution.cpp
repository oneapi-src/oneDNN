...
inline void jit_conv_ker_pipeline_ow_thr(const jit_conv_ker_t ker, jit_conv_call_s &p,
        const void *src, const void *dst, const void *filt, const void *bias,
...
void jit_avx512_common_convolution_fwd_t<src_type, wei_type,
    ...
    parallel(nthr, [&](const int ithr, const int nthr) {
        ...
        jit_conv_ker_pipeline_ow_thr(
                jit_ker, par_conv, src, dst, weights, bias, 0, 0, 0, 0, 0, 0, 0);
    });
}
    ...
    parallel(nthr, [&](const int ithr, const int nthr) {
        ...
        for (int ocb_l2 = 0; ocb_l2 < jcp.nb_oc; ocb_l2 += jcp.nb_oc_L2) {
            ...
            while (start < end) {
                ...
                int ic_off = ic_off_idx * (is_dsrc_layout_nxc ? 1 : jcp.ic_block) * sizeof(diff_src_data_t);
