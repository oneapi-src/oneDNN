...
status_t reference_inner_product<prec>::execute_forward() {
    ...
    const memory_desc_wrapper
        src_d(this->_ippd.src_primitive_desc.memory_desc),
...
status_t reference_inner_product<prec>::primitive_desc_init(
    ...
    if (ip_d.src_desc.format == any) {
        if (ip_d.src_desc.tensor_desc.ndims_spatial == 2) {
            CHECK(mkl_dnn_memory_desc_init(&ip_d.src_desc,
                    &ip_d.src_desc.tensor_desc, f32, nchw));
        } else {
            CHECK(mkl_dnn_memory_desc_init(&ip_d.src_desc,
                    &ip_d.src_desc.tensor_desc, f32, nc));
        }
    }
    ...
    if (ip_d.dst_desc.format == any)
        CHECK(mkl_dnn_memory_desc_init(&ip_d.dst_desc,
                    &ip_d.dst_desc.tensor_desc, f32, nc));
