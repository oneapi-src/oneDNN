...
#ifndef INTERFACE_PARTITION_IMPL_HPP
...
public:
    ...
    ///     same memory buffer
    compiled_partition_impl_t(const impl::engine_t &engine,
            const std::vector<impl::logical_tensor_t> &inputs,
            const std::vector<impl::logical_tensor_t> &outputs,
            const std::vector<impl::inplace_pair_t> &inplace_pairs)
        : engine_(engine)
        , inputs_(inputs)
        , outputs_(outputs)
    ...
    virtual impl::status_t execute_sycl(const impl::stream_t *astream,
            const std::vector<impl::tensor_t> &inputs,
            const std::vector<impl::tensor_t> &outputs,
            const cl::sycl::event *sycl_event)
            = 0;
#endif
