...
static inline status_t insert_reorder_before(op_ptr &op, size_t offset,
...
static inline status_t insert_reorder_after(op_ptr &op, size_t offset,
        const dnnl::memory::desc &opt_mdesc, const dnnl::engine &p_engine,
        fusion_info_mgr_t &mgr, pd_cache_t &pd_cache,
        std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_conv(op_ptr &op,
...
static status_t layout_propagation_for_deconv(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_deconv_bwd_data(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    // always create pd using any format
...
static status_t layout_propagation_for_deconv_bwd_weights(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_eltwise(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_eltwise_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_concat(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    auto pd = create_concat_pd(op, p_engine, mgr, pd_cache).first;
...
static status_t layout_propagation_for_shuffle(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_pool(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    ...
    if (op->has_attr(op_attr::is_training)
        ...
        workspace_val->set_data_type(
                static_cast<data_type_t>(ws_md.data.data_type));
        status = fill_layout_info(workspace_val, ws_md);
    }
    return status;
}

static status_t layout_propagation_for_pool_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_batchnorm(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_batchnorm_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_prelu(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_prelu_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_layernorm(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_layernorm_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_permute(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_to_group(op_ptr &op) {
    status_t status = status::success;
    std::shared_ptr<value_t> src, dst;
    src = op->get_input_value(0);
    dst = op->get_output_value(0);
...
static status_t layout_propagation_for_from_group(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_reshape(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    std::shared_ptr<value_t> src, dst;
...
static status_t layout_propagation_for_transpose(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    std::shared_ptr<value_t> src, dst;
...
static status_t layout_propagation_for_expand(op_ptr &op) {
    status_t status = status::success;
    value_ptr src = op->get_input_value(0);
    value_ptr dst = op->get_output_value(0);
    auto in_lt = src->get_logical_tensor();
...
static status_t layout_propagation_for_squeeze(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    std::shared_ptr<value_t> src, dst;
    ...
    if (!ltw(in_lt).is_any() && ltw(out_lt).is_any()) {
        ...
        for (size_t i = 0; i < static_cast<size_t>(blocking_desc.inner_nblks);
                ++i) {
            if (blocking_desc.inner_idxs[i]
                    == static_cast<int64_t>(in_ndim) + axes.back()) {
                assertm(false,
                        "squeeze failed since there is padding in the squeezed "
                        "dimension");
            }
...
static status_t layout_propagation_for_reorder(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache) {
    status_t status = status::success;
    std::shared_ptr<value_t> src, dst;
    ...
    const auto &pd = create_reorder_pd(op, p_engine, mgr, pd_cache).first;
    auto scratchpad_val = op->get_output_value(1);
    const memory::desc scratchpad_desc = pd.scratchpad_desc();
    status = fill_layout_info(scratchpad_val, scratchpad_desc);
    return status;
...
static status_t layout_propagation_for_mul_scales(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache) {
    return layout_propagation_for_reorder(op, p_engine, mgr, pd_cache);
}

static status_t layout_propagation_for_bn_folding(
...
static status_t layout_propagation_for_conv_bwd_data(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_conv_bwd_weights(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_resampling(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_resampling_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    const auto &pd_flag_pair
...
static status_t layout_propagation_for_dnnl_sum(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    value_ptr dst = op->get_output_value(0);
    ...
    auto pd = create_dnnl_sum_pd(op, p_engine, mgr, pd_cache).first;
...
static status_t layout_propagation_for_softmax(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    value_ptr src = op->get_input_value(0);
...
static status_t layout_propagation_for_softmax_bwd(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
...
static status_t layout_propagation_for_reduction(op_ptr &op,
        const dnnl::engine &p_engine, fusion_info_mgr_t &mgr,
        pd_cache_t &pd_cache, std::vector<op_ptr> &reorder_ops) {
    status_t status = status::success;
    value_ptr src = op->get_input_value(0);
