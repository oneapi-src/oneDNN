...
using pd_cache_t = std::unordered_map<op_t *, impl::graph::utils::any_t>;
...
void insert_op_before(std::shared_ptr<op_t> &inserted_op,
        std::shared_ptr<op_t> &base_op, size_t offset);
...
void insert_op_after(std::shared_ptr<op_t> &inserted_op,
        std::shared_ptr<op_t> &base_op, size_t offset);
...
inline const std::map<op_kind_t, dnnl::algorithm> &get_eltwise_alg_map() {
    static const std::map<op_kind_t, dnnl::algorithm> &eltwise_alg_map = {
            ...
            {graph::op_kind::Pow, dnnl::algorithm::eltwise_pow}};
...
inline dnnl::algorithm get_eltwise_bwd_alg(op_kind_t kind, bool use_dst) {
    ...
    switch (kind) {
        case graph::op_kind::AbsBackprop: return algo::eltwise_abs;
        case graph::op_kind::ClampBackprop:
            if (use_dst) return algo::eltwise_clip_v2_use_dst_for_bwd;
            return algo::eltwise_clip_v2;
        case graph::op_kind::EluBackprop:
            if (use_dst) return algo::eltwise_elu_use_dst_for_bwd;
            return algo::eltwise_elu;
        case graph::op_kind::GELUBackprop: return algo::eltwise_gelu_erf;
        case graph::op_kind::HardSwishBackprop: return algo::eltwise_hardswish;
        case graph::op_kind::MishBackprop: return algo::eltwise_mish;
        case graph::op_kind::ReLUBackprop:
            if (use_dst) return algo::eltwise_relu_use_dst_for_bwd;
            return algo::eltwise_relu;
        case graph::op_kind::SigmoidBackprop:
            if (use_dst) return algo::eltwise_logistic_use_dst_for_bwd;
            return algo::eltwise_logistic;
        case graph::op_kind::SqrtBackprop:
            if (use_dst) return algo::eltwise_sqrt_use_dst_for_bwd;
            return algo::eltwise_sqrt;
        case graph::op_kind::TanhBackprop:
...
inline bool is_eltwise_kind(op_kind_t kind) {
    const std::set<op_kind_t> eltwise_kinds {graph::op_kind::Abs,
            ...
            graph::op_kind::Mish, graph::op_kind::ReLU, graph::op_kind::Round,
            graph::op_kind::Sigmoid, graph::op_kind::SoftPlus,
...
inline bool is_eltwise_bwd_kind(op_kind_t kind) {
    const std::set<op_kind_t> eltwise_bwd_kinds {graph::op_kind::AbsBackprop,
            graph::op_kind::ClampBackprop, graph::op_kind::EluBackprop,
            graph::op_kind::GELUBackprop, graph::op_kind::HardSwishBackprop,
            graph::op_kind::MishBackprop, graph::op_kind::ReLUBackprop,
            graph::op_kind::SigmoidBackprop, graph::op_kind::SqrtBackprop,
            graph::op_kind::TanhBackprop};
