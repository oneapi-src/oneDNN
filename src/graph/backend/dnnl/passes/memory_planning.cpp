...
void memory_planner_t::prepare_args_for_reorder_op(
    ...
    if (op->has_attr(op_attr::with_runtime_src_zps)
        ...
        assertm(src_zps->get_logical_tensor().data_type
                        == graph::data_type::s32,
    ...
    if (op->has_attr(op_attr::with_runtime_dst_zps)
        ...
        assertm(dst_zps->get_logical_tensor().data_type
                        == graph::data_type::s32,
                "oneDNN runtime zps must be s32 type");
...
status_t memory_planner_t::assign_external_inputs_buffer(
    ...
    auto sg_ins = graph_t(subgraph).get_input_values();
...
status_t memory_planner_t::assign_external_outputs_buffer(
    ...
    for (auto &val : graph_t(subgraph).get_output_values()) {
...
status_t memory_planner_t::assign_internal_temporary_buffer(
    ...
    return topo_order_visit(graph_t(subgraph).get_output_ops(), func);
}
...
status_t memory_planner_t::prepare_execution_args_set(
    ...
    for (value_t *in : graph_t(subgraph).get_input_values()) {
    ...
    ret = topo_order_visit(graph_t(subgraph).get_output_ops(), [&](op_t *op) {
        if (op->get_kind() == op_kind::dnnl_convolution
            ...
            const bool is_training = op->has_attr(op_attr::is_training)
                    ...
                    : false;
            prepare_args_for_siso_op(op, p_engine, mgr, true, is_training);
            ...
            const bool is_training = op->has_attr(op_attr::is_training)
                    ? op->get_attr<bool>(op_attr::is_training)
                    : false;
            prepare_args_for_dnnl_pool(op, p_engine, mgr, true, is_training);
            ...
            prepare_args_for_miso_op(op, p_engine, mgr);
                ...
                || op->get_kind() == graph::op_kind::StaticReshape
                || op->get_kind() == graph::op_kind::StaticTranspose) {
            ...
            bind_memory_for_conv_bwd_data(op, p_engine, mgr);
                ...
                || op->get_kind() == op_kind::dnnl_convtranspose_bwd_weights) {
