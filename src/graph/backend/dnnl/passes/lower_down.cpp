...
static bool need_attr_adjustment(op_kind_t kind) {
    static const std::set<op_kind_t> ops {graph::op_kind::SquaredDifference};
...
status_t split_squared_difference(std::shared_ptr<subgraph_t> &sg) {
...
status_t split_quant_dequant(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();

    std::vector<op_ptr> q_dq_ops;
    for (auto &cur_op : subgraph) {
        if (cur_op->get_kind() == graph::op_kind::Quantize
                || cur_op->get_kind() == graph::op_kind::Dequantize) {
    ...
    for (auto &cur_op : q_dq_ops) {
        ...
        op_ptr op1, op2;
        if (cur_op->get_kind() == graph::op_kind::Dequantize) {
            // f32 = scales * (int8 - zps)
...
status_t replace_quant_dequant_with_mul_scales(
    ...
    for (auto &cur_op : q_dq_ops) {
        ...
        if (graph::op_kind::Dequantize == cur_op->get_kind()) {
...
status_t folding_mul_scales(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();
...
status_t fuse_to_int8_concat(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &concat_op : fusion_ops) {
        ...
        for (size_t i = 0; i < concat_op->num_inputs(); ++i) {
            ...
            assertm(dq_op.get_kind() == graph::op_kind::Dequantize,
        ...
        assertm(q_op.get_kind() == graph::op_kind::Quantize,
...
status_t fuse_zero_points(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();
    auto &mgr = sg->fusion_info_mgr_;
...
status_t fuse_mul_scales_add_zps(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();
    std::vector<std::pair<op_t *, op_t *>> fuse_groups;
    std::set<op_t *> visited;
    for (const auto &cur_op : subgraph) {
...
status_t conv_bwd_data_canonicalization(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &cur_op : subgraph) {
        ...
        if (need_permute_0) {
            // input permute
            op_ptr in_perm_op = std::make_shared<op_t>(op_kind::permute);
            ...
            // output permute
            op_ptr out_perm_op = std::make_shared<op_t>(op_kind::permute);
            ...
            if (cur_op->has_attr(op_attr::output_shape)) {
                auto nxc_dst_shape
                        = cur_op->get_attr<dims>(op_attr::output_shape);
                auto ncx_dst_shape = canonicalize(nxc_dst_shape, "NXC");
                cur_op->set_attr<dims>(op_attr::output_shape, ncx_dst_shape);
            }
        }
...
status_t conv_bwd_weights_canonicalization(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &cur_op : subgraph) {
        ...
        if (need_permute_0) {
            // input permute
            op_ptr in0_perm_op = std::make_shared<op_t>(op_kind::permute);
            ...
            op_ptr in1_perm_op = std::make_shared<op_t>(op_kind::permute);
        ...
        if (need_permute_1) {
            ...
            cur_op->set_attr<dims>(op_attr::filter_shape, filter_shape_as_oix);
            cur_op->set_attr<std::string>(op_attr::filter_format, "OIX");
        }
        ...
        auto groups = cur_op->get_attr<int64_t>(op_attr::groups);
        if (groups > 1) {
            op_ptr from_group_op = std::make_shared<op_t>(op_kind::from_group);
...
status_t pool_fwd_canonicalization(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &cur_op : subgraph) {
        ...
        if (need_permute) {
            ...
            // dst permute
            op_ptr out0_perm_op = std::make_shared<op_t>(op_kind::permute);
    ...
    for (auto &cur_op : subgraph) {
        ...
        if (need_permute) {
            ...
            if (cur_op->get_attr<std::string>(op_attr::kind) == "maxpool") {
                op_ptr src_perm_op = std::make_shared<op_t>(op_kind::permute);
            ...
            if (cur_op->has_attr(op_attr::input_shape)) {
                auto nxc_dst_shape
                        = cur_op->get_attr<dims>(op_attr::input_shape);
                auto ncx_dst_shape = canonicalize(nxc_dst_shape, "NXC");
                cur_op->set_attr<dims>(op_attr::input_shape, ncx_dst_shape);
            }
        }
    }
...
status_t fuse_post_typecast_to_matmul_or_conv(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();
    const std::set<op_kind_t> post_ops_kinds {graph::op_kind::GELU};
    std::vector<std::vector<op_t *>> fusion_groups;
    for (const auto &cur_op : subgraph) {
        if (cur_op->get_kind() != op_kind::dnnl_matmul
                && cur_op->get_kind() != op_kind::dnnl_convolution)
        ...
        if (post_ops_kinds.count(next_op.get_kind())) {
            ...
            post_out->set_data_type(graph::data_type::f32);
...
status_t fuse_post_typecast_to_softmax(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();
    std::vector<std::vector<op_t *>> fusion_groups;
    for (const auto &cur_op : subgraph) {
...
status_t batchnorm_bwd_canonicalization(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &cur_op : subgraph) {
        ...
        if (need_permute) {
            // input0 permute
            op_ptr in_perm_op_0 = std::make_shared<op_t>(op_kind::permute);
            ...
            // input1 permute
            op_ptr in_perm_op_1 = std::make_shared<op_t>(op_kind::permute);
...
status_t split_dynamic_quant(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();

    std::vector<op_ptr> q_ops;
    for (auto &cur_op : subgraph) {
        if (cur_op->get_kind() == graph::op_kind::DynamicQuantize) {
...
status_t split_dynamic_dequant(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();

    std::vector<op_ptr> dq_ops;
    for (auto &cur_op : subgraph) {
        if (cur_op->get_kind() == graph::op_kind::DynamicDequantize) {
...
status_t lower_down(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &cur_op : subgraph) {
        ...
        if (cur_op->get_kind() == graph::op_kind::Convolution) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_convolution);
        } else if (cur_op->get_kind() == graph::op_kind::ConvTranspose) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_convtranspose);
        } else if (cur_op->get_kind()
                == graph::op_kind::ConvTransposeBackpropData) {
            new_op = std::make_shared<op_t>(
                    op_kind::dnnl_convtranspose_bwd_data);
        } else if (cur_op->get_kind()
                == graph::op_kind::ConvTransposeBackpropFilters) {
            new_op = std::make_shared<op_t>(
                    op_kind::dnnl_convtranspose_bwd_weights);
        } else if (cur_op->get_kind()
                == graph::op_kind::ConvolutionBackpropData) {
        ...
        } else if (cur_op->get_kind() == graph::op_kind::MaxPool
                || cur_op->get_kind() == graph::op_kind::AvgPool) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_pool);
            if (cur_op->get_kind() == graph::op_kind::MaxPool) {
                new_op->set_attr<std::string>(op_attr::kind, "maxpool");
            } else {
                new_op->set_attr<std::string>(op_attr::kind, "avgpool");
            }
        } else if (cur_op->get_kind() == graph::op_kind::AvgPoolBackprop) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_pool_bwd);
            new_op->set_attr<std::string>(op_attr::kind, "avgpool");
        } else if (cur_op->get_kind() == graph::op_kind::SoftMaxBackprop) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_softmax_bwd);
        } else if (cur_op->get_kind() == graph::op_kind::LogSoftmaxBackprop) {
        ...
        } else if (cur_op->get_kind() == graph::op_kind::BiasAdd) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_binary);
            new_op->set_attr<int64_t>(op_attr::alg_kind,
        ...
        } else if (cur_op->get_kind() == graph::op_kind::BatchNormInference
                || cur_op->get_kind()
            ...
            if (cur_op->get_kind() == graph::op_kind::BatchNormInference)
            ...
            if (cur_op->get_kind() == graph::op_kind::ReduceL1)
                new_op->set_attr<float>(op_attr::p, 1.0f);
            else if (cur_op->get_kind() == graph::op_kind::ReduceL2)
                new_op->set_attr<float>(op_attr::p, 2.0f);
        } else if (cur_op->get_kind() == graph::op_kind::Interpolate) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_resampling);
        } else if (cur_op->get_kind() == graph::op_kind::InterpolateBackprop) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_resampling_bwd);
        } else if (cur_op->get_kind() == graph::op_kind::Concat) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_concat);
        } else if (cur_op->get_kind() == graph::op_kind::LayerNormBackprop) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_layernorm_bwd);
        } else if (cur_op->get_kind() == graph::op_kind::MatMul) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_matmul);
        } else if (cur_op->get_kind() == graph::op_kind::SoftMax) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_softmax);
        } else if (cur_op->get_kind() == graph::op_kind::LogSoftmax) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_logsoftmax);
        } else if (cur_op->get_kind() == graph::op_kind::LayerNorm) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_layernorm);
        } else if (cur_op->get_kind() == graph::op_kind::Reorder) {
        ...
        } else if (cur_op->get_kind() == graph::op_kind::TypeCast) {
            new_op = std::make_shared<op_t>(op_kind::dnnl_reorder);
            new_op->set_attr<bool>(op_attr::change_layout, false);
            insert_scratchpad = true;
        } else if (cur_op->get_kind() == graph::op_kind::Reciprocal) {
...
status_t remove_unnecessary_quant_dequant(std::shared_ptr<subgraph_t> &sg) {
    auto &subgraph = sg->get_mutable_ops();
    std::vector<op_ptr> pool_ops;
    for (auto &cur_op : subgraph) {
        if (cur_op->get_kind() == op_kind::dnnl_pool) {
    ...
    for (auto &pool_op : pool_ops) {
        ...
        const bool pool_with_post_ops
                = quant_op.get_kind() != graph::op_kind::Quantize;
        if (pool_with_post_ops) continue;

        op_t &dequant_op = pool_in_val->get_producer();
        assertm(dequant_op.get_kind() == graph::op_kind::Dequantize,
