...
static status_t select_handler(
    ...
    // For now, as primitive doesn't support broadcast for cond input, we use
    // binary select primitive for non-broadcast case only.
    auto require_broadcast_func = [](const std::shared_ptr<op_t> &op) {
        auto in_vals = op->get_input_values();

        const dims input0_dims
                = logical_tensor_wrapper_t(in_vals[0]->get_logical_tensor())
                          .vdims();
        const dims input1_dims
                = logical_tensor_wrapper_t(in_vals[1]->get_logical_tensor())
                          .vdims();

        const size_t input0_ndims
                = logical_tensor_wrapper_t(in_vals[0]->get_logical_tensor())
                          .ndims();
        const size_t input1_ndims
                = logical_tensor_wrapper_t(in_vals[1]->get_logical_tensor())
                          .ndims();

        if (!(input0_ndims == input1_ndims)) { return true; }
        const size_t min_ndims = std::min(input0_ndims, input1_ndims);

        // For bianry-select primitive, the select_other_input should have the
        // same shape as cond input, this is the requirement of primitive
        // creation.
        for (size_t i = 0; i < min_ndims; i++) {
            if (!(input0_dims[i] == input1_dims[i])) { return true; }
        }
        return false;
    };
    const bool require_broadcast = require_broadcast_func(op);
