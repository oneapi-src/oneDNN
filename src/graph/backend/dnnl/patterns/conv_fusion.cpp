...
DNNL_BACKEND_REGISTER_TRANSFORMATION_PATTERN(dnnl, conv_depthwise_fusion_cpu)
        ...
        .set_kind(partition_kind::convolution_post_ops)
...
DNNL_BACKEND_REGISTER_TRANSFORMATION_PATTERN(
        ...
        .set_priority(10.5f)
        .set_engine_kind(engine_kind::cpu)
        .set_kind(partition_kind::quantized_convolution_post_ops)
        .set_attr<FCreateV2Pattern>("FCreateV2Pattern",
                [](const std::shared_ptr<pb_graph_t> &pgraph) -> void {
...
DNNL_BACKEND_REGISTER_TRANSFORMATION_PATTERN(dnnl, conv_simple_resblock_fusion)
        ...
        .set_kind(partition_kind::residual_conv_blocks)
        .set_attr<FCreateV2Pattern>("FCreateV2Pattern",
                [](const std::shared_ptr<pb_graph_t> &pgraph) -> void {
                    pm::pb_op_t *conv0
                            = pgraph->append_op(graph::op_kind::Convolution);
                    conv0->append_decision_function(check_input_num<2>);
                    pm::pb_op_t *relu0 = pgraph->append_op(graph::op_kind::ReLU,
                    ...
                    pm::pb_op_t *relu1 = pgraph->append_op(graph::op_kind::ReLU,
                    ...
                    pgraph->append_op(graph::op_kind::ReLU,
...
DNNL_BACKEND_REGISTER_TRANSFORMATION_PATTERN(
        ...
        .set_priority(5.f) // increase the priority if needed
        .set_kind(partition_kind::quantized_residual_conv_blocks)
        .set_attr<FCreateV2Pattern>("FCreateV2Pattern",
                [](const std::shared_ptr<pb_graph_t> &pgraph) -> void {
                    pm::pb_op_t *quant_dst0
        ...
        .set_attr<FCreateV2Pattern>("FCreateV2Pattern",
                [](const std::shared_ptr<pb_graph_t> &pgraph) -> void {
                    ...
                    auto psigmoid = swish_graph->append_op(
                            graph::op_kind::Sigmoid, "psigmoid");
                    auto pmultiply
                            = swish_graph->append_op(graph::op_kind::Multiply,
...
DNNL_BACKEND_REGISTER_TRANSFORMATION_PATTERN(
        ...
        .set_enable(false)
        .set_kind(partition_kind::convolution_backprop_post_ops)
        .set_priority(9.7f)
        .set_attr<FCreateV2Pattern>("FCreateV2Pattern",
                [](const std::shared_ptr<pb_graph_t> &pgraph) -> void {
                    ...
                    pgraph->append_op(
                            graph::op_kind::ConvolutionBackpropFilters,
                            in_edges_t {in_edge(1, wildcard, 0)});
                    pgraph->append_op(graph::op_kind::BiasAddBackprop,
