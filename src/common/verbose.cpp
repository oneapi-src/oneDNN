...
int get_verbose() {
    ...
    const int len = 2;
    char val[len] = {0};
    static int get_mkldnn_verbose_val = getenv("MKLDNN_VERBOSE", val, len);
    static int get_dnnl_verbose_val = getenv("DNNL_VERBOSE", val, len);
    if (get_mkldnn_verbose_val == 1) verbose.set(atoi(val), true);
    if (get_dnnl_verbose_val == 1) verbose.set(atoi(val), true);
