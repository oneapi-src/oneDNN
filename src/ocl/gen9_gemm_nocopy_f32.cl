...
#define DO_FMA_TN(hh, i, i_mod_4, i_div_4) do { \
...
#define DO_FMA_TT(hh, i, i_mod_4, i_div_4) do { \
    c[i_div_4][0].s ## i_mod_4 = mad(sub_group_broadcast(a.s ## hh, i), \
...
#define UPDATE_C_ROW_2X(i, ii, betaZero) do { \
    if (irem > i) { \
        if (jrem > 0) { \
            float val = alpha * c[i/4][0].s ## ii + ((betaZero) ? 0 : beta * *(C_ptrs[0])); \
            POST_OP(val); \
            *(C_ptrs[0]) = val; \
        } \
        if (jrem > 16) { \
            float val = alpha * c[i/4][1].s ## ii + ((betaZero) ? 0 : beta * *(C_ptrs[1])); \
            POST_OP(val); \
            *(C_ptrs[1]) = val; \
        } \
    } \
...
#define UPDATE_C_16_ROW(betaZero) do { \
    ...
    int last_k_block, float eltwise_alpha, float eltwise_beta)
{
    ...
    for (int h = 0; h < (k >> 2); h++) {
        // Load A
        for (int j = 0; j < 4; j++) {
            a[j] = as_float2(intel_sub_group_block_read2(
                (global uint *) A_cols[j]));
            A_cols[j] += ldax4;
        }
    ...
    int last_k_block, float eltwise_alpha, float eltwise_beta)
{
    ...
    if (krem > 0) {
        // Load A
        a[0] = as_float2(intel_sub_group_block_read2(
            (global uint *) A_cols[0]));
