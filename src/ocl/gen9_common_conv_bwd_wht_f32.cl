...
#        define MULTIPLY_BLOCKS_8x8(_result, _blockA, _blockB, col)         \
            {                                                               \
                _result = FMA8(                                             \
                        _blockB.s0, TRANSPOSE_8(_blockA, 0, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s1, TRANSPOSE_8(_blockA, 1, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s2, TRANSPOSE_8(_blockA, 2, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s3, TRANSPOSE_8(_blockA, 3, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s4, TRANSPOSE_8(_blockA, 4, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s5, TRANSPOSE_8(_blockA, 5, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s6, TRANSPOSE_8(_blockA, 6, col), _result); \
                _result = FMA8(                                             \
                        _blockB.s7, TRANSPOSE_8(_blockA, 7, col), _result); \
            }
...
#    if VER_REF == 1
    const uint kw = get_global_id(0);
    const uint kdh = get_global_id(1);
    const uint kd = kdh / KH;
    const uint kh = kdh % KH;

    const uint g_ic_oc = get_global_id(2);
    const uint g = g_ic_oc / (OC * IC);
    const uint io = g_ic_oc % (OC * IC);
            ...
            for (int od = 0; od < OD; ++od)
                for (int oh = 0; oh < OH; ++oh)
                    for (int ow = 0; ow < OW; ++ow) {
                        const uint diff_dst_off = mb * G * OC * OD * OH * OW
                                + oc * OD * OH * OW + od * OH * OW + oh * OW
                                + ow;
                        d += diff_dst[diff_dst_off];
                    }
        diff_bias[oc] = d;
    }
        ...
        for (int od = 0; od < OD; ++od)
            for (int oh = 0; oh < OH; ++oh)
                for (int ow = 0; ow < OW; ++ow) {
                    ...
                    const uint diff_dst_off = mb * G * OC * OD * OH * OW
                            + oc * OD * OH * OW + od * OH * OW + oh * OW + ow;
                    const uint src_off = mb * G * IC * ID * IH * IW
                            + ic * ID * IH * IW + id * IH * IW + ih * IW + iw;

                    dw += diff_dst[diff_dst_off] * src[src_off];
                }
    const uint diff_wei_off = g * IC * OC * KD * KH * KW
            + oc * IC * KD * KH * KW + ic * KD * KH * KW + kd * KH * KW
            + kh * KW + kw;
    diff_wei[diff_wei_off] = dw;
#    endif
}
#endif
