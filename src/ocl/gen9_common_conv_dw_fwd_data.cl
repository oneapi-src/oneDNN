...
#    if NCHW
        dst += mb * G * OD * OH * OW + g * OD * OH * OW + osp;
        src += mb * G * ID * IH * IW + g * ID * IH * IW + id * IH * IW + ih * IW
                + iw;
#    endif
#    if NHWC
        dst += mb * G * OD * OH * OW + g + osp * G;
        src += mb * G * ID * IH * IW + g + (id * IH * IW + ih * IW + iw) * G;
#    endif
...
#    if NCHW
                        sum += src[kd * IH * IW + kh * IW + kw]
                                * wei[kd * KH * KW + kh * KW + kw];
#    endif
#    if NHWC
                        sum += src[kd * IH * IW * G + kh * IW * G + kw * G]
                                * wei[kd * KH * KW + kh * KW + kw];
#    endif
                    }
                }
...
#        if SUM_SCALE == 1
        sum += dst[0];
#        else
        sum = fma(dst[0], (DATA_T)sum_scale, sum);
#        endif
        if (sum < 0)
