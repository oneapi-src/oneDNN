...
gen9_common_conv_dw_fwd_kernel(const __global DATA_T *src,
    ...
    __attribute__((opencl_unroll_hint(OW_BLOCK))) for (int k = 0; k < OW_BLOCK;
                                                       k++) {
        S00[k] = B;
    }
#else
    ...
    const __global DATA_T *src1 = src;
                ...
                __attribute__((opencl_unroll_hint(OW_BLOCK))) for (int k = 0;
                                                                   k < OW_BLOCK;
                                                                   k++) {
    ...
    DATA8_T S00, S01;
    DATA_T B = AS_DATA_T(BLOCK_READ((const __global uint *)&bias[g]));
    __attribute__((opencl_unroll_hint(OW_BLOCK))) for (int k = 0; k < 8; k++) {
        S00[k] = B;
        S01[k] = B;
    }
#else
