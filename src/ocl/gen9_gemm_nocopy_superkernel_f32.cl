...
#define DO_FMA_NN(hh, i_mod_16, i_div_16, i_mod_4, i_div_4) do { \
    c[i_div_4].s ## i_mod_4 = mad(sub_group_broadcast(a[hh].s ## i_div_16, \
            i_mod_16), b.s ## hh, c[i_div_4].s ## i_mod_4); \
} while (0)

#define DO_FMA_NT(hh, i_mod_16, i_div_16, i_mod_4, i_div_4) do { \
    c[i_div_4].s ## i_mod_4 = mad(sub_group_broadcast(a[hh].s ## i_div_16, \
            i_mod_16), b[hh], c[i_div_4].s ## i_mod_4); \
} while (0)
...
#define FMA_I_LOOP_32_ROW(hh) do { \
        DO_FMA(hh, 0, 0, 0, 0); \
        DO_FMA(hh, 1, 0, 1, 0); \
        DO_FMA(hh, 2, 0, 2, 0); \
        DO_FMA(hh, 3, 0, 3, 0); \
...
#define FMA_I_LOOP_16_ROW(hh) do { \
        DO_FMA(hh, 0, 0, 0, 0); \
        DO_FMA(hh, 1, 0, 1, 0); \
        DO_FMA(hh, 2, 0, 2, 0); \
        DO_FMA(hh, 3, 0, 3, 0); \
...
#define UPDATE_C_ROW(i, ii, betaZero) do { \
    if (jrem > 0) if (irem > i) { \
        float val = alpha * c[i/4].s ## ii + ((betaZero) ? 0 : beta * *C); \
        POST_OP(val); \
        *C = val; \
    } \
    C++; \
...
#define UPDATE_C_32_ROW(betaZero) do { \
...
kernel void gen9_gemm_nocopy_superkernel_f32(global int *plan, int threads,
        ...
        float eltwise_beta)
{
    ...
    if (kid0 == 0) {
        ...
        for (int h = 0; h < (k >> 2); h++) {
            // Load A
            for (int j = 0; j < 4; j++) {
                a[j] = as_float2(intel_sub_group_block_read2((global uint *)
                        A_cols[j]));
                A_cols[j] += ldax4;
            }
        ...
        for (int h = 0; h < (k >> 2); h++) {
            for (int j = 0; j < 4; j++) {
                a[j].s0 = as_float(intel_sub_group_block_read((global uint *)
                        A_cols[j]));
                A_cols[j] += ldax4;
            }
        ...
        float eltwise_beta)
{
    ...
    if (kid0 == 0) {
        ...
        for (int h = 0; h < (k >> 1); h++) {
            ...
            // Load B
            for (int i = 0; i < 2; i++) {
                b[i] = as_float(intel_sub_group_block_read((global uint *)
                        B_rows[i]));
                B_rows[i] += ldbx2;
            }
        ...
        int krem = k & 1;
        if (krem > 0) {
            a[0] = as_float2(intel_sub_group_block_read2((global uint *)
                    A_cols[0]));

            b[0] = as_float(intel_sub_group_block_read((global uint *)
                    B_rows[0]));
        ...
        int krem = k & 1;
        if (krem > 0) {
            a[0].s0 = as_float(intel_sub_group_block_read((global uint *)
                    A_cols[0]));
            b[0] = as_float(intel_sub_group_block_read((global uint *)
                    B_rows[0]));
