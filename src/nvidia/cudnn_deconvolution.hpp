...
#ifndef CUDNN_DECONVOLUTION_HPP
#define CUDNN_DECONVOLUTION_HPP
...
#include "nvidia/cudnn_deconvolution_impl.hpp"
...
struct cudnn_deconvolution_fwd_t : public primitive_t {
    struct pd_t : public deconvolution_fwd_pd_t {
        ...
        pd_t &operator=(const pd_t &other) {
            ...
            deconvolution_fwd_pd_t::operator=(other);
...
struct cudnn_deconvolution_bwd_data_t : public primitive_t {
    struct pd_t : public deconvolution_bwd_data_pd_t {
        ...
        pd_t &operator=(const pd_t &other) {
            DNNL_SHORT_CIRCUIT_SELF_ASSIGN(other);
            deconvolution_bwd_data_pd_t::operator=(other);
            conv_pd_.reset(other.conv_pd_->clone());
            return *this;
        }
...
struct cudnn_deconvolution_bwd_weights_t : public primitive_t {
    struct pd_t : public deconvolution_bwd_weights_pd_t {
        ...
        pd_t &operator=(const pd_t &other) {
            DNNL_SHORT_CIRCUIT_SELF_ASSIGN(other);
            deconvolution_bwd_weights_pd_t::operator=(other);
            conv_pd_.reset(other.conv_pd_->clone());
            impl_ = other.impl_;
            return *this;
        }
        ...
        status_t init(engine_t *engine) {
            ...
            if (ok) {
                ...
                init_scratchpad();
                if (with_bias()) {
                    impl_.reset(new cudnn_deconvolution_bwd_bias_impl_t());
