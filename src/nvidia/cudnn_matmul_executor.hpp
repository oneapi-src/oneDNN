...
#ifndef CUDNN_MATMUL_EXECUTOR_HPP
#define CUDNN_MATMUL_EXECUTOR_HPP
...
struct cudnn_matmul_exec_base_t {
    virtual status_t execute(const exec_ctx_t &ctx, engine_t *engine,
            const std::shared_ptr<cudnn_matmul_impl_t> matmul_impl,
...
protected:
    template <typename read_acc_t, typename write_acc_t, typename scratch_acc_t,
            typename bias_acc_t>
    void interop_task(std::shared_ptr<cudnn_matmul_impl_t> matmul_impl,
            engine_t *engine, cl::sycl::handler &cgh,
            cuda::sycl_cuda_stream_t *cuda_stream, read_acc_t weights_acc,
        ...
        cgh.interop_task([=](cl::sycl::interop_handler ih) {
            ...
            matmul_impl->execute(cublas_handle, cudnn_handle, weights, src, dst,
                    bias, scratch, output_scale);
        });
    }

    template <typename T, cl::sycl::access::mode md, typename sc_t>
    void *maybe_cast_to_ptr(cl::sycl::accessor<T, 1, md> acc, sc_t &sc,
            cl::sycl::interop_handler &ih) const {
        return sc.template memory<void *>(ih, acc);
    }

    template <typename sc_t>
    std::nullptr_t maybe_cast_to_ptr(
            std::nullptr_t acc, sc_t &, cl::sycl::interop_handler &ih) const {
        return acc;
    }
};
...
struct cudnn_matmul_scratch_runtime_args_bias_exec_t
    ...
    virtual status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([=](cl::sycl::handler &cgh) {
            ...
            interop_task(matmul_impl, engine, cgh, cuda_stream, wt_acc, src_acc,
                    dst_acc, bias_acc, scratch_acc, output_scale);
        });
    }
};
...
struct cudnn_matmul_runtime_args_scratch_exec_t
    ...
    virtual status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([=](cl::sycl::handler &cgh) {
            ...
            interop_task(matmul_impl, engine, cgh, cuda_stream, wt_acc, src_acc,
                    dst_acc, nullptr, scratch_acc, output_scale);
        });
    }
};
...
struct cudnn_matmul_runtime_args_bias_exec_t : public cudnn_matmul_exec_base_t {
    virtual status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([=](cl::sycl::handler &cgh) {
            ...
            interop_task(matmul_impl, engine, cgh, cuda_stream, wt_acc, src_acc,
                    dst_acc, bias_acc, nullptr, output_scale);
        });
    }
};
...
struct cudnn_matmul_runtime_args_exec_t : public cudnn_matmul_exec_base_t {
    virtual status_t execute(const exec_ctx_t &ctx, engine_t *engine,
        ...
        return cuda_stream->interop_task([=](cl::sycl::handler &cgh) {
            ...
            interop_task(matmul_impl, engine, cgh, cuda_stream, wt_acc, src_acc,
                    dst_acc, nullptr, nullptr, output_scale);
        });
    }
};
...
#endif // CUDNN_MATMUL_EXECUTOR_HPP
