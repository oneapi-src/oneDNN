...
#include "sycl/sycl_device_selector.hpp"
...
status_t sycl_cuda_stream_t::init() {
    ...
    if (!queue_) {
        ...
        auto vendor_id = sycl_dev.get_info<cl::sycl::info::device::vendor_id>();
        auto service_stream = utils::downcast<sycl_cuda_stream_t *>(
                sycl_engine.service_stream());
        queue_.reset(
                (service_stream ? new cl::sycl::queue(service_stream->queue())
                                : new cl::sycl::queue(sycl_ctx, sycl_dev)));
    } else {
        ...
        auto service_stream = utils::downcast<sycl_cuda_stream_t *>(
                sycl_engine.service_stream());
        auto engine_streamId = service_stream->get_underlying_stream();
    ...
    cudaStream_t current_stream_id = NULL;
