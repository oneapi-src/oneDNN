...
#ifndef CUDNN_CONV_INNER_PRODUCT_IMPL_HPP
#define CUDNN_CONV_INNER_PRODUCT_IMPL_HPP
...
struct cudnn_conv_inner_product_impl_base_t
    ...
    status_t filter_tag(const memory_desc_t &md, format_tag_t &weight_tag) {
    ...
    status_t source_tag(const memory_desc_t &md, format_tag_t &src_tag) {
        using namespace format_tag;
...
struct cudnn_conv_inner_product_fwd_impl_t
    ...
    virtual status_t init(engine_t *engine, inner_product_pd_t *pd,
        ...
        // Convert DNNL data types to their cuDNN counterparts.
    ...
    virtual void execute(cudnnHandle_t handle, cublasHandle_t,
        ...
        if (with_bias_) {
            auto scaled_bias = b;
            if (scale_bias_) {
                output_scale_workspace_ = args[6];
                CUDNN_EXECUTE_FUNC(cudnnAddTensor, handle, &output_scales_,
                        tensor_descs_[io::bia], b, &beta_,
                        tensor_descs_[io::bia], output_scale_workspace_);
                scaled_bias = output_scale_workspace_;
            }
...
struct cudnn_conv_inner_product_bwd_weights_impl_t
    ...
    virtual status_t init(engine_t *engine, inner_product_pd_t *pd,
        ...
        // DNNL does not set unused dimensions and strides in the output, so we
        // do that here. If nhwc filter, then repeat the N stride for the
        // spatial dimensions.
...
#endif // CUDNN_CONV_INNER_PRODUCT_IMPL_HPP
