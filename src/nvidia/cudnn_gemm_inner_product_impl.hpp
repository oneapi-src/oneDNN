...
#ifndef CUDNN_GEMM_INNER_PRODUCT_IMPL_HPP
#define CUDNN_GEMM_INNER_PRODUCT_IMPL_HPP
...
protected:
    ...
    // if datatype is f16 or s8 and bias ia presented the compute type mist be
    ...
    status_t get_cublas_data_type(
            const cudnnDataType_t &cudnn_dt, cudaDataType_t &blas_dt) {
        switch (cudnn_dt) {
...
struct cudnn_gemm_inner_product_fwd_impl_t
    ...
    virtual bool ip_using_scratchpad() const override {
        return (use_acc_dst_ > 0);
    }
    ...
    virtual status_t init(engine_t *, inner_product_pd_t *pd, bool with_relu,
        ...
        // column major without transposing the Matices.
        // B * A =C; where B is weight and A is src, and C is dst
    ...
    virtual void execute(cudnnHandle_t cudnn_handle,
...
struct cudnn_gemm_inner_product_bwd_data_impl_t
    ...
    virtual status_t init(engine_t *, inner_product_pd_t *pd,
        ...
        // by switching the weight and source we convert the row major to
        // column major without transposing the Matices.
        // B * A =C; where B is weight and A is dy, and C is dx.
...
struct cudnn_gemm_inner_product_bwd_weights_impl_t
    ...
    virtual status_t init(engine_t *engine, inner_product_pd_t *pd,
        ...
        // column major without transposing the Matices.
        // B * A =C;
        // Here backward weight is equivalent of dy * x^T when the weight filter
        // is ICOC Therefroe B is dy and A is transposed x, and C is dw.
        // However, when the filter format is OCIC , the backward weight
        // is equivalent of X * dy^T . In this case, B is x, A is transposed dy
        // and and C is dw.
    ...
    virtual void execute(cudnnHandle_t cudnn_handle,
        ...
        if (need_reorder_) {
            ...
            // however a user requires the DNNL format as an output
...
#endif // CUDNN_GEMM_INNER_PRODUCT_IMPL_HPP
