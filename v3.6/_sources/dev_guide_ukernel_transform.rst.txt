...
The transform ukernel allows users to convert data from one format to the other, similar to what reorder primitive provides functionally.

The only output data format supported by this routine is packed format, which is required by B matrices in :ref:`BRGeMM ukernel <doxid-dev_guide_ukernel_brgemm>`. This is an out-of-place operation.
