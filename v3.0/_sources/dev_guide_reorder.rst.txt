...
.. math::
	...
	\dst(\overline{x}) = \alpha \cdot \src(\overline{x} - shift_{src}) + \beta \cdot \dst(\overline{x}) + shift_{dst}

.. note:: 

   * The intermediate operations are being done using single precision floating point data type.
   
   * :math:`\alpha` must be passed during execution runtime as a separate memory argument. Using :math:`\src` scale argument will lead to multiplication of tensor values by a scale value. Using :math:`\dst` scale argument will lead to division of tensor values by a scale value.
