...
.. note:: 
   ...
   boolean is only supported in Graph Compiler in CPU engine. No primitives support boolean during primitive computation.
...
.. note:: 
   ...
   * Boolean is only supported by the oneDNN graph API when the graph compiler backend is enabled. The graph compiler backend supports CPU engine only.
