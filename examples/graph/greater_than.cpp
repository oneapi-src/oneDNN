...
*******************************************************************************/
/// @example greater_than.cpp
/// @copybrief graph_greater_than_cpp
/// > Annotated version: @ref graph_greater_than_cpp
/// @page graph_greater_than_cpp Single op partition
/// This is an example to demonstrate how to build a simple op graph and run it.
///
/// > Example code: @ref greater_than.cpp
///
...
///
/// @page graph_greater_than_cpp
/// @section graph_greater_than_cpp_headers Public headers
///
...
///
/// @page graph_greater_than_cpp
/// @snippet greater_than.cpp Headers and namespace
...
//[Headers and namespace]
/// @page graph_greater_than_cpp
/// @section graph_greater_than_cpp_tutorial greater_than_tutorial() function
///
void greater_than(dnnl::engine::kind ekind) {
    dim M = 32, N = 32;
    dims src_dims {M, N};
    /// @page graph_greater_than_cpp
    /// @subsection graph_greater_than_cpp Build Graph and Get Partitions
    ///
    /// In this section, we are trying to create a partition containing the
    /// single op `greaterthan` without building a graph and getting partition.
    ///
    /// Create the `GreaterThan` op (#dnnl::graph::op) and attaches attributes
    //[Create greaterthan]
    logical_tensor src0 {0, data_type::f32, src_dims, layout_type::strided};
    logical_tensor src1 {1, data_type::f32, src_dims, layout_type::strided};
    logical_tensor dst {2, data_type::boolean, src_dims, layout_type::strided};
    op greaterthan(
            0, op::kind::GreaterThan, {src0, src1}, {dst}, "greaterthan");

    //[Create greaterthan]
    /// @page graph_greater_than_cpp
    /// @subsection graph_greater_than_cpp_compile Compile and Execute Partition
    ///
    ...
    ///
    /// @snippet greater_than.cpp Create engine
    //[Create engine]
    dnnl::engine eng(ekind, 0);
    //[Create engine]
    /// Create a #dnnl::stream on a given engine
    ///
    /// @snippet greater_than.cpp Create stream
    //[Create stream]
    ...
    std::unordered_map<size_t, dims> concrete_shapes {{0, src_dims}};
    ...
    ///
    /// @snippet greater_than.cpp Create partition
    //[Create partition]
    partition part(greaterthan, ekind);
    //[Create partition]
    if (!part.is_supported()) {
        std::cout << "greater_thanpartition: Got unsupported partition, "
    ...
    ///
    /// @snippet greater_than.cpp Compile partition
    //[Compile partition]
    compiled_partition cp = part.compile(inputs, outputs, eng);
    //[Compile partition]
    std::vector<float> input0_data(product(src_dims));
    std::vector<float> input1_data(product(src_dims));
    for (int i = 0; i < product(src_dims); i++) {
        input0_data[i] = i % 2 ? 0 : 1;
        input1_data[i] = i % 2 ? 1 : 0;
    }
    auto input0_ts = tensor(src0, eng);
    auto input1_ts = tensor(src1, eng);
    write_to_dnnl_tensor(input0_data.data(), input0_ts);
    write_to_dnnl_tensor(input1_data.data(), input1_ts);
    auto output_ts = tensor(dst, eng);
    /// Execute the compiled partition on the specified stream.
    ///
    /// @snippet greater_than.cpp Execute compiled partition
    //[Execute compiled partition]
    cp.execute(strm, {input0_ts, input1_ts}, {output_ts});
    //[Execute compiled partition]
    ...
    std::cout << "Graph:" << std::endl
              ...
              << std::endl;
    // bool *res = static_cast<bool *>(output_ts.get_data_handle());
    // for (int i = 0; i < M * N; i++) {
...
int main(int argc, char **argv) {
    return handle_example_errors(greater_than, parse_engine_kind(argc, argv));
}
