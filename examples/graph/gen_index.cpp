...
/// @copybrief graph_gen_index_cpp
/// > Annotated version: @ref graph_gen_index_cpp

/// @page graph_gen_index_cpp Single op partition on CPU
...
/// @page graph_gen_index_cpp
/// @section graph_gen_index_cpp_headers Public headers
///
...
/// @page graph_gen_index_cpp
/// @section graph_gen_index_cpp_tutorial cpu_single_op_partition_tutorial() function
///
void gen_index(dnnl::engine::kind ekind) {
    ...
    /// @page graph_gen_index_cpp
    /// @subsection graph_gen_index_cpp Build Graph and Get Partitions
    ///
    /// In this section, we are trying to create a partition containing the
    /// single op `genindex` without building a graph and getting partition.
    ///

    /// Create the `GenIndex` op (#dnnl::graph::op) and attaches attributes
    //[Create genindex]
    logical_tensor genindex_src {
            0, data_type::f32, src_dims, layout_type::strided};
    logical_tensor genindex_dst {
            1, data_type::s32, src_dims, layout_type::strided};
    op genindex(
            0, op::kind::GenIndex, {genindex_src}, {genindex_dst}, "genindex");
    genindex.set_attr<int64_t>(op::attr::axis, 0);

    //[Create genindex]

    /// @page graph_gen_index_cpp
    /// @subsection graph_gen_index_cpp_compile Compile and Execute Partition
    ///
    ...
    //[Create partition]
    partition part(genindex, ekind);
    //[Create partition]
    if (!part.is_supported()) {
        std::cout << "gen_indexpartition: Got unsupported partition, "
    ...
    auto input_ts = tensor(genindex_src, eng);
    auto output_ts = tensor(genindex_dst, eng);
    ...
    std::cout << "Graph:" << std::endl
              << "      [genindex_src0]" << std::endl
              << "            |       " << std::endl
              << "         genindex" << std::endl
              << "            |" << std::endl
              << "        [genindex_dst]" << std::endl
              << "Note:" << std::endl
              << " '[]' represents a logical tensor, which refers to "
                 "inputs/outputs of the graph. "
              << std::endl;

    int32_t *res = static_cast<int32_t *>(output_ts.get_data_handle());
    for (int i = 0; i < M * N; i++) {
...
int main(int argc, char **argv) {
    return handle_example_errors(gen_index, parse_engine_kind(argc, argv));
}
