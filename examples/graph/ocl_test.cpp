...
void gpu_opencl_interop_tutorial() {
    ...
    void *p = ocl_malloc_shared(N * sizeof(float), 0, dev, ctx);

    //     memory mem(mem_d, eng, nullptr);
    auto mem = dnnl::ocl_interop::make_memory(
            mem_d, eng, dnnl::ocl_interop::memory_kind::usm, nullptr);
    ...
    OCL_CHECK(usm_set_arg(ocl_init_kernel, 0, &ocl_buf));
    //     clSetKernelArgMemPointerINTEL(ocl_init_kernel, 0, &ocl_buf);
