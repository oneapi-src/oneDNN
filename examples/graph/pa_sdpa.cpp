...
void fill_random_int(std::vector<int32_t> &out, int max) {
    static std::vector<int32_t> random_data_i;
    constexpr size_t nrand = 1037;

    if (random_data_i.empty()) {
        std::mt19937 generator;
        std::uniform_int_distribution<int32_t> dist_i(0, max - 1);
    ...
    for (size_t i = 0; i < out.size(); i += nrand) {
        size_t chunk = std::min(nrand, out.size() - i);
        std::memcpy(&out[i], random_data_i.data(), chunk * sizeof(int32_t));
    }
}
...
void bench_sdpa(engine::kind ekind, logical_tensor::data_type dt,
        const pa_sdpa_dims_t &p, double time_limit = 0.) {
    ...
    auto k_cache = logical_tensor(id++, dt, kv_cache_sz, layout_type::strided);
    auto k_block_table = logical_tensor(id++, logical_tensor::data_type::s32,
            block_table_sz, layout_type::strided);
    auto key = logical_tensor(id++, dt, k_sz, layout_type::strided);

    auto k_paged_cache_load
            = op(id++, op::kind::PagedCacheLoad, "paged_cache_load");
...
void bench(engine::kind ekind, dnnl_data_type_t dt, const pa_sdpa_dims_t &p,
        double time_limit = 0.) {
    try {
        // api == api_kind::graph
...
void sdpa_perf(engine::kind ekind, int argc, char **argv) {
    ...
    if (argc > 2) {
        ...
        params.query_num = std::atoi(argv[i++]);
        params.block_num = std::atoi(argv[i++]);
        params.block_size = std::atoi(argv[i++]);

        if (params.head_num <= 0 || params.head_size <= 0
                || params.query_num <= 1 || params.block_num <= 0
                || params.block_size <= 0) {
            bad_args();
        }
    }
