...
void compute_weighted_annotations(float *weighted_annotations,
        int src_seq_length_max, int batch, int annotation_feature_size,
        int feature_size, float *weights_annot, float *annotations) {
    ...
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, feature_size,
            src_seq_length_max * batch, annotation_feature_size, 1.0f,
            weights_annot, feature_size, annotations, annotation_feature_size,
            0.0f, weighted_annotations, feature_size);
}

void compute_attention(float *context_vectors, int src_seq_length_max,
        int batch, int feature_size, int annotation_feature_size,
        float *weights_src_layer, float *dec_src_layer, float *annotations,
    ...
    std::vector<float> weighted_src_layer(batch * feature_size);
    ...
    std::vector<float> alignments(src_seq_length_max * batch);
    ...
    std::vector<float> exp_sums(batch);
    ...
    for (int i = 0; i < batch; i++)
        for (int j = 0; j < annotation_feature_size; j++)
            context_vectors[i * (feature_size + annotation_feature_size)
                    + feature_size + j]
                    = 0.0f;
        ...
        for (int i = 0; i < batch; i++)
            for (int j = 0; j < annotation_feature_size; j++)
                context_vectors[i * (feature_size + annotation_feature_size)
                        ...
                        * annotations[j
                                  + annotation_feature_size * (i + batch * k)];
}

void copy_context(float *src_iter, int n_layers, int n_states, int batch,
        int feature_size, int annotation_feature_size) {
    // we copy the context from the first layer to all other layers
    for (int k = 1; k < n_layers; k++)
        for (int j = 0; j < batch; j++)
            for (int i = 0; i < annotation_feature_size; i++)
                src_iter[(k * n_states * batch + j)
                                * (feature_size + annotation_feature_size)
                        + i]
                        = src_iter[j * (feature_size + annotation_feature_size)
                                + i];
}
...
void simple_net() {
    ...
    const int tgt_seq_length_max = 20;

    const int feature_size = 512;
    const int annotation_feature_size = 2 * feature_size;
    ...
    std::vector<float> net_src(batch * src_seq_length_max * feature_size);
    std::vector<float> net_dst(
            batch * tgt_seq_length_max * annotation_feature_size);
    ...
    memory::dims enc_bidir_dst_layer_tz
            = { src_seq_length_max, batch, annotation_feature_size };

    /* GNMT encoder: 1 bidirectional layer and 7 unidirectional layers
     */

    std::vector<float> user_enc_bidir_wei_layer(enc_bidir_n_layers * 2
            * feature_size * lstm_n_gates * feature_size);
    std::vector<float> user_enc_bidir_wei_iter(enc_bidir_n_layers * 2
            * feature_size * lstm_n_gates * feature_size);
     ...
     */
    /// memories
    std::vector<float> user_enc_uni_wei_layer(enc_unidir_n_layers * 1
            * annotation_feature_size * lstm_n_gates * annotation_feature_size);
    std::vector<float> user_enc_uni_wei_iter(enc_unidir_n_layers * 1
            * annotation_feature_size * lstm_n_gates * annotation_feature_size);
    std::vector<float> user_enc_uni_bias(
            enc_unidir_n_layers * 1 * lstm_n_gates * annotation_feature_size);
    memory::dims user_enc_uni_wei_layer_dims = { enc_unidir_n_layers, 1,
        annotation_feature_size, lstm_n_gates, annotation_feature_size };
    memory::dims user_enc_uni_wei_iter_dims = { enc_unidir_n_layers, 1,
        annotation_feature_size, lstm_n_gates, annotation_feature_size };
    ...
    rnn_forward::desc enc_uni_layer_desc(prop_kind::forward_inference,
            ...
            enc_bidir_dst_layer_md, zero_md(), user_enc_uni_wei_layer_md,
    ...
    /// @todo add a reorder when they will be available
    encoder_net.push_back(
            rnn_forward(enc_uni_prim_desc, enc_bidir_dst_layer_memory,
    ...
    std::vector<float> user_dec_wei_iter(dec_n_layers * 1
            * (feature_size + annotation_feature_size) * lstm_n_gates
            * feature_size);
    std::vector<float> user_dec_bias(
            dec_n_layers * 1 * lstm_n_gates * feature_size);
    std::vector<float> user_dec_dst(tgt_seq_length_max * batch * feature_size);
    std::vector<float> user_weights_attention_src_layer(
            feature_size * feature_size);
    std::vector<float> user_weights_annotation(
            annotation_feature_size * feature_size);
    std::vector<float> user_weights_alignments(feature_size);

    memory::dims user_dec_wei_layer_dims
            = { dec_n_layers, 1, feature_size, lstm_n_gates, feature_size };
    memory::dims user_dec_wei_iter_dims = { dec_n_layers, 1,
        feature_size + annotation_feature_size, lstm_n_gates, feature_size };
    ...
    // annotation_feature_size values. However, we do not compute or
    // access those.
    ...
    compute_weighted_annotations(weighted_annotations.data(),
            src_seq_length_max, batch, annotation_feature_size, feature_size,
