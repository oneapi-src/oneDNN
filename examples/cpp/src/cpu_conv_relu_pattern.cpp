...
/// @example cpu_conv_relu_pattern.cpp
/// @copybrief cpu_conv_relu_pattern_cpp
/// Annotated version: @ref cpu_conv_relu_pattern_cpp

/// @page cpu_conv_relu_pattern_cpp CPU example for conv+relu pattern
///
/// > Example code: @ref cpu_conv_relu_pattern.cpp
...
int main(int argc, char **argv) {
    ...
    relu0_dst_desc_any = cp1.query_logical_tensor(id_mgr["relu0_dst"]);
    std::cout << "Success!\n";
    ...
    std::vector<float> relu0_dst_data(
            cp1.query_logical_tensor(id_mgr["relu0_dst"]).get_mem_size()
                    / sizeof(float),
            0.0);
