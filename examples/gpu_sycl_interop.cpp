...
/// @example  gpu_sycl_interop.cpp
/// Annotated version: @ref gpu_sycl_interop_cpp
///
/// @page  gpu_sycl_interop_cpp Getting started on GPU with SYCL extensions API
/// Full example text: @ref gpu_sycl_interop.cpp
///
...
///   - Create a GPU engine. It uses SYCL as the runtime in this sample.
///   - Create a GPU memory descriptor/object.
///   - Create a SYCL kernel for GPU data initialization.
///   - Access a GPU memory via SYCL interoperability interface.
///   - Access a GPU queue via SYCL interoperability interface.
///   - Execute a SYCL kernel with related GPU queue and GPU memory.
...
///
/// @page gpu_sycl_interop_cpp

/// @section gpu_sycl_interop_cpp_headers Public headers
///
...
/// @page gpu_sycl_interop_cpp
/// @section gpu_sycl_interop_cpp_tutorial gpu_sycl_interop_tutorial() function
/// @page gpu_sycl_interop_cpp
void gpu_sycl_interop_tutorial() {

    /// @page gpu_sycl_interop_cpp
    /// @subsection gpu_sycl_interop_cpp_sub1 Engine and stream
    ///
    ...
    /// This example assumes SYCL being a runtime for GPU. In such case,
    /// during engine creation, an SYCL context is also created and attaches
    /// to the created engine.
    ///
    /// @snippet  gpu_sycl_interop.cpp Initialize engine
    // [Initialize engine]
    ...
    /// This example assumes SYCL being a runtime for GPU. During stream creation,
    /// a SYCL queue is also created and attaches to this stream.
    ///
    /// @snippet  gpu_sycl_interop.cpp Initialize stream
    // [Initialize stream]
    mkldnn::stream strm(eng);
    // [Initialize stream]

    /// @subsection  gpu_sycl_interop_cpp_sub2 Wrapping data into Intel MKL-DNN memory object
    ///
    ...
    /// @subsection  gpu_sycl_interop_cpp_sub3 Initialize the data executing a custom SYCL kernel
    ///
    ...
    ///
    /// @snippet  gpu_sycl_interop.cpp get sycl buf
    // [get sycl buf]
    ...
    /// @snippet gpu_sycl_interop.cpp sycl kernel exec
    // [sycl kernel exec]
    ...
    /// @subsection gpu_sycl_interop_cpp_sub4 Create and execute a primitive
    ...
    /// "heavier".
    /// @snippet gpu_sycl_interop.cpp relu creation
    //  [relu creation]
    ...
    /// @page gpu_sycl_interop_cpp
    /// @subsection gpu_sycl_interop_cpp_sub5 Validate the results
    ///
    ...
    /// @snippet gpu_sycl_interop.cpp Check the results
    // [Check the results]
...
/// @section gpu_sycl_interop_cpp_main main() function
///
...
/// @page gpu_sycl_interop_cpp
/// @snippet gpu_sycl_interop.cpp Main
// [Main]
int main(int argc, char **argv) {
    try {
        gpu_sycl_interop_tutorial();
