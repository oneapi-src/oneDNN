...
2. Adaptive max pooling is not supported but it can be obtained by using in max pooling the following formulas on stride:
   \f[
   ...
   \f[
      kernel_size = \ceil(\frac{2 \cdot input_size}{output_size}) - \floor(\frac{input_size}{output_size})
   \f]

   and padding:

   \f[
     padding = \frac{stride \cdot (output_size - 1) + kernel_size - input_size}{2}
   \f]

3. Adaptive average pooling is not supported but it can be obtained by using the same formulas as in adaptive max pooling but with #dnnl_pooling_avg_exclude_padding algorithm.
