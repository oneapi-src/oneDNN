...
/// @example implicit_concat_extended_input.cpp
//
/// This C++ API example demonstrates how to create and execute an implicit
/// concat feature managing memories properly and managing library to use
/// memory descriptors come from submemory API. Usage of both extended source
/// and destination is covered.
///
/// The following chain is used in this example:
/// Conv1 -> Pool1 -> Conv2 -> Concat -> Out
///   |                          ^
///   +---------------------------
...
void convolution_example(dnnl::engine::kind engine_kind) {
    ...
    memory::dim IC1 = 32, OC1 = IC1 /*any value*/;
    memory::dim IC2 = OC1, OC2 = IC2 /*any value*/;

    memory::dim N = 2, IH = 3, IW = 3, KH = 3, KW = 3, PH_L = 1, PH_R = 1,
    ...
    conv1_args.insert({DNNL_ARG_BIAS, conv_bias1_mem});
    conv1_args.insert({DNNL_ARG_DST, conv_dst1_mem});
    conv1_prim.execute(engine_stream, conv1_args);

    // ========================================================================
    // Create primitive descriptor for pool 1
    // ========================================================================
    // This pooling takes strided input and converts it into dense output.
    // Note: pooling re-uses convolution kernel not to shrink the output.
    // Be careful when changing those parameters.
    auto pool1_src_md = conv1_pd.dst_desc();
    auto pool1_dst_md = memory::desc(src2_dims, dts[0], tag::any);

    auto pool1_desc = pooling_forward::desc(prop_kind::forward_inference,
            algorithm::pooling_max, pool1_src_md, pool1_dst_md, strides_dims,
            kernel_dims, padding_dims_l, padding_dims_r);

    auto pool1_pd = pooling_forward::primitive_desc(pool1_desc, engine);

    auto pool_src1_mem = conv_dst1_mem;
    auto pool_dst1_mem = ocl_interop::make_memory(
            pool1_pd.dst_desc(), engine, ocl_interop::memory_kind::usm);

    auto pool1_prim = pooling_forward(pool1_pd);
    std::unordered_map<int, memory> pool1_args;
    pool1_args.insert({DNNL_ARG_SRC, pool_src1_mem});
    pool1_args.insert({DNNL_ARG_DST, pool_dst1_mem});
    pool1_prim.execute(engine_stream, pool1_args);

    // ========================================================================
    // Create primitive descriptor for conv 2
    // ========================================================================
    auto conv2_src_md = pool1_pd.dst_desc();
    ...
    auto conv_src2_mem = pool_dst1_mem;
    ...
    conv2_args.insert({DNNL_ARG_BIAS, conv_bias2_mem});
    ...
    // ========================================================================
    // Re-create primitive descriptor for conv 1 with updated destination
    // ========================================================================
    ...
    conv1_desc = convolution_forward::desc(prop_kind::forward_inference,
    ...
    conv1_upd_args.insert({DNNL_ARG_BIAS, conv_bias1_mem});
    conv1_upd_args.insert({DNNL_ARG_DST, conv_dst1_mem});
    conv1_prim.execute(engine_stream, conv1_upd_args);

    // ========================================================================
    // Re-create primitive descriptor for pool 1
    // ========================================================================

    pool1_desc = pooling_forward::desc(prop_kind::forward_inference,
            algorithm::pooling_max, conv1_pd.dst_desc(), pool1_pd.dst_desc(),
            strides_dims, kernel_dims, padding_dims_l, padding_dims_r);

    pool1_pd = pooling_forward::primitive_desc(pool1_desc, engine);

    pool_src1_mem = conv_dst1_mem;

    pool1_prim = pooling_forward(pool1_pd);
    std::unordered_map<int, memory> pool1_upd_args;
    pool1_upd_args.insert({DNNL_ARG_SRC, pool_src1_mem});
    pool1_upd_args.insert({DNNL_ARG_DST, pool_dst1_mem});
    pool1_prim.execute(engine_stream, pool1_upd_args);

    // ========================================================================
    // Re-create primitive descriptor for conv 2 with updated destination
    // ========================================================================

    memory::desc conv2_upd_dst_md = concat_dst_md.submemory_desc(
            dst2_dims, /*offsets=*/ {0, OC1, 0, 0});

    conv2_desc = convolution_forward::desc(prop_kind::forward_inference,
    ...
    conv2_upd_args.insert({DNNL_ARG_BIAS, conv_bias2_mem});
