...
cuDNN supports both explicit and implicit causal masks in its frontend and backend
APIs. In the frontend API, users can set SDPA attributes for various mask types,
including arbitrary user-generated mask, causal mask, padding mask, alibi mask and
sliding window mask on an SDPA node [3]. The backend API requires users to construct
a graph using operations like `GenIndex`, `Greater Than` and `Select` to indicate
a causal mask [4].
...
in the BERT definition in torchbenchmark [6], the SDPA forward is defined as:
...
`torch.nn.functional.scaled_dot_product_attention` [7], which is later lowered to
the aten operator for computation. This approach offers two options:

##### a. Explicit mask

For example, in gpt-fast [8]:
...
As seen in the nano_gpt implementation in torchbenchmark [9]:
...
`ScaledDotProductAttention` operation [11]. Users can either pass a generated mask
matrix or specify the `causal` flag to apply a causal mask.
...
Causal mask can be supported as a subgraph of `GenIndex`, `Greater Than` and
`Select` operations. Users need to construct a subgraph and pass it to the library,
and the library will fuse the subgraph into SDPA kernels based on the backend
capabilities. Since the sequence length of query (s_q) and key/value (s_kv) may not
be equal, the mask matrix can be divided into two categories, namely top-left and
bottom-right causal mask.

#### Option 1.1: Top-Left matrix generation

For causal masks in square matrices or top-left aligned matrices, e.g.,
...
#### Option 1.2: Bottom-Right matrix generation

For bottom-right alignment matrix, e.g.
...
User can construct the subgraph using the following structure:
...
following image), if the column index is less than the row index + s_kv - s_q, it
indicates future tokens. This condition can be represented with `Greater Than`,
`Add`, `Sub`and two`Constant` operations.
...
- This approach aligns with llama.cpp API design.
...
2. oneDNN Graph SDPA pattern, <https://oneapi-src.github.io/oneDNN/dev_guide_graph_sdpa.html>
3. cuDNN Frontend API support for causal mask, <https://github.com/NVIDIA/cudnn-frontend/blob/936021bfed8c91dc416af1588b2c4eca631a9e45/docs/operations/Attention.md>
4. cuDNN Backend API support for causal mask, <https://docs.nvidia.com/deeplearning/cudnn/latest/developer/graph-api.html#fused-flash-attention-fprop>
...
6. BERT implementation in PyTorch benchmarks, <https://github.com/pytorch/benchmark/blob/364420aeca07d9519840a5b6e771035e4dff9d72/torchbenchmark/models/BERT_pytorch/bert_pytorch/model/attention/single.py#L35>
7. PyTorch SDPA function, <https://pytorch.org/docs/stable/generated/torch.nn.functional.scaled_dot_product_attention.html>
8. gpt-fast, <https://github.com/pytorch-labs/gpt-fast/blob/32971d3129541c5bfb4f715abc33d1c5f408d204/model.py#L211>
9. nano_gpt implementation in PyTorch benchmarks, <https://github.com/pytorch/benchmark/blob/364420aeca07d9519840a5b6e771035e4dff9d72/torchbenchmark/models/nanogpt/model.py#L103>
10. PyTorch CPU flash attention implementation, <https://github.com/pytorch/pytorch/blob/6afcec0c582cb852fcf673ea3b6ce12e4b9da01d/aten/src/ATen/native/cpu/FlashAttentionKernel.cpp#L635-L643>
11. OpenVINO SDPA operation, <https://docs.openvino.ai/2024/documentation/openvino-ir-format/operation-sets/operation-specs/sequence/scaled-dot-product-attention.html>
