...
|1  | `dim0_indices`        | Row (Dimension 0) indices for the non-zero elements | 
|2  | `dim1_indices`        | Column (Dimension 1) indices for the non-zero elements |
|||
...
auto md_coo = memory::desc::coo(&coo_md, 2, {128, 64}, f32, nnz, s32);
```
...
```cpp
const int nnz = 12;
auto md_coo = memory::desc::coo({128, 64}, f32, nnz, s32);
```
...
char *dim0_indices = malloc(indices_size);
...
handles[1] = (void *)dim0_indices;
handles[2] = (void *)dim1_indices;

// Create a sparse memory object for the given buffers.
dnnl_memory_create_v2(coo_mem, md_coo, engine, nhandles, handles);
```

#### C++ API:
```cpp
// Memory creation with user-specified buffers
auto csr_mem = memory(coo_md, eng, {values, dim0_indices, dim1_indices});
```
