...
More details of the bfloat16 data type can be found at `Intel's site <https://software.intel.com/sites/default/files/managed/40/8b/bf16-hardware-numerics-definition-white-paper.pdf>`__ and `TensorFlow's documentation <https://cloud.google.com/tpu/docs/bfloat16>`__.

One of the advantages of using ``bf16`` versus ``f32`` is reduced memory footprint and, hence, increased memory access throughput. Additionally, when executing on hardware that supports `Intel DL Boost bfloat16 instructions <https://software.intel.com/sites/default/files/managed/c5/15/architecture-instruction-set-extensions-programming-reference.pdf>`__, ``bf16`` may offer an increase in computational throughput.
